...
function Account({ acknowledgeEmailWarning, onBack, onCreated }: { acknowledgeEmailWarning: boolean; onBack: () => void; onCreated: (email: string) => void }) {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) { event.preventDefault(); const data=new FormData(event.currentTarget); const name=String(data.get("name")).trim(); const email=String(data.get("email")); const password=String(data.get("password")); const confirmation=String(data.get("confirmation")); const bootstrapToken=String(data.get("bootstrapToken")).trim(); if(password!==confirmation){setError("Passwords do not match");return;} setBusy(true);setError("");try{await post("/api/onboarding/account",{name:name||undefined,email,password,passwordConfirmation:confirmation,acknowledgeEmailWarning,bootstrapToken});onCreated(email.trim().toLowerCase());}catch(cause){const err=cause as Error & {payload?:{redirect?:string}};if(err.payload?.redirect){location.assign(err.payload.redirect);return;}setError(err.message);first.current?.focus();}finally{setBusy(false);} }
  return <StepFrame number={1} eyebrow="Step 1 of 3" title="Create your admin account" lede="Set up secure dashboard access"><form onSubmit={submit}><div className={styles.fields}><Field label="Name" htmlFor="name" description="Optional. Shown in the user menu."><Input ref={first} id="name" name="name" type="text" autoComplete="name" maxLength={120}/></Field><Field label="Email" htmlFor="email"><Input id="email" name="email" type="email" autoComplete="email" required/></Field><Field label="Password" htmlFor="password" description="Use at least 12 characters. Password managers work well."><Input id="password" name="password" type="password" autoComplete="new-password" minLength={12} maxLength={128} required/></Field><Field label="Confirm Password" htmlFor="confirmation"><Input id="confirmation" name="confirmation" type="password" autoComplete="new-password" required/></Field><Field label="Setup Token" htmlFor="bootstrapToken" description="The PULSE_BOOTSTRAP_TOKEN you set in this deployment's environment. Proves you are the operator."><Input id="bootstrapToken" name="bootstrapToken" type="password" autoComplete="off" required/></Field></div>{error?<p className={styles.error} role="alert">{error}</p>:null}<div className={styles.actions}><Button size="icon" variant="secondary" aria-label="Back" onClick={onBack}><ArrowLeft size={16}/></Button><Button type="submit" disabled={busy}>{busy?"Creating…":"Create Account"}</Button></div></form></StepFrame>;
}
