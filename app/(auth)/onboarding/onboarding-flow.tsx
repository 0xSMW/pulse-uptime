...
function Account({ acknowledgeEmailWarning, onBack, onCreated }: { acknowledgeEmailWarning: boolean; onBack: () => void; onCreated: (email: string) => void }) {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) { event.preventDefault(); const data=new FormData(event.currentTarget); const email=String(data.get("email")); const password=String(data.get("password")); const confirmation=String(data.get("confirmation")); const bootstrapToken=String(data.get("bootstrapToken")).trim(); if(password!==confirmation){setError("Passwords do not match");return;} setBusy(true);setError("");try{await post("/api/onboarding/account",{email,password,passwordConfirmation:confirmation,acknowledgeEmailWarning,bootstrapToken});onCreated(email.trim().toLowerCase());}catch(cause){const err=cause as Error & {payload?:{redirect?:string}};if(err.payload?.redirect){location.assign(err.payload.redirect);return;}setError(err.message);first.current?.focus();}finally{setBusy(false);} }
