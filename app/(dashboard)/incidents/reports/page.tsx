...
import {
  ...
  ReportsEmpty,
} from "@/components/incidents/report-list"
import type {
  ReportListState,
  ReportListType,
} from "@/components/incidents/report-status"
...
function parseState(value: string | string[] | undefined): ReportListState {
  ...
  return state === "draft" || state === "ongoing" || state === "resolved"
    ? state
    : "all"
}
...
export default async function ReportsPage({
  ...
  if (cursorParam) {
    try {
      cursor = parseStatusReportListQuery({
        state,
        type,
        cursor: cursorParam,
      }).cursor
  ...
  const { data: reports, nextCursor } = await listStatusReportSummaries({
    state,
    type,
    cursor,
    limit: 100,
        ...
        <div className="flex items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-[13px] text-[var(--fg-muted)]">
              Status reports published to your status page
            </p>
          </div>
          <Link
            className={cn(
