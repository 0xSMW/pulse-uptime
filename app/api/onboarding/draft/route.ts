...
export async function POST(request: Request) {
  ...
  try {
    const draft = await saveMonitorDraft(auth.session.userId, await request.json());
    return NextResponse.json({ draft, nextStep: "verify" });
  } catch (error) {
    return NextResponse.json({ error: safeError(error, "Website validation failed") }, { status: 400 });
  }
}
