...
describe("login route failures", () => {
  it("returns generic 429 copy and Retry-After when limited", async () => {
    const response = loginFailure(new AuthServiceError("RATE_LIMITED", "Sign in failed", 37));
  ...
  it("preserves generic 401 responses for invalid credentials", async () => {
    const response = loginFailure(new AuthServiceError("INVALID_LOGIN", "Sign in failed"));
