...
const { idempotencyRecords } = vi.hoisted(() => ({
  idempotencyRecords: new Map<string, { status: number; body: unknown; operationId: string }>(),
}));
...
vi.mock("@/lib/api/idempotency", async (importOriginal) => ({
  ...
  executeIdempotent: vi.fn(async <T>({
    request,
    work,
    persistBody,
    replayBody,
    ...
    work: (context: {
      operationId: string;
      transaction: (run: (tx: unknown) => Promise<{ status: number; body: T }>) => Promise<{ status: number; body: T }>;
    }) => Promise<{ status: number; body: T }>;
    persistBody?: (body: T) => unknown;
    replayBody?: (stored: unknown, context: { operationId: string; transaction: never }) => T | Promise<T>;
  }) => {
...
import { createApiToken, validateTokenInput, type TokenRecord } from "@/lib/api/token-service";
...
async function createAndReplay(context: ApiContext, clamped: boolean) {
  ...
  const first = await POST(request({ name: "Deploy", scopes: ["monitors:read"] }, key));
  const firstBody = await first.json();
  const second = await POST(request({ name: "Deploy", scopes: ["monitors:read"] }, key));
...
describe("CreatedToken persist/replay helpers", () => {
  ...
  it("retains future non-secret fields automatically via typed omit", () => {
    ...
    const persisted = persistCreatedToken(extended) as PersistedCreatedTokenData & { futureField?: string };
...
describe("POST /api/v1/tokens idempotency contract", () => {
  it("returns expiryClamped false on create and replay when unclamped", async () => {
    const { first, firstBody, second, secondBody } = await createAndReplay(humanContext, false);
  ...
  it("returns expiryClamped true on create and replay when clamped", async () => {
    const { first, firstBody, second, secondBody } = await createAndReplay(cliContext, true);
    expect(first.status).toBe(201);
    expect(second.status).toBe(201);
  ...
  it("stores a body without the secret and still reconstructs it on replay", async () => {
    const { firstBody, secondBody, key } = await createAndReplay(humanContext, false);
    ...
    expect(firstBody.data.token).toBe("pulse_live_derived-secret-value-aaaaaaaaaaa");
    expect(secondBody.data.token).toBe("pulse_live_derived-secret-value-aaaaaaaaaaa");
    expect(firstBody.data).toEqual(secondBody.data);
  });
});
