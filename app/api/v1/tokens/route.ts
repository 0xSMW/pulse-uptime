...
export function persistCreatedToken(
  body: CreatedTokenData
): PersistedCreatedTokenData {
...
export async function POST(request: Request) {
  const context = await authorize(request, {
    scope: "tokens:manage",
    rateLimit: TOKEN_CREATE_LIMIT,
  })
  if (isApiResponse(context)) {
    return context
  }
  try {
    const { clamped, ...input } = validateTokenInput(
      await request.json(),
      context.principal
    )
...
export async function GET(request: Request) {
  ...
  return apiJson(
    listEnvelope(
      "TokenList",
      page.tokens.map(serializeToken),
      context.requestId,
