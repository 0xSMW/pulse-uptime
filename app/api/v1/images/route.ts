...
import { createImage, ImageServiceError, MAX_IMAGE_BYTES } from "@/lib/api/images";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    return apiError(context.requestId, 400, "INVALID_FORM", 'Request must be multipart/form-data with "file" and "kind" fields');
  }
  ...
  if (file.size > MAX_IMAGE_BYTES) {
    return apiError(context.requestId, 400, "IMAGE_TOO_LARGE", `Images must be at most ${Math.floor(MAX_IMAGE_BYTES / 1024)} KB`);
  }
  try {
    ...
    return apiJson(objectEnvelope("Image", { id }, context.requestId), { status: 201 });
  } catch (error) {
