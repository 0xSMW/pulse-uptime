...
function imageRequest(id = IMAGE_ID) {
  return GET(new Request(`https://pulse.test/api/v1/images/${id}`), { params: Promise.resolve({ imageId: id }) });
}
...
describe("GET /api/v1/images/{imageId}", () => {
  it("refuses bearer principals with SESSION_REQUIRED", async () => {
    vi.mocked(authorize).mockResolvedValue({
      ...humanContext,
      principal: { type: "api_token", id: "tok-1", name: "agent", scopes: ["config:write"], expiresAt: new Date() },
    });
