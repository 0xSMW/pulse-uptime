...
const context: ApiContext = {
  principal: { type: "human", id: "user-1", email: "admin@example.com", scopes: ["config:write"] },
...
function uploadRequest(parts: { file?: File; kind?: string }) {
  ...
  if (parts.file) form.append("file", parts.file);
  if (parts.kind !== undefined) form.append("kind", parts.kind);
  return new Request("https://pulse.test/api/v1/images", { method: "POST", body: form });
}
...
beforeEach(() => {
  ...
  vi.spyOn(databaseImageStore, "insert").mockResolvedValue({ id: "44444444-4444-4444-8444-444444444444" });
});
...
describe("POST /api/v1/images", () => {
  ...
  it("returns the authorization failure untouched", async () => {
    ...
    const response = await POST(uploadRequest({ file: pngFile(), kind: "avatar" }));
  ...
  it("stores a valid upload and returns the id in an envelope", async () => {
    ...
    expect(databaseImageStore.insert).toHaveBeenCalledWith(expect.objectContaining({
      kind: "avatar", mimeType: "image/png", byteSize: PNG.length,
    }));
  });
  ...
  it("rejects unknown kinds and disallowed mime types", async () => {
    const badKind = await POST(uploadRequest({ file: pngFile(), kind: "banner" }));
    expect(badKind.status).toBe(400);
    expect((await badKind.json()).error.code).toBe("INVALID_KIND");

    const badMime = await POST(uploadRequest({ file: pngFile(PNG, "image/gif"), kind: "avatar" }));
  ...
  it("rejects content that does not match the declared type", async () => {
    const spoofed = new File([new Uint8Array(Buffer.from("<svg onload=alert(1)>"))], "logo.png", { type: "image/png" });
    const response = await POST(uploadRequest({ file: spoofed, kind: "logo-light" }));
    expect(response.status).toBe(400);
    expect((await response.json()).error.code).toBe("INVALID_IMAGE");
  });

  it("enforces the 512 KB general cap before buffering", async () => {
    const oversized = pngFile(Buffer.concat([PNG, Buffer.alloc(MAX_IMAGE_BYTES)]));
    const response = await POST(uploadRequest({ file: oversized, kind: "logo-light" }));
    expect(response.status).toBe(400);
  ...
  it("enforces the 32 KB favicon cap", async () => {
    const icoBytes = Buffer.concat([Buffer.from([0x00, 0x00, 0x01, 0x00]), Buffer.alloc(32 * 1024)]);
    const favicon = new File([new Uint8Array(icoBytes)], "favicon.ico", { type: "image/x-icon" });
    const response = await POST(uploadRequest({ file: favicon, kind: "favicon" }));
    expect(response.status).toBe(400);
    expect((await response.json()).error.code).toBe("IMAGE_TOO_LARGE");
  });
});
