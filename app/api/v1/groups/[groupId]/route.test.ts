...
const context: ApiContext = {
  principal: { type: "api_token", id: "tok-1", name: "agent", scopes: ["monitors:write"], expiresAt: new Date() },
...
describe("PATCH /api/v1/groups/{groupId}", () => {
  ...
  it("renames a group and returns 200 with the group envelope", async () => {
    vi.mocked(updateGroup).mockResolvedValue({ id: "production", name: "Core", monitorCount: 0 });
    const response = await PATCH(patchRequest({ name: "Core" }), params);
    expect(response.status).toBe(200);
    const payload = await response.json();
    expect(payload.kind).toBe("Group");
    expect(payload.data).toEqual({ id: "production", name: "Core", monitorCount: 0 });
  ...
  it("stores a GROUP_NOT_FOUND domain error as the operation's own completed 404, the same status a first attempt maps to", async () => {
    vi.mocked(updateGroup).mockRejectedValue(new GroupApiError("GROUP_NOT_FOUND", "Group was not found"));
    const response = await PATCH(patchRequest({ name: "Core" }), params);
    ...
    expect(completions[0].body).toMatchObject({ kind: "Error", error: { code: "GROUP_NOT_FOUND" } });
  });

  it("stores a GROUP_EXISTS domain error as the operation's own completed 409", async () => {
    vi.mocked(updateGroup).mockRejectedValue(new GroupApiError("GROUP_EXISTS", "A group with this name already exists"));
    const response = await PATCH(patchRequest({ name: "Core" }), params);
  ...
  it("replays a stored completion verbatim without re-invoking updateGroup", async () => {
    const storedBody = {
      ...
      error: { code: "GROUP_NOT_FOUND", message: "Group was not found", details: {}, requestId: "req_groups" },
    };
    vi.mocked(executeIdempotent).mockResolvedValue({ status: 404, body: storedBody, replayed: true });
    const response = await PATCH(patchRequest({ name: "Core" }), params);
    expect(response.status).toBe(404);
...
describe("DELETE /api/v1/groups/{groupId}", () => {
  ...
  it("deletes a group and returns 200 with the deletion envelope", async () => {
    vi.mocked(deleteGroup).mockResolvedValue({ id: "production", deleted: true });
  ...
  it("stores a GROUP_NOT_EMPTY domain error as the operation's own completed 409, the same status a first attempt maps to", async () => {
    vi.mocked(deleteGroup).mockRejectedValue(new GroupApiError("GROUP_NOT_EMPTY", "Move or ungroup monitors before deleting this group", { monitorCount: 2 }));
    const response = await DELETE(deleteRequest(), params);
    expect(response.status).toBe(409);
    const payload = await response.json();
    ...
    expect(completions).toMatchObject([{ status: 409 }]);
    expect(completions[0].body).toMatchObject({ kind: "Error", error: { code: "GROUP_NOT_EMPTY" } });
  });

  it("stores a GROUP_NOT_FOUND domain error as the operation's own completed 404", async () => {
    vi.mocked(deleteGroup).mockRejectedValue(new GroupApiError("GROUP_NOT_FOUND", "Group was not found"));
    const response = await DELETE(deleteRequest(), params);
    expect(response.status).toBe(404);
    expect(completions).toMatchObject([{ status: 404 }]);
  });
  ...
  it("replays a stored completion verbatim without re-invoking deleteGroup", async () => {
    const storedBody = {
      apiVersion: "v1", kind: "Error",
      error: { code: "GROUP_NOT_EMPTY", message: "Move or ungroup monitors before deleting this group", details: { monitorCount: 2 }, requestId: "req_groups" },
    };
    vi.mocked(executeIdempotent).mockResolvedValue({ status: 409, body: storedBody, replayed: true });
    const response = await DELETE(deleteRequest(), params);
    expect(response.status).toBe(409);
    expect(await response.json()).toEqual(storedBody);
