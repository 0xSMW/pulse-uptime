...
const context: ApiContext = {
  principal: { type: "api_token", id: "tok-1", name: "agent", scopes: ["monitors:read", "monitors:write"], expiresAt: new Date() },
...
describe("GET /api/v1/groups", () => {
  beforeEach(() => {
    ...
    vi.mocked(listGroups).mockReset().mockResolvedValue([{ id: "production", name: "Production", monitorCount: 0 }]);
  });
  ...
  it("returns the group list envelope", async () => {
    ...
    expect(payload.data).toEqual([{ id: "production", name: "Production", monitorCount: 0 }]);
  });
});
...
describe("POST /api/v1/groups", () => {
  ...
  it("creates a group and returns 201 with the group envelope", async () => {
    vi.mocked(createGroup).mockResolvedValue({ id: "production", name: "Production", monitorCount: 0 });
    const response = await POST(postRequest({ id: "production", name: "Production" }));
  ...
  it("stores a GROUP_EXISTS domain error as the operation's own completed 409, the same status a first attempt maps to", async () => {
    vi.mocked(createGroup).mockRejectedValue(new GroupApiError("GROUP_EXISTS", "A group with this name already exists"));
    const response = await POST(postRequest({ id: "production", name: "Production" }));
    ...
    expect(completions[0].body).toMatchObject({ kind: "Error", error: { code: "GROUP_EXISTS" } });
  });
  ...
  it("replays a stored completion verbatim without re-invoking createGroup", async () => {
    const storedBody = {
      ...
      error: { code: "GROUP_EXISTS", message: "A group with this name already exists", details: {}, requestId: "req_groups" },
    };
