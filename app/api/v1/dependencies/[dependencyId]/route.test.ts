...
beforeEach(() => {
  ...
  vi.mocked(requireDependencyDetail)
    .mockReset()
    .mockResolvedValue(dependency as never)
  vi.mocked(patchDependency)
    .mockReset()
    .mockResolvedValue(dependency as never)
  vi.mocked(removeDependency)
    .mockReset()
    .mockResolvedValue({ id: "dep-1", removed: true })
...
describe("GET /api/v1/dependencies/{dependencyId}", () => {
  it("requires the dependencies:read scope and returns the Dependency envelope", async () => {
    const response = await GET(
      new Request("https://pulse.test/api/v1/dependencies/dep-1"),
      params
    )
  ...
  it("maps DEPENDENCY_NOT_FOUND to 404", async () => {
    vi.mocked(requireDependencyDetail).mockRejectedValue(
...
describe("PATCH /api/v1/dependencies/{dependencyId}", () => {
  ...
  it("requires the dependencies:write scope and changes only notificationsEnabled", async () => {
    const response = await PATCH(
      patchRequest({ notificationsEnabled: false }),
      params
    )
    expect(authorize).toHaveBeenCalledWith(expect.any(Request), {
    ...
    expect(response.status).toBe(200)
    expect(patchDependency).toHaveBeenCalledWith(
      "dep-1",
  ...
  it("maps DEPENDENCY_NOT_FOUND to 404", async () => {
    vi.mocked(patchDependency).mockRejectedValue(
      new DependencyApiError("DEPENDENCY_NOT_FOUND", "Dependency was not found")
    )
    const response = await PATCH(
      patchRequest({ notificationsEnabled: false }),
...
describe("DELETE /api/v1/dependencies/{dependencyId}", () => {
  ...
  it("maps DEPENDENCY_NOT_FOUND to 404 for a fresh key on an already-absent dependency", async () => {
    vi.mocked(removeDependency).mockRejectedValue(
      new DependencyApiError("DEPENDENCY_NOT_FOUND", "Dependency was not found")
    )
    const response = await DELETE(deleteRequest(), params)
    expect(response.status).toBe(404)
