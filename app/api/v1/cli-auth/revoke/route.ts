import {
  resolveRevokedCliRevokeReplay,
  revokeCliInstallation,
...
export async function POST(request: Request) {
  ...
  const replay = isApiResponse(authorized)
    ? await resolveRevokedCliRevokeReplay(request)
    : null
  if (isApiResponse(authorized) && !replay) {
    return authorized
  }
  const context = isApiResponse(authorized)
    ? {
        principal: { type: "cli_session", id: replay!.id },
        principalKey: replay!.principalKey,
