...
export async function POST(request: Request) {
  ...
  try {
    const body = await request.json() as { baseConfigHash?: unknown; targetConfig?: unknown };
    if (typeof body.baseConfigHash !== "string") return apiError(context.requestId, 400, "INVALID_REQUEST", "baseConfigHash is required");
    const plan = await configurationService.plan({ baseConfigHash: body.baseConfigHash, targetConfig: body.targetConfig });
    return apiJson(objectEnvelope("ConfigurationPlan", {
    ...
    return apiError(context.requestId, code === "CONFIG_VERSION_CONFLICT" ? 409 : 500, code ?? "CONFIG_PLAN_FAILED", "Configuration plan could not be created");
  }
}
