...
describe("POST /api/v1/me/password", () => {
  it("refuses bearer principals with SESSION_REQUIRED", async () => {
    ...
    const response = await POST(passwordRequest({ currentPassword: "old", newPassword: "new-password-123" }));
  ...
  it("threads the session and first forwarded hop into the service", async () => {
    ...
    const response = await POST(passwordRequest({ currentPassword: "old-password-12", newPassword: "new-password-123" }));
  ...
  it("prefers the platform-set x-real-ip over x-forwarded-for", async () => {
    vi.mocked(changeAccountPassword).mockResolvedValue({ changed: true });
    await POST(new Request("https://pulse.test/api/v1/me/password", {
      method: "POST",
      headers: { "x-real-ip": "198.51.100.9", "x-forwarded-for": "203.0.113.7, 10.0.0.1" },
      body: JSON.stringify({ currentPassword: "old-password-12", newPassword: "new-password-123" }),
    }));
    expect(changeAccountPassword).toHaveBeenCalledWith(expect.objectContaining({ ip: "198.51.100.9" }));
  });

  it("maps a wrong current password to 403", async () => {
    vi.mocked(changeAccountPassword).mockRejectedValue(
      new AccountServiceError("INVALID_PASSWORD", "Current password is incorrect"),
    );
    const response = await POST(passwordRequest({ currentPassword: "wrong", newPassword: "new-password-123" }));
    expect(response.status).toBe(403);
    const payload = await response.json();
  ...
  it("maps a policy violation to 400", async () => {
    ...
    const response = await POST(passwordRequest({ currentPassword: "old", newPassword: "short" }));
  ...
  it("returns 429 with Retry-After when the login limiter blocks", async () => {
    vi.mocked(changeAccountPassword).mockRejectedValue(
      new AccountServiceError("RATE_LIMITED", "Too many attempts. Try again later.", 42),
    );
    const response = await POST(passwordRequest({ currentPassword: "old", newPassword: "new-password-123" }));
  ...
  it("rejects unsupported body fields before calling the service", async () => {
    const response = await POST(passwordRequest({ currentPassword: "old", newPassword: "new-password-123", extra: 1 }));
    expect(response.status).toBe(400);
    expect(changeAccountPassword).not.toHaveBeenCalled();
  });
});
