...
beforeEach(() => {
  ...
  vi.mocked(listStatusReportSummaries)
    .mockReset()
    .mockResolvedValue({ data: [listRow], nextCursor: "cursor-2" })
...
describe("GET /api/v1/status-reports", () => {
  ...
  it("returns the authorization failure untouched", async () => {
    ...
    const response = await GET(
      new Request("https://pulse.test/api/v1/status-reports")
    )
  ...
  it("returns the lean list envelope with the cursor and threads filters", async () => {
    const response = await GET(
      new Request(
        "https://pulse.test/api/v1/status-reports?state=draft&type=incident&limit=10"
      )
    )
    ...
    expect(listStatusReportSummaries).toHaveBeenCalledWith({
      state: "draft",
      type: "incident",
      cursor: null,
      limit: 10,
  ...
  it("rejects invalid state filters and limits", async () => {
    const bad = await GET(
      new Request("https://pulse.test/api/v1/status-reports?state=open")
    )
    expect(bad.status).toBe(400)
    expect((await bad.json()).error.code).toBe("VALIDATION_ERROR")
    const limit = await GET(
      new Request("https://pulse.test/api/v1/status-reports?limit=0")
    )
...
describe("POST /api/v1/status-reports", () => {
  ...
  it("maps validation failures to 400 VALIDATION_ERROR", async () => {
    vi.mocked(createStatusReport).mockRejectedValue(
      new StatusReportError("VALIDATION_ERROR", "Title is required")
    )
