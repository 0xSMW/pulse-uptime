...
import { DatabaseHealthUnavailableError, refreshDatabaseHealth } from "@/lib/database-health";
...
export async function POST(request: Request) {
  ...
  try {
    const result = await executeIdempotent({
      ...
      work: async () => ({
        status: 200,
        body: objectEnvelope("DatabaseHealth", await refreshDatabaseHealth(), context.requestId),
      }),
    });
