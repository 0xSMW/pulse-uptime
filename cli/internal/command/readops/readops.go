...
func List(ctx context.Context, client Client, options ListOptions) (ListEnvelope, error) {
	...
	result, err := paginator.Aggregate(ctx, paginator.Options{
		...
		APIVersion: "v1", Kind: "IncidentList", PageName: "incident", RecordName: "incidents", LimitError: pageLimit,
	}, func(ctx context.Context, query url.Values) (paginator.Page[Meta], error) {
		var page ListEnvelope
		if err := client.Do(ctx, Request{Method: http.MethodGet, Path: "/api/v1/incidents", Query: query, Result: &page}); err != nil {
