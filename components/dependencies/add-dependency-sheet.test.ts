...
function preset(
  overrides: Partial<DependencyCatalogPreset> & Pick<DependencyCatalogPreset, "id" | "scopeSelection">,
...
const categories: DependencyCatalogCategory[] = [
  {
    category: "ai",
    presets: [
      preset({ id: "openai_api", name: "OpenAI API", provider: "OpenAI", scopeSelection: null }),
      preset({ id: "chatgpt", name: "ChatGPT", provider: "OpenAI", scopeSelection: null }),
    ],
  },
  {
    category: "data",
...
describe("selectedScopeForPreset", () => {
  ...
  it("returns null scope for presets with no scopeSelection", () => {
    expect(selectedScopeForPreset(preset({ id: "openai_api", scopeSelection: null }), {}, "openai_api")).toEqual({
      ready: true,
      scopeId: null,
    });
  });
  ...
  it("blocks required discovered selectors until an available option is chosen", () => {
    const droplets = preset({ id: "digitalocean_droplets", scopeSelection: requiredDiscovered });
    expect(selectedScopeForPreset(droplets, {}, "digitalocean_droplets")).toEqual({ ready: false });
    expect(selectedScopeForPreset(droplets, { digitalocean_droplets: "nyc1" }, "digitalocean_droplets")).toEqual({
      ready: true,
      scopeId: "nyc1",
    });
  });
  ...
  it("rejects an unavailable discovered option even if selected", () => {
    const droplets = preset({ id: "digitalocean_droplets", scopeSelection: requiredDiscovered });
    expect(selectedScopeForPreset(droplets, { digitalocean_droplets: "sfo3" }, "digitalocean_droplets")).toEqual({
      ready: false,
    });
  });
  ...
  it("rejects unknown option ids", () => {
    const droplets = preset({ id: "digitalocean_droplets", scopeSelection: requiredDiscovered });
    expect(selectedScopeForPreset(droplets, { digitalocean_droplets: "bogus" }, "digitalocean_droplets")).toEqual({
      ready: false,
    });
  });

  it("defaults optional location scopes to unscoped (All locations)", () => {
    const locations = preset({ id: "google_cloud_storage", scopeSelection: optionalLocations });
    expect(selectedScopeForPreset(locations, {}, "google_cloud_storage")).toEqual({
      ready: true,
      scopeId: null,
    });
    expect(
  ...
  it("accepts an available location when optional scopes pick a region", () => {
    const locations = preset({ id: "google_cloud_storage", scopeSelection: optionalLocations });
    expect(
      selectedScopeForPreset(locations, { google_cloud_storage: "us-central1" }, "google_cloud_storage"),
  ...
  it("uses the same path for static required scopes as discovered ready scopes", () => {
    ...
    expect(selectedScopeForPreset(neon, {}, "neon_database")).toEqual({ ready: false });
    expect(selectedScopeForPreset(neon, { neon_database: "aws-us-east-1" }, "neon_database")).toEqual({
      ready: true,
  ...
  it("blocks install while discovery is pending", () => {
    ...
    expect(selectedScopeForPreset(pending, {}, "supabase_database")).toEqual({ ready: false });
    expect(
      selectedScopeForPreset(pending, { supabase_database: "anything" }, "supabase_database"),
    ).toEqual({ ready: false });
  });
  ...
  it("allows optional unscoped install while discovery is still pending", () => {
    ...
    expect(selectedScopeForPreset(pendingOptional, {}, "google_cloud_storage")).toEqual({
      ready: true,
      scopeId: null,
    });
    expect(
  ...
  it("blocks install when discovery reports unavailable", () => {
    ...
    expect(selectedScopeForPreset(unavailable, {}, "upstash_redis_regional")).toEqual({ ready: false });
  });
  ...
  it("gates Enter-key submission the same way as the Add button", () => {
    ...
    const after = selectedScopeForPreset(droplets, { digitalocean_droplets: "nyc1" }, "digitalocean_droplets");
    ...
    if (unscoped.ready) {
      expect(unscoped.scopeId ? { scopeId: unscoped.scopeId } : { presetId: "gcp" }).toEqual({
        presetId: "gcp",
      });
    }
  });
...
describe("showsScopeSelector", () => {
  it("shows for static and ready only", () => {
    ...
    expect(
      showsScopeSelector({ required: true, allowsUnscoped: false, status: "static", options: [] }),
    ).toBe(true);
    expect(
      showsScopeSelector({
        required: true,
    ...
    expect(
      showsScopeSelector({ required: true, allowsUnscoped: false, status: "pending", options: [] }),
    ).toBe(false);
    expect(
      showsScopeSelector({
        required: true,
