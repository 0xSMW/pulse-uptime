...
import { dependencyPendingLabel, dependencyStateLabels, dependencyStatusLabel } from "./dependency-status";
...
describe("dependencyStatusLabel", () => {
  ...
  it("never surfaces the raw Unknown label while pending", () => {
    expect(dependencyStatusLabel("UNKNOWN", true)).not.toBe(dependencyStateLabels.UNKNOWN);
  });
});
