  ...
  return (
    <section aria-labelledby="dependencies-title" className="mt-8">
      <div className="mb-4 flex items-baseline justify-between gap-4">
        <div>
          <h2 id="dependencies-title" className="text-xl font-semibold tracking-[-0.02em]">Dependencies</h2>
          <p className="mt-1 text-[13px] text-[var(--fg-muted)]">Provider reported</p>
        </div>
      </div>
        ...
        <table className="w-full min-w-[820px] border-collapse text-left text-[13px]">
          ...
          <tbody>
            {dependencies.map((dependency) => (
              <DependencyPanelRow key={dependency.id} dependency={dependency} />
            ))}
          </tbody>
        </table>
      </div>
