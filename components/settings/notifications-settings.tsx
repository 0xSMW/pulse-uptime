...
type ConfigurationOperation = { id: string; state: "written" | "accepted" | "rejected" | "failed"; rejectionReason: string | null };
...
export function NotificationsSettings({ data }: { data: NotificationSettingsData }) {
  ...
  function pollOperation(operationId: string, deadline: number, committedText: string) {
    const tick = async () => {
      pollTimer.current = null;
      try {
        const envelope = await apiRequest<ApiEnvelope<ConfigurationOperation>>(`/api/v1/config/operations/${encodeURIComponent(operationId)}`);
        ...
        if (state === "rejected" || state === "failed") {
          setNotificationStatus(rejectionReason || "Configuration update failed. Reload and try again.");
          setNotificationBusy(null);
          return;
        }
      } catch {
      ...
      pollTimer.current = window.setTimeout(() => { void tick(); }, OPERATION_POLL_INTERVAL_MS);
    };
    pollTimer.current = window.setTimeout(() => { void tick(); }, OPERATION_POLL_INTERVAL_MS);
  }
  ...
  async function saveRecipients() {
    ...
    try {
      ...
      const applied = await apiRequest<ApiEnvelope<ConfigurationOperation>>("/api/v1/config/apply", {
        method: "POST",
  ...
  async function sendTestNotification() {
    ...
    finally { setNotificationBusy((current) => current === "test" ? null : current); }
  }
        ...
        <CardContent className="pt-0">
          ...
          <div className="max-w-[640px] space-y-4">
            <label className="block">
              <span className="mb-2 block text-[13px] font-medium">Default Recipients</span>
              <textarea value={recipientsText} onChange={(e) => setRecipientsText(e.target.value)} rows={Math.max(3, Math.min(recipients.length || 3, 6))} aria-invalid={Boolean(recipientError)} placeholder="ops@example.com" className="w-full resize-y rounded-[6px] border border-[var(--border-strong)] bg-[var(--bg)] px-3 py-2 font-data text-[13px] leading-5" />
              {recipientError ? <span className="mt-1 block text-xs text-[var(--down-text)]">{recipientError}</span> : null}
            </label>
            <div className="flex flex-wrap items-center justify-between gap-3 border-t border-[var(--border)] pt-4">
              <span className="font-data text-[13px] text-[var(--fg-muted)]">{data.sender ? `${data.sender} via Resend` : "Email sender is not configured"}</span>
              <div className="flex gap-2">
                <Button variant="secondary" onClick={sendTestNotification} disabled={Boolean(notificationBusy)}>{notificationBusy === "test" ? "Sending…" : "Send Test Email"}</Button>
                <Button onClick={saveRecipients} disabled={Boolean(notificationBusy) || Boolean(recipientError)}>{notificationBusy === "save" ? "Saving…" : "Save Recipients"}</Button>
              </div>
            </div>
            {notificationStatus ? <p aria-live="polite" className={`text-[13px] ${notificationStatus.includes("changed elsewhere") || notificationStatus.includes("unavailable") || notificationStatus.includes("failed") ? "text-[var(--down-text)]" : "text-[var(--fg-muted)]"}`}>{notificationStatus}</p> : null}
          </div>
        </CardContent>
      </Card>
    </div>
