...
import { AccountSettings, initialsFor, type AccountSettingsData } from "./account-settings";
...
describe("AccountSettings", () => {
  ...
  it("renders the stored avatar from the authenticated image route", () => {
    ...
    expect(avatar?.getAttribute("src")).toBe("/api/v1/images/11111111-1111-4111-8111-111111111111");
  });
  ...
  it("renders theme and time-zone preferences with per-scope descriptions", () => {
    ...
    expect(screen.getByText("Use a different time zone on this device")).toBeDefined();
  });
  ...
  it("blocks the email change while the confirmation does not match", () => {
    ...
    fireEvent.change(screen.getByLabelText("Confirm new email"), { target: { value: "other@example.com" } });
    ...
    const submit = screen.getByRole("button", { name: "Change Email" }) as HTMLButtonElement;
