  ...
  return (
    ...
    >
      <form ref={formRef} onSubmit={submit} className="space-y-4">
        ...
        <Collapsible open={advancedOpen} onOpenChange={setAdvancedOpen} className="-mx-6 border-y border-[var(--border)] px-6">
        ...
        <label className="-mx-6 flex items-center justify-between gap-4 border-y border-[var(--border)] px-6 py-3 text-[13px] font-medium"><span>Enabled</span><input type="checkbox" checked={values.enabled} onChange={(e) => set("enabled", e.target.checked)} className="size-4 accent-[var(--fg)]" /></label>
        ...
        <div className="flex justify-end gap-2 pt-1"><Button type="button" variant="secondary" onClick={onClose} disabled={Boolean(busy)}>Cancel</Button><Button type="submit" disabled={Boolean(busy)}>{busy === "save" ? "Saving…" : busy === "propagation" ? "Updating…" : monitor ? "Save Monitor" : "Create Monitor"}</Button></div>
      </form>
    </Sheet>
