...
import { passwordPolicyError, SecuritySettings, type SecuritySettingsData } from "./security-settings";
...
describe("passwordPolicyError", () => {
  it("mirrors the 12-128 server policy", () => {
    ...
    expect(passwordPolicyError("a".repeat(129))).toBe("Use no more than 128 characters");
...
describe("SecuritySettings password form", () => {
  ...
  it("posts the change, clears the fields, and announces success", async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      new Response(JSON.stringify({ data: { changed: true } }), { status: 200 }),
    );
    ...
    await waitFor(() => {
      expect(screen.getByText("Password changed. Your other sessions have been signed out.")).toBeDefined();
    });
    expect(fetchMock).toHaveBeenCalledWith("/api/v1/me/password", expect.objectContaining({ method: "POST" }));
...
describe("SecuritySettings sessions", () => {
  ...
  it("confirms before signing out all other sessions and reports the count", async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      new Response(JSON.stringify({ data: { revokedCount: 1 } }), { status: 200 }),
    );
    vi.stubGlobal("fetch", fetchMock);
    renderSecurity();
  ...
  it("disables the sign-out-others action when this is the only session", () => {
    renderSecurity({ sessions: data.sessions.filter((session) => session.current) });
    const button = screen.getByRole("button", { name: "Sign Out Other Sessions" }) as HTMLButtonElement;
