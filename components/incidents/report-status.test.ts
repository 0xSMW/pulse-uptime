...
const updates: ReportUpdateData[] = [
  { id: "u2", status: "resolved", markdown: "Fixed.", publishedAt: "2026-07-18T12:00:00.000Z" },
  { id: "u1", status: "monitoring", markdown: "Watching.", publishedAt: "2026-07-18T10:00:00.000Z" },
];
...
describe("currentStatusAfterEdit", () => {
  ...
  it("recomputes the latest when a timestamp is backdated", () => {
    expect(
      currentStatusAfterEdit(updates, { id: "u2", publishedAt: "2026-07-18T09:00:00.000Z" }),
    ).toBe("monitoring");
  });

  it("uses the original order as the tiebreak for identical timestamps", () => {
    expect(
      currentStatusAfterEdit(updates, { id: "u1", publishedAt: "2026-07-18T12:00:00.000Z" }),
    ).toBe("resolved");
  });
  ...
  it("breaks publishedAt ties by createdAt, matching the server total order", () => {
    ...
    expect(
      currentStatusAfterEdit(withCreated, { id: "b", publishedAt: "2026-07-18T12:00:00.000Z" }),
    ).toBe("monitoring");
  });

  it("breaks full (publishedAt, createdAt) ties by id, matching the server", () => {
    const tied: ReportUpdateData[] = [
      { id: "aaa", status: "resolved", markdown: "Fixed.", publishedAt: "2026-07-18T12:00:00.000Z", createdAt: "2026-07-18T12:00:00.000Z" },
      { id: "zzz", status: "monitoring", markdown: "Watching.", publishedAt: "2026-07-18T12:00:00.000Z", createdAt: "2026-07-18T12:00:00.000Z" },
    ];
...
describe("stateFlipDirection", () => {
  it("returns null when the report state does not change", () => {
    expect(stateFlipDirection(updates, { id: "u2", status: "completed" })).toBeNull();
    expect(stateFlipDirection(updates, { id: "u1", status: "identified" })).toBeNull();
  });

  it("detects a backdate that moves the report back to Ongoing", () => {
    expect(
      stateFlipDirection(updates, { id: "u2", publishedAt: "2026-07-18T09:00:00.000Z" }),
    ).toBe("to_ongoing");
  ...
  it("detects a status edit that resolves the report", () => {
    ...
    expect(stateFlipDirection(ongoing, { id: "u1", status: "resolved" })).toBe("to_resolved");
  });

  it("detects a status edit that reopens the report", () => {
    expect(stateFlipDirection(updates, { id: "u2", status: "monitoring" })).toBe("to_ongoing");
  });
  ...
  it("fires for a minute-precision tie that the server resolves by createdAt", () => {
    ...
    expect(
      stateFlipDirection(withCreated, { id: "b", publishedAt: "2026-07-18T12:00:00.000Z" }),
    ).toBe("to_ongoing");
  });
});
...
describe("stateFlipAfterRemoval", () => {
  ...
  it("warns when deleting the latest ongoing update resolves the report", () => {
    const reopened: ReportUpdateData[] = [
      { id: "u2", status: "monitoring", markdown: "Watching.", publishedAt: "2026-07-18T12:00:00.000Z" },
      { id: "u1", status: "resolved", markdown: "Fixed.", publishedAt: "2026-07-18T10:00:00.000Z" },
    ];
...
describe("validateReportForm", () => {
  ...
  it("requires a title", () => {
    expect(validateReportForm({ ...valid, title: "  " }).title).toBe("Enter a title");
    expect(validateReportForm({ ...valid, title: "a".repeat(161) }).title).toBe("Use 160 characters or fewer");
  });

  it("requires the initial update only when asked", () => {
    expect(validateReportForm({ ...valid, markdown: "" }).markdown).toBe("Write the first update");
    expect(validateReportForm({ ...valid, markdown: "", requireUpdate: false }).markdown).toBeUndefined();
  });

  it("checks the maintenance window ordering", () => {
    const maintenance = { ...valid, type: "maintenance" as const, endsAt: "2026-07-18T09:00" };
    expect(validateReportForm(maintenance).endsAt).toBe("End must be after start");
    expect(validateReportForm({ ...maintenance, endsAt: "2026-07-18T11:00" }).endsAt).toBeUndefined();
    expect(validateReportForm({ ...maintenance, endsAt: "" }).endsAt).toBeUndefined();
  });

  it("requires a parseable start", () => {
    expect(validateReportForm({ ...valid, startsAt: "" }).startsAt).toBe("Enter a start time");
  });
});

describe("impactOptions", () => {
  it("scopes impact choices to the report type", () => {
    expect(impactOptions("incident").map((option) => option.value)).toEqual(["none", "degraded", "down"]);
    expect(impactOptions("maintenance").map((option) => option.value)).toEqual(["none", "maintenance", "degraded"]);
  });
});
