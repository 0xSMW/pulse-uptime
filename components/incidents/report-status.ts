...
function toCandidates(updatesNewestFirst: readonly ReportUpdateData[], edit?: UpdateEdit): RankedUpdate[] {
  return updatesNewestFirst.map((update, index) => {
    const edited = edit !== undefined && update.id === edit.id;
    const publishedAt = edited && edit.publishedAt !== undefined ? edit.publishedAt : update.publishedAt;
    return {
      id: update.id,
      status: edited && edit.status !== undefined ? edit.status : update.status,
      publishedAt: new Date(publishedAt).getTime(),
      createdAt: update.createdAt !== undefined ? new Date(update.createdAt).getTime() : null,
      rank: index,
    };
  });
}
...
export function currentStatusAfterEdit(
  ...
  const candidates = toCandidates(updatesNewestFirst, edit).sort(compareNewestFirst);
...
export function stateFlipAfterRemoval(
  ...
  const first = updatesNewestFirst[0];
  const before = first ? isResolvingStatus(first.status) : false;
  const remaining = updatesNewestFirst.filter((update) => update.id !== removeId);
  if (remaining.length === 0) return null;
  const afterStatus = toCandidates(remaining).sort(compareNewestFirst)[0]?.status ?? null;
  const after = afterStatus !== null && isResolvingStatus(afterStatus);
  if (before === after) return null;
  return after ? "to_resolved" : "to_ongoing";
}
...
export function validateReportForm(input: {
  ...
  else if (input.title.trim().length > 160) errors.title = "Use 160 characters or fewer";
  const startsAt = fromDatetimeLocal(input.startsAt);
  if (!startsAt) errors.startsAt = "Enter a start time";
  if (input.type === "maintenance" && input.endsAt.trim()) {
    const endsAt = fromDatetimeLocal(input.endsAt);
    if (!endsAt) errors.endsAt = "Enter a valid end time";
    else if (startsAt && endsAt <= startsAt) errors.endsAt = "End must be after start";
  }
  if (input.requireUpdate) {
    if (!input.markdown.trim()) errors.markdown = "Write the first update";
    else if (input.markdown.length > MAX_REPORT_MARKDOWN) errors.markdown = "Use 10 KB or fewer";
    if (!fromDatetimeLocal(input.publishedAt)) errors.publishedAt = "Enter a valid time";
  }
  return errors;
}
