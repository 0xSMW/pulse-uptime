...
async function fetchLive(url: string): Promise<MonitorLiveData> {
  ...
  if (!response.ok) {
    throw new LiveFetchError(response.status)
  }
...
export function useMonitorLive(
  ...
  useEffect(() => {
    if (
      livePollConfigChanged(refreshedConfigRef.current, acceptedConfigToken)
    ) {
  ...
  useEffect(() => {
    if (liveD30 === undefined || liveD90 === undefined) {
      return
    }
    if (
      livePollUnlockAdvanced(
    ...
    liveD30,
    liveD90,
    server.rangeUnlocked.d30,
    server.rangeUnlocked.d90,
    router,
  ])
