...
export function TimelineBar({
  ...
  const [active, setActive] = React.useState<{ index: number; element: HTMLElement } | null>(null);
      ...
      >
        ...
        {buckets.map((bucket, index) => (
          <span
            ...
            onPointerEnter={(event) => setActive({ index, element: event.currentTarget })}
          />
        ))}
      </div>
      {active && activeBucket ? (
        <TooltipContent anchor={active.element}>
          <span className="block whitespace-nowrap">{bucketRangeText(activeBucket, timeZone)}</span>
