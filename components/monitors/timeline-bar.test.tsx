...
describe("TimelineBar tooltips", () => {
  it("keeps the bar as a single non-focusable image summary", () => {
    render(<TimelineBar buckets={buckets} label="Last 24 hours" timeZone="Asia/Bangkok" />);
    const bar = screen.getByRole("img", { name: "Last 24 hours: 8 checks, 4 failed" });
  ...
  it("falls back to the ISO label range when a bucket carries no structured times", () => {
    ...
    render(<TimelineBar buckets={labelOnly} label="Dependency" timeZone="Asia/Bangkok" />);
