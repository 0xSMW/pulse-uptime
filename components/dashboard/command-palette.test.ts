...
describe("command palette helpers", () => {
  it("builds navigation, monitor, and DOWN-only live incident groups", () => {
    const groups = buildPaletteGroups(monitors, incidents, new Date("2026-07-18T12:00:00.000Z"));

    expect(groups.map((group) => group.label)).toEqual(["Navigation", "Monitors", "Live Incidents"]);
    expect(groups[1]?.items.map((item) => item.hint)).toEqual(["Down", "42 ms"]);
    expect(groups[2]?.items).toEqual([expect.objectContaining({
  ...
  it("omits Live Incidents when no monitor is down", () => {
    const groups = buildPaletteGroups(monitors.map((monitor) => ({ ...monitor, state: "UP" })), []);
    expect(groups.map((group) => group.label)).toEqual(["Navigation", "Monitors"]);
  });

  it("filters case-insensitively and removes empty groups", () => {
    const filtered = filterPaletteGroups(buildPaletteGroups(monitors, incidents), "http 503");
