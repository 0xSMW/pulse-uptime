...
describe("restricted markdown rendering", () => {
  it("renders blank-line separated paragraphs", () => {
    expect(
      renderRestrictedMarkdown("First paragraph.\n\nSecond paragraph.")
  ...
  it("renders http, https, and mailto links with hardened rel/target", () => {
    expect(
      renderRestrictedMarkdown("[Pulse status](https://pulse.example.com)")
    ).toBe(
      '<p><a href="https://pulse.example.com" target="_blank" rel="noopener noreferrer">Pulse status</a></p>'
    )
    expect(
      renderRestrictedMarkdown("[Email us](mailto:ops@example.com)")
    ).toContain('href="mailto:ops@example.com"')
  ...
  it("does not double-decode pre-encoded entities", () => {
    expect(renderRestrictedMarkdown("Fish &amp; chips")).toBe(
      "<p>Fish &amp;amp; chips</p>"
    )
    expect(renderRestrictedMarkdown("&lt;script&gt;")).toBe(
      "<p>&amp;lt;script&amp;gt;</p>"
    )
  })
  ...
  it("rejects schemes smuggled with embedded whitespace or control characters", () => {
    expect(
      renderRestrictedMarkdown("[x](java\nscript:alert(1))")
    ).not.toContain("href")
    expect(
      renderRestrictedMarkdown("[x](java\tscript:alert(1))")
    ).not.toContain("href")
    expect(
      renderRestrictedMarkdown("[x](\tjavascript:alert(1))")
    ).not.toContain("href")
  })

  it("rejects entity-obfuscated schemes because escaping happens first", () => {
    expect(
      renderRestrictedMarkdown("[x](javascript&colon;alert(1))")
    ).not.toContain("href")
  })

  it("rejects relative and protocol-relative URLs", () => {
    expect(renderRestrictedMarkdown("[x](/status)")).not.toContain("<a")
    expect(renderRestrictedMarkdown("[x](//evil.example.com)")).not.toContain(
      "<a"
    )
  })
  ...
  it("renders links inside bold", () => {
    expect(
      renderRestrictedMarkdown("**[Docs](https://docs.example.com)**")
    ).toBe(
  ...
  it("renders emphasis and code spans inside link labels", () => {
    expect(
      renderRestrictedMarkdown("[**Docs**](https://example.com)")
    ).toContain("><strong>Docs</strong></a>")
  ...
  it("treats markdown inside code spans as literal text", () => {
    ...
    expect(html).toBe(
      "<p><code>**not bold** [x](https://example.com)</code></p>"
    )
  ...
  it("does not render headings or lists", () => {
    expect(renderRestrictedMarkdown("# Not a heading")).toBe(
      "<p># Not a heading</p>"
    )
    expect(renderRestrictedMarkdown("- not a list item")).toBe(
      "<p>- not a list item</p>"
    )
  })

  it("truncates input beyond 16 KB before processing", () => {
    expect(renderRestrictedMarkdown("x".repeat(17_000))).toBe(
      `<p>${"x".repeat(16_384)}</p>`
    )
  })
  ...
  it("strips NUL bytes so placeholder tokens cannot be forged", () => {
    ...
    expect(renderRestrictedMarkdown(`\`x\` ${NUL}0${NUL}`)).toBe(
      "<p><code>x</code> 0</p>"
    )
  })
})
