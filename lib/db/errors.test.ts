...
describe("isDatabaseUnavailableError", () => {
  ...
  it.each([
    ...
    expect(isDatabaseUnavailableError(errorWithCode(code, `write ${code} 127.0.0.1:5432`))).toBe(true);
  });

  it("classifies an unrecognized postgres.js CONNECTION_ prefixed code as unavailable", () => {
    expect(isDatabaseUnavailableError(errorWithCode("CONNECTION_SOME_FUTURE_VARIANT"))).toBe(true);
  });
  ...
  it.each([
    ...
    expect(isDatabaseUnavailableError(errorWithCode(code, "postgres error"))).toBe(true);
  });
  ...
  it("rethrows (returns false) for a plain app error", () => {
    expect(isDatabaseUnavailableError(new TypeError("Cannot read properties of undefined"))).toBe(false);
  });

  it("rethrows (returns false) for a Postgres constraint violation", () => {
    expect(isDatabaseUnavailableError(errorWithCode("23505", "duplicate key value violates unique constraint"))).toBe(
      false,
    );
  });

  it("rethrows (returns false) for a Postgres syntax error", () => {
    expect(isDatabaseUnavailableError(errorWithCode("42601", "syntax error at or near"))).toBe(false);
  });
