...
describe("database schema", () => {
  ...
  it("uses timestamptz for every timestamp", () => {
    ...
    expect(timestampColumns.map((column) => column.getSQLType())).toEqual(
      timestampColumns.map(() => "timestamp with time zone")
    )
  })
  ...
  it("keeps the reliability indexes unique or partial where required", () => {
    ...
    expect(indexes.get("device_authorizations_active_user_code")?.unique).toBe(
      true
    )
    expect(
      indexes.get("device_authorizations_active_user_code")?.where
    ).toBeDefined()
    ...
    expect(
      indexes.get("monitoring_config_snapshots_accepted_order")?.where
    ).toBeDefined()
    expect(indexes.get("api_tokens_active_creator")?.where).toBeDefined()
    expect(indexes.get("config_operations_principal_idempotency")?.unique).toBe(
      false
    )
  })
