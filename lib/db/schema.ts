...
export const monitorRegistry = pgTable(
  "monitor_registry",
  {
...
export const monitoringConfigSnapshots = pgTable(
  ...
  (table) => [
    ...
    check(
      "monitoring_config_snapshots_version_nonnegative",
...
export const monitorState = pgTable(
  ...
  (table) => [
    ...
    check(
      "monitor_state_failures_nonnegative",
      sql`${table.consecutiveFailures} >= 0`
    ),
    check(
      "monitor_state_successes_nonnegative",
      sql`${table.consecutiveSuccesses} >= 0`
    ),
    check(
      "monitor_state_latency_nonnegative",
      sql`${table.lastLatencyMs} is null or ${table.lastLatencyMs} >= 0`
    ),
...
export const checkResults = pgTable(
  ...
  (table) => [
    uniqueIndex("check_results_monitor_schedule").on(
      table.monitorId,
      table.scheduledAt
    ),
    index("check_results_monitor_time").on(
      table.monitorId,
    ...
    check(
      "check_results_redirect_count",
      sql`${table.redirectCount} between 0 and 5`
    ),
  ]
)
...
export const incidents = pgTable(
  ...
  (table) => [
    index("incidents_monitor_opened").on(
      table.monitorId,
      table.openedAt.desc()
    ),
    ...
    check(
      "incidents_opened_after_failure",
      sql`${table.openedAt} >= ${table.firstFailureAt}`
    ),
    check(
      "incidents_resolution_order",
...
export const cronRuns = pgTable(
  ...
  (table) => [
    uniqueIndex("cron_runs_job_schedule").on(
      table.jobName,
      table.scheduledMinute
    ),
    ...
    index("cron_runs_job_release_completed")
      ...
      .where(
        sql`${table.status} = 'completed' and ${table.releaseId} is not null`
      ),
    check(
      "cron_runs_status",
...
export const checkBatches = pgTable(
  "check_batches",
  {
...
export const atomicMinuteCommits = pgTable(
  "atomic_minute_commits",
  {
    scheduledMinute: timestamptz("scheduled_minute")
      .primaryKey()
  ...
  (table) => [
    check(
      "atomic_minute_commits_state_count",
      sql`${table.stateMutationCount} >= 0`
    ),
  ]
)

export const exceptionPayloads = pgTable(
  "exception_payloads",
  {
    id: uuid("id").primaryKey(),
    payload: jsonb("payload").notNull(),
  ...
  (table) => [
    ...
    check(
      "exception_payloads_expiry_order",
      sql`${table.expiresAt} > ${table.createdAt}`
    ),
  ]
)

export const monitorExceptions = pgTable(
  "monitor_exceptions",
  {
    id: uuid("id").primaryKey(),
    monitorId: text("monitor_id").references(() => monitorRegistry.id),
    ...
    incidentId: uuid("incident_id").references(() => incidents.id),
    payloadId: uuid("payload_id").references(() => exceptionPayloads.id, {
      onDelete: "set null",
    }),
  },
  (table) => [
    uniqueIndex("monitor_exceptions_identity").on(
      table.monitorId,
      table.eventType,
      table.identityHash,
      sql`coalesce(${table.incidentId}, '00000000-0000-0000-0000-000000000000'::uuid)`
    ),
    index("monitor_exceptions_retention").on(table.lastSeenAt, table.id),
    index("monitor_exceptions_incident").on(table.incidentId),
    check(
      "monitor_exceptions_event_type",
      sql`${table.eventType} in ('failure', 'recovery', 'pause', 'resume', 'scheduler_gap', 'configuration')`
    ),
    check("monitor_exceptions_occurrences", sql`${table.occurrenceCount} > 0`),
    check(
      "monitor_exceptions_seen_order",
      sql`${table.lastSeenAt} >= ${table.firstSeenAt}`
    ),
    check(
      "monitor_exceptions_latency",
      sql`${table.worstLatencyMs} is null or ${table.worstLatencyMs} >= 0`
    ),
  ]
)
...
export const metricRollups = pgTable(
  ...
  (table) => [
    primaryKey({
      columns: [table.monitorId, table.resolution, table.bucketStart],
    }),
    index("metric_rollups_retention").on(
      table.resolution,
      table.bucketStart,
      table.monitorId
    ),
    check(
      "metric_rollups_resolution",
      sql`${table.resolution} in ('15m', 'hour', 'day')`
    ),
    check(
      "metric_rollups_counts",
      sql`${table.expectedChecks} >= 0 and ${table.completedChecks} >= 0 and ${table.successfulChecks} >= 0 and ${table.failedChecks} >= 0 and ${table.unknownChecks} >= 0 and ${table.completedChecks} <= ${table.expectedChecks} and ${table.successfulChecks} + ${table.failedChecks} = ${table.completedChecks} and ${table.unknownChecks} = ${table.expectedChecks} - ${table.completedChecks}`
    ),
    check(
      "metric_rollups_seconds",
      sql`${table.downtimeSeconds} >= 0 and ${table.unknownSeconds} >= 0`
    ),
    check(
      "metric_rollups_latency",
      sql`${table.latencyCount} >= 0 and ${table.latencySumMs} >= 0 and (${table.latencyMinMs} is null or ${table.latencyMinMs} >= 0) and (${table.latencyMaxMs} is null or ${table.latencyMaxMs} >= 0) and (${table.latencyMinMs} is null or ${table.latencyMaxMs} is null or ${table.latencyMinMs} <= ${table.latencyMaxMs})`
    ),
    check(
      "metric_rollups_histogram",
      sql`${table.histogramVersion} > 0 and cardinality(${table.latencyHistogram}) = 8`
    ),
  ]
)

export const databaseUsageSnapshots = pgTable(
  "database_usage_snapshots",
  {
    ...
    projected30DayBytes: bigint("projected_30_day_bytes", {
      mode: "bigint",
    }).notNull(),
    governorMode: text("governor_mode", {
      enum: [
        "full",
        "compact_early",
        "shortened",
    ...
    schedulerCoverage: numeric("scheduler_coverage", {
      precision: 7,
      scale: 4,
    }),
  ...
  (table) => [
    check(
      "database_usage_snapshots_bytes",
      sql`${table.storageBytes} >= 0 and ${table.indexBytes} >= 0 and ${table.projected30DayBytes} >= 0 and (${table.historyBytes} is null or ${table.historyBytes} >= 0) and (${table.monthlyTransferBytes} is null or ${table.monthlyTransferBytes} >= 0)`
    ),
    check(
      "database_usage_snapshots_governor",
      sql`${table.governorMode} in ('full', 'compact_early', 'shortened', 'incident_only', 'essential')`
    ),
    check(
      "database_usage_snapshots_coverage",
      sql`${table.schedulerCoverage} is null or ${table.schedulerCoverage} between 0 and 1`
    ),
  ]
)
 ...
 */
export const statusPageConfig = pgTable(
  "status_page_config",
  {
    id: smallint("id").primaryKey().default(1),
    name: text("name"),
    layout: text("layout", { enum: ["vertical", "horizontal"] })
      .notNull()
      .default("vertical"),
    ...
    unknownAsOperational: boolean("unknown_as_operational")
      .notNull()
      .default(false),
  ...
  (table) => [
    ...
    check(
      "status_page_config_layout",
      sql`${table.layout} in ('vertical', 'horizontal')`
    ),
    check(
      "status_page_config_theme",
      sql`${table.theme} in ('system', 'light', 'dark')`
    ),
    check(
      "status_page_config_history_days",
      sql`${table.historyDays} in (30, 60, 90)`
    ),
    check(
      "status_page_config_uptime_decimals",
      sql`${table.uptimeDecimals} between 0 and 3`
    ),
    check(
...
export const onboardingProgress = pgTable("onboarding_progress", {
  ...
  emailWarningAcknowledged: boolean("email_warning_acknowledged")
    .notNull()
    .default(false),
  updatedAt: timestamptz("updated_at").notNull(),
  completedAt: timestamptz("completed_at"),
...
export const apiTokens = pgTable(
  ...
  (table) => [
    ...
    check(
      "api_tokens_expiry_order",
      sql`${table.expiresAt} > ${table.createdAt}`
    ),
    check(
      "api_tokens_expiry_limit",
      sql`${table.expiresAt} <= ${table.createdAt} + interval '365 days'`
    ),
  ]
)
...
export const cliInstallations = pgTable(
  ...
  (table) => [
    check(
      "cli_installations_link_order",
      sql`${table.linkedAt} >= ${table.createdAt}`
    ),
  ]
)
...
export const cliSessions = pgTable(
  "cli_sessions",
  {
    ...
    installationId: uuid("installation_id")
      .notNull()
      .references(() => cliInstallations.id),
    tokenPrefix: text("token_prefix").notNull(),
    tokenDigest: bytea("token_digest").notNull().unique(),
    userEmail: text("user_email").notNull(),
    scopes: text("scopes").array().notNull(),
...
export const deviceAuthorizations = pgTable(
  ...
  (table) => [
    ...
    check(
      ...
      sql`${table.state} in ('pending', 'approved', 'denied', 'consumed', 'expired')`
    ),
    check(
      "device_authorizations_expiry_order",
      sql`${table.expiresAt} > ${table.createdAt}`
    ),
    check(
      "device_authorizations_poll_interval",
      sql`${table.pollingIntervalSeconds} > 0`
    ),
...
export const apiIdempotency = pgTable(
  ...
  (table) => [
    uniqueIndex("api_idempotency_principal_key").on(
      table.principalKey,
...
export const apiRateLimitBuckets = pgTable(
  ...
  (table) => [
    ...
    check(
      "api_rate_limit_buckets_expiry_order",
...
export const statusReports = pgTable(
  ...
  (table) => [
    ...
    check(
      "status_reports_type",
      sql`${table.type} in ('incident', 'maintenance')`
    ),
    check(
      "status_reports_title_length",
      sql`char_length(${table.title}) between 1 and 160`
    ),
  ]
)

export const statusReportUpdates = pgTable(
  "status_report_updates",
  {
    id: uuid("id").primaryKey(),
    reportId: uuid("report_id")
      .notNull()
      .references(() => statusReports.id, { onDelete: "cascade" }),
...
export const statusReportAffected = pgTable(
  "status_report_affected",
  {
    reportId: uuid("report_id")
      .notNull()
      .references(() => statusReports.id, { onDelete: "cascade" }),
...
export const configOperations = pgTable(
  "config_operations",
  {
    id: uuid("id").primaryKey(),
    principalKey: text("principal_key").notNull(),
  ...
  (table) => [
    index("config_operations_target_hash").on(
      table.targetConfigHash,
      table.state
    ),
    index("config_operations_principal_idempotency").on(
      table.principalKey,
      table.idempotencyKey
    ),
    check(
      "config_operations_state",
      sql`${table.state} in ('written', 'accepted', 'rejected', 'failed')`
    ),
    check(
      "config_operations_edge_version",
      sql`${table.edgeConfigVersion} is null or ${table.edgeConfigVersion} >= 0`
    ),
    check(
...
export const dependencyDiscoveredScopeKinds = [
  "discovered_child",
  "discovered_location",
] as const
...
export const incidentComponentAssociationKinds = [
  "explicit",
  "inferred",
] as const

export const dependencyIncidentMatchKinds = [
  "component_match",
  "inferred",
] as const

export const dependencySources = pgTable(
  "dependency_sources",
  {
    id: text("id").primaryKey(),
  ...
  (table) => [
    ...
    check(
      "dependency_sources_failures_nonnegative",
      sql`${table.consecutiveFailures} >= 0`
    ),
  ]
)

export const dependencyCatalog = pgTable(
  "dependency_catalog",
  {
    id: text("id").primaryKey(),
    sourceId: text("source_id")
      .notNull()
      .references(() => dependencySources.id),
    displayName: text("display_name").notNull(),
    ...
    fidelity: text("fidelity", { enum: dependencyFidelities })
      .notNull()
      .default("component"),
    catalogVersion: text("catalog_version").notNull(),
    enabled: boolean("enabled").notNull(),
 ...
 */
export const dependencyDiscoveredScopeOptions = pgTable(
  "dependency_discovered_scope_options",
  {
    catalogId: text("catalog_id")
      .notNull()
      .references(() => dependencyCatalog.id),
    scopeId: text("scope_id").notNull(),
    label: text("label").notNull(),
    scopeKind: text("scope_kind", {
      enum: dependencyDiscoveredScopeKinds,
    }).notNull(),
  ...
  (table) => [
    primaryKey({
      columns: [table.catalogId, table.scopeId],
      name: "dependency_discovered_scope_options_catalog_id_scope_id_pk",
    }),
    index("dependency_discovered_scope_options_catalog_available_label").on(
      table.catalogId,
      table.available,
      table.label
    ),
...
export const dependencyState = pgTable(
  "dependency_state",
  {
...
export const dependencyStateIntervals = pgTable(
  "dependency_state_intervals",
  {
    id: text("id").primaryKey(),
    dependencyId: text("dependency_id")
      .notNull()
  ...
  (table) => [
    ...
    index("dependency_state_intervals_dependency_time").on(
      table.dependencyId,
    ...
    check(
      "dependency_state_intervals_order",
...
export const providerIncidents = pgTable(
  ...
  (table) => [
    uniqueIndex("provider_incidents_source_external").on(
      table.sourceId,
      table.externalId
    ),
    index("provider_incidents_source_started").on(
      table.sourceId,
      table.startedAt.desc()
    ),
    ...
    check(
      "provider_incidents_resolution_order",
      sql`${table.resolvedAt} is null or ${table.resolvedAt} >= ${table.startedAt}`
    ),
  ]
)

export const providerIncidentComponents = pgTable(
  "provider_incident_components",
  {
    incidentId: text("incident_id")
      .notNull()
      .references(() => providerIncidents.id),
    externalComponentId: text("external_component_id").notNull(),
    associationKind: text("association_kind", {
      enum: incidentComponentAssociationKinds,
    }).notNull(),
  },
  (table) => [
    primaryKey({ columns: [table.incidentId, table.externalComponentId] }),
    check(
      "provider_incident_components_association_kind",
      sql`${table.associationKind} in ('explicit', 'inferred')`
    ),
  ]
)

export const providerIncidentUpdates = pgTable(
...
export const dependencyIncidentMatches = pgTable(
  "dependency_incident_matches",
  {
    dependencyId: text("dependency_id")
      .notNull()
      .references(() => dependencies.id),
    ...
    matchKind: text("match_kind", {
      enum: dependencyIncidentMatchKinds,
    }).notNull(),
  ...
  (table) => [
    ...
    check(
      "dependency_incident_matches_kind",
      sql`${table.matchKind} in ('component_match', 'inferred')`
    ),
  ]
)
