...
import { clientIpFromHeaders, firstForwardedIp, validClientIpFromHeaders } from "./client-ip";
...
describe("clientIpFromHeaders", () => {
  it("prefers the platform-set x-real-ip over the spoofable forwarded chain", () => {
    const headers = new Headers({ "x-real-ip": "198.51.100.9", "x-forwarded-for": "203.0.113.7, 10.0.0.1" });
  ...
  it("falls back to the first forwarded hop when x-real-ip is absent or blank", () => {
    ...
    expect(clientIpFromHeaders(new Headers({ "x-real-ip": "  ", "x-forwarded-for": "203.0.113.7" }))).toBe("203.0.113.7");
...
describe("validClientIpFromHeaders", () => {
  it("returns the address only when it is a valid IPv4 or IPv6 literal", () => {
    expect(validClientIpFromHeaders(new Headers({ "x-real-ip": "198.51.100.9" }))).toBe("198.51.100.9");
    expect(validClientIpFromHeaders(new Headers({ "x-forwarded-for": "2001:db8::1, 10.0.0.1" }))).toBe("2001:db8::1");
  });

  it("returns null when the extracted value is not a valid IP", () => {
    expect(validClientIpFromHeaders(new Headers({ "x-real-ip": "not-an-ip" }))).toBeNull();
    expect(validClientIpFromHeaders(new Headers({ "x-forwarded-for": "example.com" }))).toBeNull();
    expect(validClientIpFromHeaders(new Headers())).toBeNull();
  });

  it("validates the x-real-ip winner rather than the forwarded fallback", () => {
    const headers = new Headers({ "x-real-ip": "bogus", "x-forwarded-for": "203.0.113.7" });
