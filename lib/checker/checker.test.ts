...
function peerAwareDispatcher(
  ...
  const close = vi.fn(async () => undefined);
  const createDispatcher: DispatcherFactory = ({ origin, onConnectedAddress }) => {
    const address = typeof peer === "function" ? peer(origin) : peer;
    return {
      close,
      __reportPeer() {
        if (address) onConnectedAddress(address);
      },
...
const noopLookup = ((_host, _opts, cb) => cb(null, "8.8.8.8", 4)) as SecureLookup;
...
describe("HTTP checker", () => {
  ...
  it("records the second DNS candidate when that is the connected peer", async () => {
    ...
    const request: RequestExecutor = async (_url, options) => {
      (options.dispatcher as unknown as { __reportPeer(): void }).__reportPeer();
      return response(200);
    };
    ...
    await expect(checker(target)).resolves.toMatchObject({ resolvedAddress: "8.8.4.4" });
  });
  ...
  it("records IPv4 when IPv6 fails and IPv4 succeeds", async () => {
    ...
    await expect(checker(target)).resolves.toMatchObject({ resolvedAddress: "199.232.165.91" });
  });
  ...
  it("never reports a connected peer when DNS validation fails", async () => {
    ...
    const request: RequestExecutor = async (_url, options) => {
      const connect = (options.dispatcher as unknown as { __connect: BaseConnector }).__connect;
...
describe("createSecureConnect", () => {
  const connectOpts = { hostname: "example.com", protocol: "https:", port: "443" };
  ...
  it("propagates base connector failures without a peer", async () => {
    const onConnectedAddress = vi.fn();
    const baseError = Object.assign(new Error("refused"), { code: "ECONNREFUSED" });
    const baseConnect: BaseConnector = (_opts, callback) => {
