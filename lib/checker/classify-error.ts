...
export function classifyCheckError(error: unknown): CheckErrorCode {
  for (const item of errorChain(error)) {
    if (item instanceof BlockedTargetError) return "BLOCKED_TARGET";
    if (item instanceof MonitorValidationError) return "INVALID_URL";
    if (item instanceof DOMException && item.name === "AbortError") return "TIMEOUT";
    const code = typeof item === "object" && item !== null && "code" in item
    ...
    if (code.startsWith("UND_ERR_")) return "RESPONSE_ERROR";
  }
  return "UNKNOWN";
}
