    ...
    void resolveAll(hostname).then(
      (addresses) => {
        if (addresses.length === 0) {
          const error = Object.assign(new Error("Hostname did not resolve"), {
            code: "ENOTFOUND",
          })
          callback(error, "")
          return
        ...
        try {
          for (const { address, family } of addresses) {
            ...
            const parsedFamily =
              parsed?.kind() === "ipv4" ? 4 : parsed?.kind() === "ipv6" ? 6 : 0
            if ((family !== 4 && family !== 6) || family !== parsedFamily) {
              throw new BlockedTargetError(
                "Resolver returned an invalid address family"
              )
            }
          }
        } catch (error) {
        ...
        const ordered = orderByFamilyPreference(
          addresses,
        ...
        if (wantsAll(lookupOptions)) {
          ...
          callback(
            null,
            ordered.map(({ address, family }) => ({ address, family }))
          )
        } else {
