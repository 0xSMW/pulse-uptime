 ...
 */
export function filterShortResolvedIncidents<
  T extends { durationSeconds: number },
>(incidents: readonly T[], minIncidentSeconds: number): T[] {
  if (minIncidentSeconds <= 0) {
    return [...incidents]
  }
  return incidents.filter(
    (incident) => incident.durationSeconds >= minIncidentSeconds
  )
}
