...
describe("status page config document schema", () => {
  it("accepts a fully populated document", () => {
    const result = statusPageConfigDocumentSchema.safeParse(
      baseDocument({
        name: "Acme Status",
        layout: "horizontal",
        theme: "dark",
  ...
  it("requires image references to be UUIDs or null", () => {
    ...
    expect(
      statusPageConfigDocumentSchema.safeParse(
        baseDocument({ faviconImageId: IMAGE_ID })
      ).success
    ).toBe(true)
  })
  ...
  it("limits navLinks to 8 entries with bounded labels and http(s)/mailto URLs", () => {
    ...
    expect(
      statusPageConfigDocumentSchema.safeParse(
        baseDocument({
          navLinks: [{ label: "Mail", url: "mailto:ops@example.com" }],
        })
      ).success
    ).toBe(true)
  })
  ...
  it("restricts homepage to http(s) and contact to http(s)/mailto", () => {
    ...
    expect(
      statusPageConfigDocumentSchema.safeParse(
        baseDocument({ homepageUrl: "http://example.com" })
      ).success
    ).toBe(true)
    expectRejected({ contactUrl: "ftp://example.com" })
    expect(
      statusPageConfigDocumentSchema.safeParse(
        baseDocument({ contactUrl: "mailto:ops@example.com" })
      ).success
    ).toBe(true)
  })
  ...
  it("validates the Google tag id format", () => {
    expect(
      statusPageConfigDocumentSchema.safeParse(
        baseDocument({ googleTagId: "G-XYZ789" })
      ).success
    ).toBe(true)
    expect(
      statusPageConfigDocumentSchema.safeParse(
        baseDocument({ googleTagId: "GT-XYZ789" })
      ).success
    ).toBe(true)
  ...
  it("caps custom css/head at 10 KB and the announcement at 2 KB of UTF-8", () => {
    expect(
      statusPageConfigDocumentSchema.safeParse(
        baseDocument({ customCss: "x".repeat(10 * 1024) })
      ).success
    ).toBe(true)
    ...
    expect(
      statusPageConfigDocumentSchema.safeParse(
        baseDocument({ announcementMarkdown: "x".repeat(2048) })
      ).success
    ).toBe(true)
  ...
  it("bounds minIncidentSeconds to zero through seven days", () => {
    expect(
      statusPageConfigDocumentSchema.safeParse(
        baseDocument({ minIncidentSeconds: 604_800 })
      ).success
    ).toBe(true)
    expectRejected({ minIncidentSeconds: 604_801 })
  ...
  it("requires timezone to be a real IANA zone or null", () => {
    expect(
      statusPageConfigDocumentSchema.safeParse(
        baseDocument({ timezone: "Asia/Bangkok" })
      ).success
    ).toBe(true)
  ...
  it("rejects unknown fields and missing fields", () => {
    ...
    void _name
    expect(statusPageConfigDocumentSchema.safeParse(missing).success).toBe(
      false
    )
  })
})
