...
describe("reportBannerTier", () => {
  it("floors incident reports at degraded, even with no affected services", () => {
    ...
    expect(
      reportBannerTier(
        report({ affected: [{ monitorId: "a", impact: "degraded" }] })
      )
    ).toBe("degraded")
  })
  ...
  it("keeps maintenance reports at the maintenance tint, even with down impact", () => {
    ...
    expect(
      reportBannerTier(
        report({
          type: "maintenance",
          affected: [{ monitorId: "a", impact: "down" }],
        })
      )
    ).toBe("maintenance")
  })
...
describe("deriveOverallState", () => {
  ...
  it("keeps the machine-only tiers intact without reports", () => {
    ...
    expect(deriveOverallState(["UP", "VERIFYING_DOWN"], [])).toBe(
      "investigating"
    )
    ...
    expect(
      deriveOverallState(
        ...
        [
          report({
            type: "maintenance",
            affected: [{ monitorId: "a", impact: "maintenance" }],
          }),
        ]
      )
    ).toBe("maintenance")
  ...
  it("tints maintenance only when nothing redder is happening", () => {
    ...
    expect(deriveOverallState(["UP", "VERIFYING_DOWN"], [maintenance])).toBe(
      "investigating"
    )
...
describe("promotion dedupe", () => {
  ...
  it("suppresses incidents represented by a report and keeps the rest", () => {
    ...
    expect(
      excludePromotedIncidents(incidents, promoted).map(
...
describe("monitorReportAnnotations", () => {
  it("annotates each affected monitor with its impact label and report id", () => {
    const annotations = monitorReportAnnotations([
      ...
      report({
        id: "rep-b",
        type: "maintenance",
        affected: [{ monitorId: "batch", impact: "maintenance" }],
      }),
    ])
    expect(annotations.get("api")).toEqual({
      reportId: "rep-a",
      impact: "degraded",
      label: "Degraded — see report",
    })
    expect(annotations.get("web")).toEqual({
      reportId: "rep-a",
      impact: "down",
      label: "Down — see report",
    })
    expect(annotations.get("batch")).toEqual({
      reportId: "rep-b",
      impact: "maintenance",
      label: "Maintenance — see report",
    })
  ...
  it("keeps the worst impact when several ongoing reports affect one monitor", () => {
    const annotations = monitorReportAnnotations([
      ...
      report({
        id: "rep-b",
        affected: [{ monitorId: "api", impact: "degraded" }],
      }),
    ])
    expect(annotations.get("api")).toMatchObject({
      reportId: "rep-a",
      impact: "down",
    })
  })
...
describe("publicReportPhase", () => {
  ...
  it("resolved wins over everything", () => {
    expect(
      publicReportPhase(
        { ...base, resolvedAt: "2026-07-18T11:00:00.000Z" },
        now
      )
    ).toBe("resolved")
  })
  ...
  it("demotes a window past endsAt with no completing update", () => {
    expect(
      publicReportPhase({ ...base, endsAt: "2026-07-18T11:30:00.000Z" }, now)
    ).toBe("window_ended")
  })
  ...
  it("keeps an open started maintenance window ongoing", () => {
    expect(publicReportPhase(base, now)).toBe("ongoing")
    expect(
      publicReportPhase({ ...base, endsAt: "2026-07-18T13:00:00.000Z" }, now)
    ).toBe("ongoing")
  })
