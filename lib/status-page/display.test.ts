...
describe("defaultStatusPageDocument", () => {
  ...
  it("prefers the trimmed env page name", () => {
    expect(defaultStatusPageDocument({ NEXT_PUBLIC_STATUS_PAGE_NAME: "  Acme Status " }).name).toBe("Acme Status");
  });
...
describe("timezoneDisplay", () => {
  ...
  it("defaults to UTC labeled exactly as before", () => {
    expect(timezoneDisplay(null, at)).toEqual({ timeZone: "UTC", short: "UTC", full: "UTC" });
    expect(timezoneDisplay("UTC", at)).toEqual({ timeZone: "UTC", short: "UTC", full: "UTC" });
  });
...
describe("timestamp formatting", () => {
  it("formats in the requested zone", () => {
    expect(formatStatusTimestamp("2026-07-18T12:00:00.000Z", "UTC")).toBe("Jul 18, 12:00");
    expect(formatStatusTimestamp("2026-07-18T18:30:00.000Z", "Asia/Bangkok")).toBe("Jul 19, 01:30");
    expect(formatStatusClock("2026-07-18T12:00:05.000Z", "Asia/Bangkok")).toBe("19:00:05");
  });
...
describe("filterShortResolvedIncidents", () => {
  ...
  it("hides resolved blips shorter than the floor", () => {
    expect(filterShortResolvedIncidents(incidents, 300).map((incident) => incident.id)).toEqual(["b", "c"]);
  });
});
...
describe("historyWindowStart", () => {
  ...
  it("derives the fetch window from the configured history days", () => {
    expect(historyWindowStart(90, completedDay).toISOString()).toBe("2026-04-19T00:00:00.000Z");
    expect(historyWindowStart(30, completedDay).toISOString()).toBe("2026-06-18T00:00:00.000Z");
  });
});

describe("asset helpers", () => {
  it("builds a base64 data URI from image bytes", () => {
    expect(imageDataUri("image/png", new Uint8Array([137, 80, 78, 71]))).toBe("data:image/png;base64,iVBORw==");
  });
