...
function baseStore(overrides: Partial<MaintenanceStore> = {}): MaintenanceStore {
  ...
  return {
    ...
    reconcileDependencyCatalog: async () => ({ checkedSources: 0, disabledPresets: 0 }),
...
describe("performMaintenance", () => {
  it("reconciles, compacts mergeable rollups, and uses bounded retention batches", async () => {
    ...
    const summary = await performMaintenance(baseStore({
    ...
    expect(calls.find(([name]) => name === "snapshots")?.slice(2)).toEqual([50, 10_000, expect.any(Number)]);
    expect(calls.find(([name]) => name === "orphan-images")?.slice(1, 4))
    ...
    if (gapsIdx >= 0) expect(measureIdx).toBeLessThan(gapsIdx);
  });
  ...
  it("skips dependency catalog validation when the hard deadline is already spent", async () => {
    ...
    expect(summary.skippedTasks.some((skip) => skip.task === "catalog_reconciliation")).toBe(true);
  });
  ...
  it("runs catalog validation under heavy retention because its slice is reserved", async () => {
    ...
    const reconcileDependencyCatalog = vi.fn().mockResolvedValue({ checkedSources: 2, disabledPresets: 0 });
  ...
  it("bounds catalog validation to its slice so it cannot consume the whole window", async () => {
    const nowMs = () => 1_000;
    const reconcileDependencyCatalog = vi.fn().mockResolvedValue({ checkedSources: 3, disabledPresets: 1 });
    // A window far wider than the slice: validation must still get only its slice.
    const deadlineAtMs = 1_000_000;
    await performMaintenance(baseStore({
      reconcileDependencyCatalog,
    }), new Date(), { nowMs, deadlineAtMs });
    expect(reconcileDependencyCatalog).toHaveBeenCalledTimes(1);
  ...
  it("when gap recovery consumes the pre-catalog window, compaction and promotion do not start", async () => {
    ...
    const summary = await performMaintenance(baseStore({
      ...
      schedulerCoverageStart: async () => new Date(now.getTime() - 24 * 3_600_000),
      compact15Minute: compact,
      promoteRollups: promote,
      measureAndSnapshotUsage: measure,
      reconcileDependencyCatalog: async () => ({ checkedSources: 1, disabledPresets: 0 }),
    }), now, { nowMs: () => clock, deadlineAtMs: 20_000 });
    ...
    expect(summary.dependencyCatalog).toEqual({ checkedSources: 1, disabledPresets: 0 });
    expect(summary.skippedTasks.some((skip) => skip.reason === "pre_catalog_budget")).toBe(true);
  });
  ...
  it("reads the latest stored governor mode when measurement cannot run", async () => {
    ...
    const summary = await performMaintenance(baseStore({
      ...
      reconcileDependencyCatalog: async () => ({ checkedSources: 0, disabledPresets: 0 }),
    }), new Date(), { nowMs: () => 0, deadlineAtMs: CATALOG_VALIDATION_BUDGET_MS });
  ...
  it("runs recent compaction and each rollup promotion as separate guarded operations", async () => {
    ...
    await performMaintenance(baseStore({
      schedulerCoverageStart: async () => now,
      compact15Minute: async () => { order.push("compact"); return 1; },
  ...
  it("stops after the first failed task", async () => {
    const later = vi.fn();
    await expect(performMaintenance(baseStore({
  ...
  it("soft-skips pre-catalog statement timeouts so catalog still runs", async () => {
    const catalog = vi.fn(async () => ({ checkedSources: 2, disabledPresets: 0 }));
    ...
    }), new Date(), { nowMs: () => 0, deadlineAtMs: CATALOG_VALIDATION_BUDGET_MS + 5_000 });
    expect(catalog).toHaveBeenCalled();
    expect(summary.dependencyCatalog).toEqual({ checkedSources: 2, disabledPresets: 0 });
    expect(summary.skippedTasks.some((entry) => entry.task === "delete_raw_checks")).toBe(true);
  });

  it("classifies postgres statement timeout as a budget error", () => {
    expect(isStatementBudgetError(Object.assign(new Error("statement timeout"), { code: "57014" }))).toBe(true);
    expect(isStatementBudgetError(new Error("database unavailable"))).toBe(false);
  });
  ...
  it("stops full batches at the injected pre-catalog deadline", async () => {
    ...
    await performMaintenance(baseStore({
      deleteRawChecks: raw,
    }), new Date(), { nowMs: () => clock, deadlineAtMs: CATALOG_VALIDATION_BUDGET_MS + 600 });
