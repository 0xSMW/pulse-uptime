 ...
 */
export function bootstrapTokenConfigured(env: NodeJS.ProcessEnv = process.env): boolean {
  return typeof env.PULSE_BOOTSTRAP_TOKEN === "string" && env.PULSE_BOOTSTRAP_TOKEN.length >= 16;
}
...
export function bootstrapTokenFrom(request: Request, body: unknown): string | undefined {
