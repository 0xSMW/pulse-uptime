...
export async function checkOnboardingReadiness() {
  ...
  return runReadinessChecks(Object.fromEntries(
...
async function probeDatabase() {
  ...
  try {
    await db.transaction(async (tx) => {
      await tx.execute(drizzleSql`create temporary table pulse_readiness_probe (id integer) on commit drop`);
      await tx.execute(drizzleSql`insert into pulse_readiness_probe (id) values (1)`);
      throw rollback;
    });
  } catch (error) {
...
function timeoutResult(system: "vercel" | "database" | "edge" | "email"): ReadinessResult {
  return system === "email"
    ? { system, state: "warning", code: "EMAIL_TIMEOUT", remediation: "Verify your Resend sender" }
    : { system, state: "blocked", code: `${system.toUpperCase()}_TIMEOUT`, remediation: remediation[system] };
}
