...
export type DestructiveChangeReason =
  ...
  | { type: "removed-monitor-percentage"; removedCount: number; previousActiveCount: number; percentage: number; threshold: 20 }
...
export function evaluateDestructiveChange(
  ...
  const candidateActiveIds = new Set(candidateActive.map((monitor) => monitor.id));
  const removed = previousActive.filter((monitor) => !candidateActiveIds.has(monitor.id));
  const removedCount = removed.length;
  const percentage = previousActive.length === 0 ? 0 : (removedCount / previousActive.length) * 100;
  const reasons: DestructiveChangeReason[] = [];

  if (previousActive.length > 0 && candidateActive.length === 0) {
    reasons.push({ type: "all-active-monitors-removed", previousActiveCount: previousActive.length });
  }
  if (removedCount > 5) reasons.push({ type: "removed-monitor-count", removedCount, threshold: 5 });
  if (percentage > 20) {
    reasons.push({
      type: "removed-monitor-percentage", removedCount, previousActiveCount: previousActive.length,
  ...
  return {
    ...
    removedActiveMonitorIds: removed.map((monitor) => monitor.id).sort(compareLexically),
    reasons,
  };
}
...
export function isValidDestructiveApproval(
  ...
  if (!approval || approval.action !== "bulk_archive" || approval.targetConfigHash !== targetConfigHash || approval.consumedAt !== null) {
    return false;
  }
  const expiresAt = approval.expiresAt instanceof Date ? approval.expiresAt : new Date(approval.expiresAt);
