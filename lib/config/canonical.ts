...
export function hashMonitoringConfig(
  config: MonitoringConfig | LegacyMonitoringConfig
): string {
...
export function normalizeDeclarativeConfig(
...
export function normalizeMonitoringConfig(
  ...
  const document = normalizeDeclarativeConfig({
    version: 2,
    settings: config.settings,
    groups: config.groups,
    monitors: config.monitors,
...
function normalizeLegacyDocument(config: {
  version: 1
  settings: MonitoringConfig["settings"]
  monitors: LegacyMonitoringConfig["monitors"]
}) {
  return {
    version: 1 as const,
    settings: {
...
export function assertConfigSize(value: unknown): void {
  ...
  if (bytes > MAX_CONFIG_BYTES) {
    throw new ConfigSizeError(bytes, MAX_CONFIG_BYTES)
  }
}
...
export class ConfigSizeError extends Error {
  ...
  constructor(
    ...
    super(
      `Serialized configuration is ${actualBytes} bytes; maximum is ${maximumBytes}`
    )
