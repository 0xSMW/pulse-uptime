import { createConfigurationPlan, type ConfigurationPlan, type PlanOptions } from "./plan";
...
export class ConfigApplyError extends Error {
  constructor(readonly code: ConfigApplyErrorCode, message: string) {
    super(message);
 ...
 */
export function validateApplyPreconditions(input: ApplyPreconditionInput): ConfigurationPlan {
  const { request } = input;
  if (parseIfMatch(input.ifMatch) !== request.baseConfigHash) {
    throw new ConfigApplyError("PRECONDITION_MISMATCH", "If-Match must equal body baseConfigHash");
  }
  if (request.baseConfigHash !== input.currentConfigHash) {
    throw new ConfigApplyError("CONFIG_VERSION_CONFLICT", "The monitor configuration changed after it was loaded");
  }

  const authoritative = createConfigurationPlan(input.currentConfig, request.targetConfig, {
  ...
  if (request.targetConfigHash !== authoritative.targetConfigHash) {
    throw new ConfigApplyError("TARGET_CONFIG_HASH_MISMATCH", "Target configuration hash does not match the supplied configuration");
  }
