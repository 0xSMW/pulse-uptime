...
describe("findAcceptedSnapshot", () => {
  ...
  it("returns the config, hash, and acceptedAt for a valid, hash-matching row", async () => {
    ...
    const { executor } = fakeExecutor([
      { configJson: CONFIG, configHash: HASH, acceptedAt },
    ])
    await expect(findAcceptedSnapshot(executor)).resolves.toEqual({
      config: CONFIG,
      hash: HASH,
      acceptedAt,
    })
  ...
  it("orders by acceptedAt then seenAt so ties resolve deterministically", async () => {
    const { executor, captured } = fakeExecutor([
      { configJson: CONFIG, configHash: HASH, acceptedAt: null },
    ])
  ...
  it("throws when the persisted hash no longer matches the stored config", async () => {
    const { executor } = fakeExecutor([
      { configJson: CONFIG, configHash: "sha256:stale", acceptedAt: null },
    ])
