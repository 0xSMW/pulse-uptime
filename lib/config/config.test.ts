...
function document(
  ...
  overrides: Partial<DeclarativeConfig["settings"]> = {}
): DeclarativeConfig {
  const ids = [
    ...new Set(
      monitors
...
describe("configuration schemas and normalization", () => {
  it("adapts legacy group names to stable v2 group references without changing the legacy hash", () => {
    const legacyMonitor = Object.fromEntries(
      Object.entries(monitor("api")).filter(([key]) => key !== "groupId")
    ) as LegacyMonitorConfig
    const legacy = {
  ...
  it("folds legacy group case variants without making valid v1 unreadable", () => {
    const legacyMonitors = [
      ...
      {
        ...Object.fromEntries(
          Object.entries(monitor("web")).filter(([key]) => key !== "groupId")
        ),
        group: "production",
      },
    ]
    const adapted = validateMonitoringConfig({
      schemaVersion: 1,
      configVersion: 7,
      settings,
  ...
  it("rejects dangling group references and case-insensitive duplicate names", () => {
    expect(() =>
      validateDeclarativeConfig({
        ...document([monitor("api")]),
        monitors: [monitor("api", { groupId: "missing" })],
      })
    ).toThrow()
    expect(() =>
      validateDeclarativeConfig({
  ...
  it("accepts valid complete documents and applies documented monitor defaults", () => {
    const created = createMonitorWithDefaults({
      id: "web",
      name: "Website",
    ...
    expect(
      resolveMonitorRecipients({ recipients: ["owner@example.com"] }, settings)
  ...
  it.each([
    ...
    [
      ...
      document([monitor("api", { url: "http://203.0.113.10/health" })]),
    ],
    [
      "disallowed port",
      document([monitor("api", { url: "https://example.com:8443" })]),
    ],
    ...
    [
      ...
      document([
        monitor("api", { expectedStatus: { minimum: 500, maximum: 200 } }),
      ]),
    ],
    [
  ...
  it("normalizes monitor order, recipients, strings, URLs, and object keys deterministically", () => {
    ...
    expect(normalized.monitors[0]).toMatchObject({
      name: "Alpha",
      groupId: "ops",
    })
    ...
    expect(normalized.settings.defaultRecipients).toEqual([
      "a@example.com",
      "z@example.com",
    ])
  ...
  it("enforces the 55 KB canonical serialized limit", () => {
    const huge = document([
      monitor("api", { recipients: [`a@${"x".repeat(56_000)}.com`] }),
    ])
...
describe("destructive-change tripwire", () => {
  it("trips when a non-empty active config becomes empty", () => {
    const result = evaluateDestructiveChange(
      document([monitor("api")]),
      document([])
    )
    expect(result.required).toBe(true)
    expect(result.reasons.map(({ type }) => type)).toContain(
      "all-active-monitors-removed"
    )
  })
  ...
  it("uses strict count and percentage thresholds", () => {
    ...
    const countResult = evaluateDestructiveChange(
      document(thirty),
      document(thirty.slice(6))
    )
    expect(countResult.reasons.map(({ type }) => type)).toContain(
      "removed-monitor-count"
    )
    expect(countResult.reasons.map(({ type }) => type)).not.toContain(
      "removed-monitor-percentage"
    )

    const twenty = Array.from({ length: 20 }, (_, index) =>
      monitor(`web-${index}`)
    )
    const percentageResult = evaluateDestructiveChange(
      document(twenty),
      document(twenty.slice(5))
    )
    expect(percentageResult.reasons.map(({ type }) => type)).toContain(
      "removed-monitor-percentage"
    )
    expect(percentageResult.reasons.map(({ type }) => type)).not.toContain(
      "removed-monitor-count"
    )
  })
  ...
  it("trips on removal of every previously active member of a 2+ monitor group", () => {
    const grouped = [
      monitor("grp-a", { groupId: "production" }),
      monitor("grp-b", { groupId: "production" }),
    ]
    ...
    const result = evaluateDestructiveChange(
      document([...grouped, ...unrelated]),
      document([...unrelated, replacement])
    )
    ...
    expect(result.reasons.map(({ type }) => type)).not.toContain(
      "removed-monitor-percentage"
    )
  })
...
describe("planning and export", () => {
  ...
  it("round-trips exported accepted configuration with an empty semantic plan", () => {
    ...
    expect(toMonitoringConfig(exported, 9)).toMatchObject({
      schemaVersion: 2,
      configVersion: 9,
    })
    expect(hashMonitoringConfig(accepted)).toBe(hashDeclarativeConfig(exported))
    expect(hashMonitoringConfig({ ...accepted, configVersion: 99 })).toBe(
      hashMonitoringConfig(accepted)
    )
  })
  ...
  it("produces stable diffs and plan hashes regardless of source monitor order", () => {
    ...
    const targetA = document(
      [monitor("bbb", { timeoutMs: 9000 }), monitor("aaa")],
      { concurrency: 30 }
    )
    const targetB = document([...targetA.monitors].reverse(), {
      concurrency: 30,
    })
    const first = createConfigurationPlan(current, targetA)
    const second = createConfigurationPlan(current, targetB)
    expect(first.planHash).toBe(second.planHash)
    expect(first.diff.settingsChanged.map(({ path }) => path)).toEqual([
  ...
  it("classifies pauses, resumes, updates, omitted archives, and creates", () => {
    const current = document([
      monitor("pause"),
      monitor("resume", { enabled: false }),
      monitor("gone"),
      monitor("changed", { enabled: false }),
    ])
    const target = document([
      monitor("pause", { enabled: false }),
      monitor("resume"),
      monitor("changed", { enabled: false, timeoutMs: 10_000 }),
      monitor("new-one"),
    ])
    const diff = calculateConfigurationDiff(
      validateDeclarativeConfig(current),
      validateDeclarativeConfig(target)
    )
  ...
  it("classifies an archived ID as a restore rather than a create", () => {
    ...
    const plan = createConfigurationPlan(
      document([]),
      document([monitor("old")]),
      { archivedMonitors: [archived] }
    )
...
describe("acceptance, approvals, and fallback", () => {
  ...
  it("accepts valid candidates and falls back to the last snapshot for invalid candidates", () => {
    ...
    const snapshot = {
      ...
      hash: hashMonitoringConfig(acceptedConfig),
    }
    expect(
      evaluateConfigurationAcceptance(
        runtime([monitor("api"), monitor("web")], 2),
        snapshot
      ).status
    ).toBe("accepted")
    ...
    expect(evaluateConfigurationAcceptance({ nope: true }, null)).toMatchObject(
  ...
  it("requires an exact, live, unconsumed approval for destructive acceptance", () => {
    ...
    expect(
      evaluateConfigurationAcceptance(desired, snapshot, { now })
    ).toMatchObject({
      status: "rejected",
...
describe("pure apply preconditions", () => {
  const current = validateDeclarativeConfig(
    document([monitor("api"), monitor("web")])
  )
  ...
  it.each([
    ...
    [
      ...
      { request: { ...request, targetConfigHash: "sha256:forged" } },
    ],
    [
      "PLAN_HASH_MISMATCH",
...
describe("authoritative allowDelete requirement", () => {
  it("requires allowDelete for a destructive change that archives no monitors", () => {
    ...
    const target = validateDeclarativeConfig(
      document(active.map((entry) => ({ ...entry, enabled: false })))
    )
    const baseConfigHash = hashDeclarativeConfig(current)
    const plan = createConfigurationPlan(current, target, { baseConfigHash })
