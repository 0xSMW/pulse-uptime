...
export function adaptLegacyDeclarativeConfig(input: unknown): DeclarativeConfig {
  ...
  for (const monitor of legacy.monitors) {
    ...
    if (name) {
      ...
      if (!namesByFolded.has(folded)) namesByFolded.set(folded, name);
    }
  }
  const groups = [...namesByFolded].map(([folded, name]) => ({ id: legacyGroupId(folded), name }));
  const ids = new Map(groups.map((group) => [group.name.toLocaleLowerCase("en-US"), group.id]));
...
export function validateDeclarativeConfig(input: unknown): DeclarativeConfig {
  if (typeof input === "object" && input !== null && (input as { version?: unknown }).version === 1) return adaptLegacyDeclarativeConfig(input);
...
export function validateMonitoringConfig(input: unknown): MonitoringConfig {
  if (typeof input === "object" && input !== null && (input as { schemaVersion?: unknown }).schemaVersion === 1) {
    ...
    const document = adaptLegacyDeclarativeConfig({ version: 1, settings: legacy.settings, monitors: legacy.monitors });
    return normalizeMonitoringConfig({ schemaVersion: 2, configVersion: legacy.configVersion, settings: document.settings, groups: document.groups, monitors: document.monitors });
  }
