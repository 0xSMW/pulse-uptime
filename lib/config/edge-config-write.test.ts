...
describe("writeMonitoringEdgeConfig", () => {
  ...
  it("PATCHes the monitoring item and returns the reported store version", async () => {
    const fetchSpy = vi.spyOn(global, "fetch").mockResolvedValue(
      new Response(null, { status: 200, headers: { "x-vercel-edge-config-version": "42" } }),
    );
    ...
    expect(JSON.parse(String(init?.body))).toEqual({ items: [{ operation: "upsert", key: "monitoring", value: CONFIG }] });
  });
