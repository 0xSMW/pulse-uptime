...
import { hashMonitoringConfig, validateMonitoringConfig, type MonitoringConfig } from "@/lib/config";
...
export async function readAcceptedSnapshot(executor: SnapshotReader = db): Promise<AcceptedSnapshot | null> {
  ...
  if (hashMonitoringConfig(raw) !== row.configHash) throw new Error("Accepted monitoring configuration hash is invalid");
  return { config, hash: row.configHash, acceptedAt: row.acceptedAt ?? null };
}

export async function findAcceptedSnapshot(executor: SnapshotReader = db): Promise<AcceptedSnapshot | null> {
  return readAcceptedSnapshot(executor);
}

export async function requireAcceptedSnapshot(executor: SnapshotReader = db): Promise<AcceptedSnapshot> {
  const snapshot = await readAcceptedSnapshot(executor);
  if (!snapshot) throw new Error("No accepted monitoring configuration is available");
  return snapshot;
}
