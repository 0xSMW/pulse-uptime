...
import {
  ...
  type DependencyPresetManifest,
  type DependencySourceManifest,
  loadCatalogManifest,
} from "./manifest"
import type {
...
export const SOURCE_DROPPED_FROM_MANIFEST_ERROR =
...
function droppedSourceIds(
  manifestSourceIds: ReadonlySet<string>,
  storedSourceIds: readonly string[]
): string[] {
...
export async function syncCatalog(
  ...
  return db.transaction(async (tx) => {
    ...
    const sourceFidelityById = new Map(
      manifest.sources.map(
        (source) => [source.id, source.fidelity ?? "component"] as const
      )
    )
    for (const preset of manifest.presets) {
      ...
      const fidelity =
        preset.fidelity ??
        sourceFidelityById.get(preset.sourceId) ??
        "component"
...
function presetDefinitionChanged(
  ...
  return (
    existing.sourceId !== preset.sourceId ||
 ...
 */
export function sourceUpsertPlan(
  source: DependencySourceManifest,
  catalogVersion: string
): SourceUpsertPlan {
  const insert = {
...
async function disablePresetSql(
  ...
  await tx
    .update(dependencyCatalog)
    .set({ enabled: false, validatedAt, validationError: error })
 ...
 */
export async function flipDependenciesToUnknownSql(
  tx: DatabaseTransaction,
  catalogId: string,
  observedAt: Date
): Promise<number> {
  const installed = await tx
    .select({ id: dependencies.id, state: dependencyState.state })
    .from(dependencies)
    .innerJoin(
      dependencyState,
    ...
    .where(
      and(eq(dependencies.catalogId, catalogId), isNull(dependencies.removedAt))
    )
  ...
  await tx
    ...
    .set({ state: "UNKNOWN", pendingFirstPoll: false, observedAt })
    .where(inArray(dependencyState.dependencyId, ids))

  const transitioning = installed
  ...
  if (transitioning.length > 0) {
    ...
    await tx
      ...
      .set({
        endedAt: sql`greatest(${observedAt.toISOString()}, ${dependencyStateIntervals.startedAt})`,
      })
      .where(
        and(
          inArray(dependencyStateIntervals.dependencyId, transitioning),
          isNull(dependencyStateIntervals.endedAt)
        )
      )
    await tx.insert(dependencyStateIntervals).values(
      transitioning.map((dependencyId) => ({
        id: randomUUID(),
        dependencyId,
    ...
    transaction: (work) =>
      db.transaction(async (tx) =>
        work({
          lock: async () => {
            await tx.execute(
              sql`select pg_advisory_xact_lock(hashtext('pulse:dependency-catalog'))`
            )
          },
          getStoredCatalogVersion: async () => {
            const [row] = await tx
              .select({ catalogVersion: dependencySources.catalogVersion })
          ...
          upsertPreset: async (preset, catalogVersion, fidelity) => {
            const [existing] = await tx
              ...
              .where(eq(dependencyCatalog.id, preset.id))
              .limit(1)
            const plan = presetUpsertPlan(
              existing
                ? {
                    ...
                    scope:
                      (existing.scopeOptions as DependencyScope | null) ?? null,
                  }
                : null,
              preset,
              catalogVersion,
          ...
          listEnabledSourceIds: async () =>
            (
              await tx
                .select({ id: dependencySources.id })
                .from(dependencySources)
                .where(eq(dependencySources.enabled, true))
            ).map((row) => row.id),
          disableSource: async (sourceId, observedAt, error) => {
            await tx
              ...
              .set({
                enabled: false,
                catalogValidatedAt: observedAt,
                catalogValidationError: error,
              })
          ...
          listEnabledPresetIdsForSource: async (sourceId) =>
            (
              await tx
                .select({ id: dependencyCatalog.id })
                .from(dependencyCatalog)
                .where(
                  and(
          ...
          disablePreset: (presetId, validatedAt, error) =>
            disablePresetSql(tx, presetId, validatedAt, error),
          flipDependenciesToUnknown: (catalogId, observedAt) =>
            flipDependenciesToUnknownSql(tx, catalogId, observedAt),
        })
      ),
  }
}
...
export function observedScopeOptionsForPreset(
  ...
  if (scope.kind === "discovered_locations") {
    const productId =
      preset.selector.kind === "google_product"
        ? preset.selector.productId
        : null
    const locations = productId
      ? (directory.locationsByProduct.get(productId) ?? [])
      : []
...
export function planDiscoveredScopeSync(
  ...
  return {
    upserts: observed.map((option) => ({
      ...option,
      available: true as const,
      lastSeenAt: observedAt,
    })),
    unavailableScopeIds: prior
      .map((row) => row.scopeId)
      .filter((scopeId) => !observedIds.has(scopeId)),
    observedAt,
  }
}
...
export async function syncDiscoveredScopeOptionsSql(
  ...
  const prior = await tx
    .select({ scopeId: dependencyDiscoveredScopeOptions.scopeId })
  ...
  for (const option of plan.upserts) {
    await tx
      .insert(dependencyDiscoveredScopeOptions)
      .values({
        catalogId,
      ...
      .onConflictDoUpdate({
        target: [
          ...
          dependencyDiscoveredScopeOptions.scopeId,
        ],
        set: {
          label: option.label,
          scopeKind: option.scopeKind,
  ...
  if (plan.unavailableScopeIds.length > 0) {
    await tx
      .update(dependencyDiscoveredScopeOptions)
      .set({ available: false })
      .where(
        and(
...
export async function reconcileCatalog(
  deps: ReconcileCatalogDeps
): Promise<ReconcileCatalogSummary> {
  const now = deps.now ?? (() => new Date())
  const nowMs = deps.nowMs ?? Date.now
  const deadlineAtMs = deps.deadlineAtMs ?? Number.POSITIVE_INFINITY
  const fetchDirectory =
    deps.fetchCatalogDirectory ?? deps.fetchSourceComponents
  if (!fetchDirectory) {
      ...
      for (const preset of presets) {
        ...
        if (missing.length > 0) {
          ...
          await tx.disablePreset(
            preset.id,
            validatedAt,
            `Missing upstream component ids: ${missing.join(", ")}`
          )
  ...
  return {
    checkedSources: processed.length,
    validatedPresets,
    disabledPresets,
    unknownDependencies,
  }
...
export function createSqlCatalogReconcileStore(
  db: Database
): CatalogReconcileStore {
  return {
        ...
        work({
            ...
            (await tx
              ...
              .where(
                and(
                  ...
                  or(
                    eq(dependencyCatalog.enabled, true),
                    isNotNull(dependencyCatalog.validationError)
                  )
                )
              )) as PresetRow[],
          recordSourceValidation: async (sourceId, validatedAt, error) => {
            await tx
              .update(dependencySources)
              .set({
                catalogValidatedAt: validatedAt,
                catalogValidationError: error,
              })
              .where(eq(dependencySources.id, sourceId))
          },
          recordPresetValidationOk: async (presetId, validatedAt) => {
            await tx
              .update(dependencyCatalog)
              .set({ validatedAt, validationError: null })
              .where(eq(dependencyCatalog.id, presetId))
          },
          reEnablePreset: async (presetId, validatedAt) => {
            await tx
              .update(dependencyCatalog)
              .set({ enabled: true, validatedAt, validationError: null })
              .where(eq(dependencyCatalog.id, presetId))
          },
          disablePreset: (presetId, validatedAt, error) =>
            disablePresetSql(tx, presetId, validatedAt, error),
