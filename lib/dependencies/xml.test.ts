...
<rss><channel><item><title>&lol3;</title></item></channel></rss>`
  ...
  it("rejects oversized input before scanning", () => {
    ...
    expect(() => parseFeed(huge, { maxInputBytes: 1000 })).toThrow(
      XmlParseError
    )
    try {
  ...
  it("caps the number of items and the length of each text field", () => {
    const manyItems = Array.from(
      { length: 50 },
      (_, index) => `<item><title>t${index}</title></item>`
    ).join("")
    const items = parseFeed(`<rss><channel>${manyItems}</channel></rss>`, {
      maxItems: 5,
    })
...
describe("parseFeed structural validation", () => {
  ...
  it("returns an empty successful feed for a valid empty Atom envelope", () => {
    expect(
      parseFeed('<feed xmlns="http://www.w3.org/2005/Atom"></feed>')
    ).toEqual([])
  })
