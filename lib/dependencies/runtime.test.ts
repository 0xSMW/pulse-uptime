...
describe("runDependencyCronCoordinator", () => {
  ...
  it("syncs the catalog, loads recipients, polls, reconciles stale claims, and drains the outbox in order on a completed run", async () => {
    ...
    const deps = fakeDeps({
      ...
      reconcileOutbox: vi.fn(async () => { callOrder.push("reconcile"); return 2; }),
      deliverOutbox: vi.fn(async () => { callOrder.push("deliver"); return { claimed: 1, sent: 1, failed: 0, dead: 0, lostClaims: 0 }; }),
    });

    const result = await runDependencyCronCoordinator(deps);

    expect(callOrder).toEqual(["sync", "recipients", "poll:ops@example.com", "reconcile", "deliver"]);
...
describe("createDueSourceStore", () => {
  it("selects due sources under a skip-locked lock and advances next_poll_at in the same statement", () => {
    ...
    expect(CLAIM_DUE_SOURCES_SQL).toMatch(/update dependency_sources[\s\S]*set next_poll_at = \$2[\s\S]*returning/i);
    ...
    expect(CLAIM_DUE_SOURCES_SQL).toMatch(/next_poll_at is null or ds\.next_poll_at <= \$1/i);
  });
  ...
  it("claims with a near-future floor and enriches each row's cadence from the manifest", async () => {
    ...
    expect(query).toHaveBeenCalledWith(CLAIM_DUE_SOURCES_SQL, [claimed, new Date("2026-07-19T15:31:00.000Z")]);
  ...
  it("drops a claimed source that is not in the shipped manifest rather than guessing a cadence", async () => {
    ...
    const store = createDueSourceStore({ query } as unknown as SqlExecutor);
    const rows = await store.listDueSources(new Date("2026-07-19T15:30:00.000Z"));
    expect(rows).toEqual([]);
  });
});
