...
describe("loadCatalogManifest", () => {
  ...
  it("declares incidentInventory for every incident_feed source", () => {
    const manifest = loadCatalogManifest()
    const feeds = manifest.sources.filter(
      (source) => source.adapter === "incident_feed"
    )
    expect(feeds.length).toBeGreaterThanOrEqual(2)
    for (const source of feeds) {
      expect(["active_only", "rolling_history"]).toContain(
        source.config.incidentInventory
      )
    }
    expect(
      manifest.sources.find((source) => source.id === "azure")?.config
        .incidentInventory
    ).toBe("active_only")
    expect(
      manifest.sources.find((source) => source.id === "openrouter")?.config
        .incidentInventory
    ).toBe("rolling_history")
  })
  ...
  it("keeps source and preset IDs unique", () => {
    const manifest = loadCatalogManifest()
    expect(new Set(manifest.sources.map((source) => source.id)).size).toBe(
      manifest.sources.length
    )
    expect(new Set(manifest.presets.map((preset) => preset.id)).size).toBe(
      manifest.presets.length
    )
  })
  ...
  it("requires a region for Neon and only ever offers validated region containers", () => {
    const manifest = loadCatalogManifest()
    const neon = manifest.presets.find(
      (preset) => preset.id === "neon_database"
    )
  ...
  it("discloses the wider scope of Stripe's broad presets", () => {
    const manifest = loadCatalogManifest()
    const checkout = manifest.presets.find(
      (preset) => preset.id === "stripe_checkout"
    )
    const webhooks = manifest.presets.find(
      (preset) => preset.id === "stripe_webhooks"
    )
    expect(checkout?.sourceScopeNote).toBe(
      "Provider source also covers Elements, Connect, Payment Links, Terminal, and 3DS."
    )
    expect(webhooks?.sourceScopeNote).toBe(
      "Provider source also covers Dashboard, support, and payouts."
    )
  })
})
...
describe("catalogManifestSchema invariants", () => {
  ...
  it("rejects a preset referencing a source that does not exist", () => {
    ...
    const broken = {
      ...manifest,
      presets: [
        { ...manifest.presets[0], sourceId: "does-not-exist" },
        ...manifest.presets.slice(1),
      ],
    }
  ...
  it("rejects a source whose allowedHosts do not cover its URLs", () => {
    ...
    const broken = {
      ...
      sources: [
        {
          ...manifest.sources[0],
          allowedHosts: ["not-the-right-host.example"],
        },
        ...manifest.sources.slice(1),
      ],
    }
    expect(() => catalogManifestSchema.parse(broken)).toThrow()
  })
...
describe("catalogManifestSchema fidelity", () => {
  ...
  it("accepts an incident_only fidelity on a source and a preset", () => {
    ...
    const withFidelity = {
      ...manifest,
      sources: [
        { ...manifest.sources[0], fidelity: "incident_only" },
        ...manifest.sources.slice(1),
      ],
      presets: [
        { ...manifest.presets[0], fidelity: "incident_only" },
        ...manifest.presets.slice(1),
      ],
    }
  ...
  it("treats a missing fidelity as valid, defaulting resolution to component at sync time", () => {
    ...
    expect(
      manifest.sources.every(
        (source) =>
          source.fidelity === undefined ||
          source.fidelity === "component" ||
          source.fidelity === "incident_only"
      )
    ).toBe(true)
  ...
  it("rejects an unknown fidelity value", () => {
    ...
    const broken = {
      ...manifest,
      presets: [
        { ...manifest.presets[0], fidelity: "guessed" },
        ...manifest.presets.slice(1),
      ],
    }
    expect(() => catalogManifestSchema.parse(broken)).toThrow()
...
describe("catalogManifestSchema adapter names", () => {
  ...
  it("rejects an adapter name outside the union", () => {
    ...
    const broken = {
      ...manifest,
      sources: [
        { ...manifest.sources[0], adapter: "made_up_adapter" },
        ...manifest.sources.slice(1),
      ],
    }
    expect(() => catalogManifestSchema.parse(broken)).toThrow()
...
describe("catalogManifestSchema incident_feed inventory", () => {
  ...
  it("rejects an incident_feed source that omits incidentInventory", () => {
    ...
    const { incidentInventory: _dropped, ...configWithout } = azure.config as {
      incidentInventory?: string
    ...
    const broken = {
      ...
      sources: manifest.sources.map((source) =>
        source.id === "azure"
          ? {
              ...source,
              config: { ...source.config, incidentInventory: "guessed" },
            }
          : source
      ),
    }
