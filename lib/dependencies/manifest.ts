...
const dependencyCategorySchema = z.enum([
  "ai",
  "hosting",
  "auth",
  "data",
...
const componentIdsSelectorSchema = z
  .object({
...
const googleProductSelectorSchema = z
  .object({
...
const statusioComponentContainerSelectorSchema = z
  .object({
...
const requiredOptionsScopeSchema = z
  .object({
...
const discoveredChildrenScopeSchema = z
  .object({
...
const discoveredLocationsScopeSchema = z
  .object({
    ...
    if (source.adapter === "incident_feed") {
      ...
      if (inventory !== "active_only" && inventory !== "rolling_history") {
        context.addIssue({
          code: "custom",
          message:
            'config.incidentInventory is required for incident_feed and must be "active_only" or "rolling_history"',
