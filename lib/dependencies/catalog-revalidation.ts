...
async function fetchAdapterCatalogDirectory(
  ...
  try {
    const collected = await collectAdapterDocuments({
      ...
      fetchDocument: (request, fetchOptions) => fetchAdapterRequest(source, request, {
        dispatcher,
    ...
    if (collected.status !== "complete") return null;
    if (collected.documents.length === 0) return null;
  ...
  return async (args) => {
    ...
    try {
      if (manifestSource.adapter === "google_cloud_status") {
        ...
        return await fetchGoogleProductsDirectory(manifestSource, productsUrl, args);
      }
