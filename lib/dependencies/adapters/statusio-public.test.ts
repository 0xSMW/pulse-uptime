...
const neonPreset = manifest.presets.find(
...
describe("statusioPublicAdapter.requests", () => {
  it("requests only the current state document, Neon's incident endpoints return 403", () => {
    expect(statusioPublicAdapter.requests(neonSource)).toEqual([
      { kind: "current", url: neonSource.currentUrl, optional: false },
    ])
  })
})
...
describe("statusioPublicAdapter.normalize: component and region selection against the catalog", () => {
  it("selects the neon_database preset's component and its required region containers", () => {
    expect(neonPreset.selector).toMatchObject({
      kind: "statusio_component_container",
      componentId: "690ccc238c3745059f2b33b2",
    })
    expect(neonPreset.scope).toMatchObject({ kind: "required_options" })
    const snapshot = statusioPublicAdapter.normalize({
      source: neonSource,
      documents: [currentDoc(operational)],
      observedAt: "2026-07-19T12:00:00.000Z",
    })
    expect(snapshot.components["690ccc238c3745059f2b33b2"]).toMatchObject({
      state: "OPERATIONAL",
    })
    const region =
      neonPreset.scope?.kind === "required_options"
        ? neonPreset.scope.options[0]
        : null
    expect(region?.id).toBe("690ccafbbfd4c50578f181cc")
    expect(snapshot.components[region!.id]).toMatchObject({
      state: "OPERATIONAL",
    })
  })
  ...
  it("maps a degraded region container independently from the parent component's own state", () => {
    ...
    expect(snapshot.components["690ccafbbfd4c50578f181cc"]).toMatchObject({
      state: "OPERATIONAL",
    })
    expect(snapshot.components["690ccb48769e0d058c5c353d"]).toMatchObject({
      state: "DEGRADED",
    })
  })
...
describe("statusioPublicAdapter.normalize: no invented incidents", () => {
  ...
  it("normalizing the same fixture twice yields an identical snapshot shape", () => {
    const first = statusioPublicAdapter.normalize({
      source: neonSource,
      documents: [currentDoc(operational)],
      observedAt: "2026-07-19T12:00:00.000Z",
    })
    const second = statusioPublicAdapter.normalize({
      source: neonSource,
      documents: [currentDoc(operational)],
...
describe("statusioPublicAdapter.normalize: failure handling", () => {
  it("throws AdapterParseError on an unrecognized top-level shape", () => {
    expect(() =>
      statusioPublicAdapter.normalize({
        source: neonSource,
  ...
  it("throws SCHEMA_INVALID on an unparseable component timestamp instead of storing an Invalid Date", () => {
    try {
      statusioPublicAdapter.normalize({
        source: neonSource,
        documents: [currentDoc(malformedTimestamp)],
        observedAt: "2026-07-19T12:00:00.000Z",
      })
      expect.unreachable()
    } catch (error) {
