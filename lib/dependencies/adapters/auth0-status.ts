...
function incidentComponentState(incident: Auth0Incident): ComponentState {
  if (isMaintenance(incident)) return "MAINTENANCE";
  if (incident.impact === "critical" || incident.impact === "major") return "OUTAGE";
  return "DEGRADED";
}
...
function mapIncidentStatus(status: string, sourceId: string): string {
  const folded = status === "verifying" ? "monitoring" : status === "postmortem" ? "resolved" : status;
...
function extractNextData(text: string, sourceId: string): unknown {
  ...
  if (!match) {
    throw new AdapterParseError("SCHEMA_INVALID", `${sourceId}: homepage has no __NEXT_DATA__ payload`);
  }
  try {
    return JSON.parse(match[1]);
  } catch {
    throw new AdapterParseError("SCHEMA_INVALID", `${sourceId}: __NEXT_DATA__ payload is not valid JSON`);
  }
}

function requireCurrentText(documents: AdapterDocument[], sourceId: string): string {
  const current = documentsOfKind(documents, "current")[0];
  if (!current || typeof current.text !== "string") {
    throw new AdapterParseError("MISSING_DOCUMENT", `${sourceId}: missing homepage document`);
  }
  return current.text;
}
...
export const auth0StatusAdapter: DependencyAdapter = {
  ...
  normalize(input: NormalizeInput): NormalizedProviderSnapshot {
    ...
    const parsed = nextDataSchema.safeParse(extractNextData(requireCurrentText(documents, sourceId), sourceId));
    if (!parsed.success) {
      throw new AdapterParseError("SCHEMA_INVALID", `${sourceId}: activeIncidents failed schema validation: ${parsed.error.message}`);
    }
    ...
    for (const region of regions) {
      ...
      for (const incident of region.response.incidents) {
        ...
        if (existing) {
          ...
          existing.authenticationAffected = existing.authenticationAffected || incident.authenticationAffected;
          if (new Date(incident.updated_at) > new Date(existing.latestUpdatedAt)) {
    ...
    const incidents: NormalizedProviderSnapshot["incidents"] = [...incidentsById.values()].map((entry) => {
    ...
    const maintenances: NormalizedProviderSnapshot["maintenances"] = [...maintenancesById.values()].map((entry) => {
      const { incident } = entry;
      return {
        externalId: incident.id,
        state: mapIncidentStatus(incident.status, sourceId),
        startsAt: requireIsoTimestamp(incidentStartedAt(incident), sourceId, "maintenance.startsAt"),
        endsAt: incident.resolved_at ? requireIsoTimestamp(incident.resolved_at, sourceId, "maintenance.resolved_at") : null,
        componentIds: entry.regions,
      };
    });
