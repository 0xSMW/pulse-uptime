  ...
  switch (state) {
    ...
    default:
      throw new AdapterParseError(
        "UNKNOWN_STATUS",
        `${sourceId}: unrecognized component state "${state}"`
      )
  }
}
...
function componentsUrlOf(source: DependencySourceManifest): string {
  ...
  if (typeof value !== "string") {
    throw new AdapterParseError(
      "SCHEMA_INVALID",
      `${source.id}: config.componentsUrl is required for sorry_v1`
    )
  }
  return value
}

function noticeDetailUrlOf(
  ...
  if (typeof template !== "string") {
    throw new AdapterParseError(
      "SCHEMA_INVALID",
      `${source.id}: config.noticeDetailUrlTemplate is required for sorry_v1`
    )
  }
  return template.replace("{id}", String(noticeId))
}

function unplannedNoticesUrl(
  source: DependencySourceManifest,
  timelineState: "present" | "past"
): string {
  if (!source.incidentsUrl) {
    throw new AdapterParseError(
      "SCHEMA_INVALID",
      `${source.id}: incidentsUrl is required for sorry_v1`
    )
  }
...
function peekComponentsMeta(
  document: AdapterDocument
): z.infer<typeof pageMetaSchema> | null {
  const parsed = componentsDocSchema.safeParse(document.json)
  return parsed.success ? parsed.data.meta : null
}

function peekNoticesList(
  document: AdapterDocument
): z.infer<typeof noticesListDocSchema> | null {
...
function fetchedPastListPages(
  ...
  for (const document of documentsOfKind(fetchedSoFar, "incidents")) {
    ...
    if (list) {
      listByUrl.set(document.url, list)
    }
  }
  const chain: string[] = []
...
export const sorryV1Adapter: DependencyAdapter = {
  requests(
    ...
    if (!fetchedSoFar || fetchedSoFar.length === 0) {
      return [
        ...
        {
          kind: "incidents",
          url: presentUnplannedNoticesUrl(source),
          optional: false,
        },
        {
          kind: "incidents",
          url: pastUnplannedNoticesUrl(source),
          optional: false,
        },
      ]
    }
    ...
    for (const document of documentsOfKind(fetchedSoFar, "current")) {
      ...
      const nextUrl = meta
        ? resolveNextPage(meta.next_page, document.url)
        : null
      if (nextUrl && !fetchedUrls.has(nextUrl)) {
        next.push({ kind: "current", url: nextUrl, optional: false })
      }
    }
    ...
    for (const document of documentsOfKind(fetchedSoFar, "incidents")) {
      ...
      const followNextPage =
        !isPastList || pastPageIndex < PAST_LIST_MAX_PAGES - 1
      const nextUrl = followNextPage
        ? resolveNextPage(list.meta.next_page, document.url)
        : null
      if (nextUrl && !fetchedUrls.has(nextUrl)) {
        next.push({ kind: "incidents", url: nextUrl, optional: false })
      }
      for (const notice of list.notices) {
        if (notice.type !== "unplanned") {
          continue
        }
        const detailUrl = noticeDetailUrlOf(source, notice.id)
        if (!fetchedUrls.has(detailUrl)) {
          next.push({ kind: "incidents", url: detailUrl, optional: false })
        }
      }
    }
  ...
  normalize(input: NormalizeInput): NormalizedProviderSnapshot {
    ...
    if (currentDocuments.length === 0) {
      throw new AdapterParseError(
        "MISSING_DOCUMENT",
        `${source.id}: missing components document`
      )
    }
    ...
    for (const document of currentDocuments) {
      ...
      if (!parsed.success) {
        throw new AdapterParseError(
          "SCHEMA_INVALID",
          `${source.id}: components document failed schema validation: ${parsed.error.message}`
        )
      }
    ...
    const listDocuments = incidentDocuments.filter(
      (document) => !isNoticeDetailDocument(document)
    )
    const detailDocuments = incidentDocuments.filter((document) =>
      isNoticeDetailDocument(document)
    )
    ...
    for (const document of listDocuments) {
      ...
      if (!parsed.success) {
        throw new AdapterParseError(
          "SCHEMA_INVALID",
          `${source.id}: notices document failed schema validation: ${parsed.error.message}`
        )
      }
      for (const notice of parsed.data.notices) {
        if (notice.type === "unplanned") {
          observedUnplannedIds.add(notice.id)
        }
      }
    }
    ...
    for (const document of detailDocuments) {
      ...
      if (!parsed.success) {
        throw new AdapterParseError(
          "SCHEMA_INVALID",
          `${source.id}: notice detail document failed schema validation: ${parsed.error.message}`
        )
      }
      const { notice } = parsed.data
      if (notice.type !== "unplanned" || seenNoticeIds.has(notice.id)) {
        continue
      }
      ...
      incidents.push({
        ...
        startedAt: requireIsoTimestamp(
          notice.began_at ?? notice.created_at,
          source.id,
          "notice.began_at"
        ),
        resolvedAt: notice.ended_at
          ? requireIsoTimestamp(notice.ended_at, source.id, "notice.ended_at")
          : null,
        ...
        scope: scopeFromComponentIds(
          notice.components.map((component) => String(component.id))
        ),
        updates: notice.updates.map((update) => ({
          ...
          createdAt: requireIsoTimestamp(
            update.created_at,
            source.id,
            "notice_update.created_at"
          ),
          updatedAt: requireIsoTimestamp(
            update.updated_at,
            source.id,
            "notice_update.updated_at"
          ),
        })),
      })
    }
    ...
    for (const id of observedUnplannedIds) {
      if (!seenNoticeIds.has(id)) {
        throw new AdapterParseError(
          "MISSING_DOCUMENT",
          `${source.id}: missing notice detail document for notice ${id}`
        )
      }
    }
