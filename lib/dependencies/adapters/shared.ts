...
export type AdapterParseErrorCode = "SCHEMA_INVALID" | "UNKNOWN_STATUS" | "MISSING_DOCUMENT";

export class AdapterParseError extends Error {
  constructor(readonly code: AdapterParseErrorCode, message: string) {
    super(message);
    this.name = "AdapterParseError";
  }
}

const PROVIDER_INCIDENT_STATE_SET: ReadonlySet<string> = new Set(providerIncidentStates);

/** True only for the 9-value vocabulary the provider_incidents check constraint accepts. */
export function isProviderIncidentState(value: string): value is (typeof providerIncidentStates)[number] {
...
export function requireProviderIncidentState(value: string, sourceId: string): (typeof providerIncidentStates)[number] {
...
export function requireIsoTimestamp(value: string, sourceId: string, field: string): string {
  const parsed = new Date(value);
  if (Number.isNaN(parsed.getTime())) {
    throw new AdapterParseError("SCHEMA_INVALID", `${sourceId}: invalid timestamp for ${field}: "${value}"`);
  }
  return value;
}
...
export function latestTimestamp(values: Array<string | null | undefined>): string | null {
  ...
  return present.reduce((latest, current) => (new Date(current) > new Date(latest) ? current : latest));
}

/** All fetched documents of one role (there can be several: paginated components, one notice detail per notice). */
export function documentsOfKind(documents: AdapterDocument[], kind: AdapterDocumentKind): AdapterDocument[] {
...
export function requireJson(document: AdapterDocument | undefined, sourceId: string, what: string): unknown {
  if (!document || document.json === undefined) {
    throw new AdapterParseError("MISSING_DOCUMENT", `${sourceId}: missing ${what} document`);
  }
