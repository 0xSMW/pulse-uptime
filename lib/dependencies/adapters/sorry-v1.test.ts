...
const postmarkSource = manifest.sources.find(
  (source) => source.id === "postmark"
)!
const emailDeliveryPreset = manifest.presets.find(
  (preset) => preset.id === "postmark_email_delivery"
)!

const componentsUrl = postmarkSource.config.componentsUrl as string
const noticesBaseUrl = postmarkSource.incidentsUrl!
const notice503440Url = (
  postmarkSource.config.noticeDetailUrlTemplate as string
).replace("{id}", "503440")
const notice503441Url = (
  postmarkSource.config.noticeDetailUrlTemplate as string
).replace("{id}", "503441")
const notice503442Url = (
  postmarkSource.config.noticeDetailUrlTemplate as string
).replace("{id}", "503442")

function doc(
  kind: AdapterDocument["kind"],
  url: string,
  json: unknown
): AdapterDocument {
...
describe("sorryV1Adapter.requests: pagination follow-up", () => {
  it("asks for the next components page when meta.next_page is present", () => {
    const fetchedSoFar: AdapterDocument[] = [
      ...
      doc(
        "incidents",
        `${noticesBaseUrl}?filter%5Btimeline_state_eq%5D=present&filter%5Btype_eq%5D=unplanned`,
        noticesEmpty
      ),
    ]
    ...
    expect(requests).toEqual([
      {
        kind: "current",
        url: new URL("/api/v1/components?page=2", componentsUrl).toString(),
        optional: false,
      },
    ])
  })
  ...
  it("follows a paginated notices list across pages, requesting each page's own notice detail", () => {
    ...
    const afterPage1Requests = sorryV1Adapter.requests(
      postmarkSource,
      afterPage1
    )
    const page2Url = new URL(
      "/api/v1/notices?page=2",
      noticesBaseUrl
    ).toString()
    expect(afterPage1Requests).toEqual(
      expect.arrayContaining([
        { kind: "incidents", url: page2Url, optional: false },
        { kind: "incidents", url: notice503440Url, optional: false },
      ])
    )
    ...
    const afterPage2: AdapterDocument[] = [
      ...afterPage1,
      doc("incidents", page2Url, noticesPage2),
      doc("incidents", notice503440Url, noticeDetail503440),
    ]
    const afterPage2Requests = sorryV1Adapter.requests(
      postmarkSource,
      afterPage2
    )
    expect(afterPage2Requests).toEqual([
      { kind: "incidents", url: notice503441Url, optional: false },
    ])
  })
  ...
  it("follows the past-unplanned list beyond its first page so a notice that resolved onto a deeper past page is still reached", () => {
    ...
    const pastPage2Url = new URL(
      "/api/v1/notices?filter%5Btimeline_state_eq%5D=past&filter%5Btype_eq%5D=unplanned&page=2",
      noticesBaseUrl
    ).toString()
    ...
    const pastPage3Url = new URL(
      "/api/v1/notices?page=3",
      noticesBaseUrl
    ).toString()
    expect(afterPage2Requests).toEqual(
      expect.arrayContaining([
  ...
  it("stops following the past-unplanned list at the bounded page cap, it does not walk deeper history", () => {
    ...
    const pastPage4Url = new URL(
      "/api/v1/notices?page=4",
      noticesBaseUrl
    ).toString()
    ...
    const afterPage3Requests = sorryV1Adapter.requests(
      postmarkSource,
      afterPage3
    )
    // The one remaining request is page 3's own notice detail, never a fourth past page.
    expect(afterPage3Requests).toEqual([
      { kind: "incidents", url: notice503441Url, optional: false },
    ])
    expect(
      afterPage3Requests.some((request) => request.url === pastPage4Url)
    ).toBe(false)

    const afterAll: AdapterDocument[] = [
      ...afterPage3,
...
describe("sorryV1Adapter.normalize: pagination handling", () => {
  it("merges components across multiple fetched pages", () => {
    const snapshot = sorryV1Adapter.normalize({
      ...
      documents: [
        doc("current", componentsUrl, componentsPage1),
        doc("current", `${componentsUrl}?page=2`, componentsPage2),
...
describe("sorryV1Adapter.normalize: component selection against the catalog", () => {
  it("selects the postmark_email_delivery preset's components by their real numeric ids", () => {
    expect(emailDeliveryPreset.selector).toMatchObject({
      kind: "component_ids",
      ids: ["87154", "87151"],
    })
    const snapshot = sorryV1Adapter.normalize({
      source: postmarkSource,
      documents: [
        doc("current", componentsUrl, componentsDegraded),
        doc("incidents", noticesBaseUrl, noticesEmpty),
      ],
      observedAt: "2026-07-19T12:00:00.000Z",
    })
  ...
  it("maps operational, degraded, and under_maintenance", () => {
    ...
    expect(
      sorryV1Adapter.normalize({
        source: postmarkSource,
        documents: [
          doc("current", componentsUrl, componentsMaintenance),
          empty,
        ],
        observedAt: "2026-07-19T12:00:00.000Z",
...
describe("sorryV1Adapter.normalize: notices", () => {
  it("builds a full incident from the notice list plus its detail document, with impacted components and updates", () => {
    const snapshot = sorryV1Adapter.normalize({
      ...
      documents: [
        doc("current", componentsUrl, componentsOperational),
        doc("incidents", noticesBaseUrl, noticesListOne),
        doc("incidents", notice503440Url, noticeDetail503440),
      ],
  ...
  it("normalizing the same documents twice yields identical incident and update external ids", () => {
    ...
    const first = sorryV1Adapter.normalize({
      source: postmarkSource,
      documents,
      observedAt: "2026-07-07T02:00:00.000Z",
    })
    const second = sorryV1Adapter.normalize({
      source: postmarkSource,
      documents,
      observedAt: "2026-07-07T02:05:00.000Z",
    })
  ...
  it("assembles incidents across a paginated notice list, each with its own detail document", () => {
    const snapshot = sorryV1Adapter.normalize({
      ...
      documents: [
        ...
        doc(
          "incidents",
          new URL("/api/v1/notices?page=2", noticesBaseUrl).toString(),
          noticesPage2
        ),
        doc("incidents", notice503440Url, noticeDetail503440),
        doc("incidents", notice503441Url, noticeDetail503441),
...
describe("sorryV1Adapter.normalize: a notice that ends between polls", () => {
  ...
  it("observes a resolution that landed on the second past page, not just the first", () => {
    ...
    const resolved = snapshot.incidents.find(
      (incident) => incident.externalId === "503442"
    )
...
describe("sorryV1Adapter.normalize: failure handling", () => {
  it("throws AdapterParseError on an unrecognized top-level shape", () => {
    expect(() =>
      sorryV1Adapter.normalize({
        source: postmarkSource,
        documents: [
          doc("current", componentsUrl, malformedComponents),
          doc("incidents", noticesBaseUrl, noticesEmpty),
        ],
        observedAt: "2026-07-19T12:00:00.000Z",
      })
  ...
  it("does not throw when a catalog component id has been removed from the feed", () => {
    const snapshot = sorryV1Adapter.normalize({
      source: postmarkSource,
      documents: [
        doc("current", componentsUrl, componentsMissingComponent),
        doc("incidents", noticesBaseUrl, noticesEmpty),
      ],
      observedAt: "2026-07-19T12:00:00.000Z",
    })
