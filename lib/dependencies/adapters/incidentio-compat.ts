...
function findDocument(documents: AdapterDocument[], kind: "current" | "incidents"): AdapterDocument | undefined {
  return documentsOfKind(documents, kind)[0];
}

function mapIncident(incident: Incident, componentIds: string[], sourceId: string): NormalizedProviderSnapshot["incidents"][number] {
  return {
    externalId: incident.id,
    title: incident.name,
    state: requireProviderIncidentState(incident.status, sourceId),
    impact: incident.impact ?? null,
    startedAt: requireIsoTimestamp(incident.created_at, sourceId, "incident.created_at"),
...
export const incidentioCompatAdapter: DependencyAdapter = {
  requests(source: DependencySourceManifest): AdapterRequestDescriptor[] {
    ...
    if (source.incidentsUrl) {
      requests.push({ kind: "incidents", url: source.incidentsUrl, optional: false });
    }
  ...
  normalize(input: NormalizeInput): NormalizedProviderSnapshot {
    ...
    const inferredComponentIds = activeIncidents.length === 1 ? nonOperationalComponentIds : [];

    const incidents = allIncidents.map((incident) =>
      mapIncident(incident, incident.status !== "resolved" && activeIncidents.length === 1 ? inferredComponentIds : [], source.id),
    );
