...
function mapIncidentState(raw: string, sourceId: string): string {
  ...
  if (isProviderIncidentState(raw)) {
    return raw
  }
  throw new AdapterParseError(
    "UNKNOWN_STATUS",
    `${sourceId}: unrecognized incident state "${raw}"`
  )
}
...
export function extractNextDataSlice(html: string, sourceId: string): string {
  ...
  if (!openMatch) {
    throw new AdapterParseError(
      "MISSING_DOCUMENT",
      `${sourceId}: no __NEXT_DATA__ script tag in status page`
    )
  }
  ...
  if (end === -1) {
    throw new AdapterParseError(
      "MISSING_DOCUMENT",
      `${sourceId}: __NEXT_DATA__ script tag was never closed`
    )
  }
  ...
  if (slice.length > MAX_NEXT_DATA_BYTES) {
    throw new AdapterParseError(
      "SCHEMA_INVALID",
      `${sourceId}: embedded __NEXT_DATA__ payload exceeds the ${MAX_NEXT_DATA_BYTES} byte cap`
    )
  }
  ...
  if (byteLength > MAX_NEXT_DATA_BYTES) {
    throw new AdapterParseError(
      "SCHEMA_INVALID",
      `${sourceId}: embedded __NEXT_DATA__ payload of ${byteLength} bytes exceeds the ${MAX_NEXT_DATA_BYTES} byte cap`
    )
  }
  return slice
}
...
function requireText(
  ...
  if (!document || document.text === undefined) {
    throw new AdapterParseError(
      "MISSING_DOCUMENT",
      `${sourceId}: missing status page document`
    )
  }
...
export const nextdataEmbeddedAdapter: DependencyAdapter = {
  ...
  normalize(input: NormalizeInput): NormalizedProviderSnapshot {
    ...
    const html = requireText(
      documentsOfKind(documents, "current")[0],
      source.id
    )
    ...
    if (!parsed.success) {
      throw new AdapterParseError(
        "SCHEMA_INVALID",
        `${source.id}: __NEXT_DATA__ failed schema validation: ${parsed.error.message}`
      )
    }
    ...
    for (const item of allItems) {
      ...
      // emitted once.
      if (seenIncidentIds.has(externalId)) {
        continue
      }
      ...
      if (item.incidentType === "maintenance") {
        maintenances.push(
          mapMaintenance(item, mappedState, systemId, source.id)
        )
      } else {
...
function mapIncident(
  ...
  const title =
    firstNonEmpty(item.titleEn, item.titleDe) ??
    `Hetzner ${item.incidentType} notice`
  ...
  const explicitEnd = item.endTime
  ...
  return {
    ...
    updates: item.incidentUpdates.map((update) => ({
      ...
      bodyText: toBoundedPlainText(
        update.descriptionEn ?? update.descriptionDe
      ),
      createdAt: requireIsoTimestamp(
        update.createdAt,
        sourceId,
        "incident_update.createdAt"
      ),
      updatedAt: requireIsoTimestamp(
        update.updatedAt,
        sourceId,
        "incident_update.updatedAt"
      ),
    })),
  }
}
...
function mapMaintenance(
  ...
  return {
    ...
    endsAt: item.endTime
      ? requireIsoTimestamp(item.endTime, sourceId, "maintenance.endTime")
      : null,
