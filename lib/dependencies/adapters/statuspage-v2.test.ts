...
const anthropicSource = manifest.sources.find((source) => source.id === "anthropic")!;
const anthropicApiPreset = manifest.presets.find((preset) => preset.id === "anthropic_api")!;
...
describe("statuspageV2Adapter.requests", () => {
  it("always requests current, and marks incidents and maintenance as optional", () => {
    ...
    expect(requests.find((request) => request.kind === "current")).toMatchObject({ url: anthropicSource.currentUrl, optional: false });
    expect(requests.find((request) => request.kind === "incidents")).toMatchObject({ optional: true });
    expect(requests.find((request) => request.kind === "maintenance")).toMatchObject({ optional: true });
  });
});

describe("statuspageV2Adapter.normalize: component status mapping", () => {
  it("maps operational", () => {
    const snapshot = statuspageV2Adapter.normalize({ source: anthropicSource, documents: [currentDoc(operational)], observedAt: "2026-07-19T15:00:00Z" });
    expect(snapshot.components["k8w3r06qmzrp"]).toMatchObject({ state: "OPERATIONAL" });
  });
  ...
  it("maps degraded_performance to DEGRADED", () => {
    ...
    expect(snapshot.components["k8w3r06qmzrp"]).toMatchObject({ state: "DEGRADED" });
  });

  it("maps major_outage to OUTAGE", () => {
    const snapshot = statuspageV2Adapter.normalize({ source: anthropicSource, documents: [currentDoc(outage)], observedAt: "2026-07-19T16:00:00Z" });
    expect(snapshot.components["k8w3r06qmzrp"]).toMatchObject({ state: "OUTAGE" });
    expect(snapshot.components["rwppv331jlwc"]).toMatchObject({ state: "OUTAGE" });
  });

  it("maps under_maintenance to MAINTENANCE", () => {
    const snapshot = statuspageV2Adapter.normalize({ source: anthropicSource, documents: [currentDoc(maintenance)], observedAt: "2026-07-19T02:30:00Z" });
    expect(snapshot.components["k8w3r06qmzrp"]).toMatchObject({ state: "MAINTENANCE" });
  });

  it("maps the scheduled-maintenance verifying status onto the monitoring incident state", () => {
    const snapshot = statuspageV2Adapter.normalize({ source: anthropicSource, documents: [currentDoc(maintenance)], observedAt: "2026-07-19T02:30:00Z" });
    const verifying = snapshot.maintenances.find((entry) => entry.externalId === "samplemaint02");
    expect(verifying?.state).toBe("monitoring");
    const inProgress = snapshot.maintenances.find((entry) => entry.externalId === "samplemaint01");
...
describe("statuspageV2Adapter.normalize: component selection against the catalog", () => {
  it("selects the anthropic_api preset's Claude API component by its real upstream id", () => {
    expect(anthropicApiPreset.selector).toMatchObject({ kind: "component_ids", ids: ["k8w3r06qmzrp"] });
    const snapshot = statuspageV2Adapter.normalize({ source: anthropicSource, documents: [currentDoc(outage)], observedAt: "2026-07-19T16:00:00Z" });
    const ids = anthropicApiPreset.selector.kind === "component_ids" ? anthropicApiPreset.selector.ids : [];
...
describe("statuspageV2Adapter.normalize: incidents", () => {
  ...
  it("strips HTML and caps update bodies", () => {
    ...
    const snapshot = statuspageV2Adapter.normalize({ source: anthropicSource, documents: [currentDoc(withHtml)], observedAt: "2026-07-19T15:10:00Z" });
    expect(snapshot.incidents[0].updates.find((update) => update.externalId === "sample1degraded01-u2")?.bodyText).toBe("A fix has been applied .");
  });

  it("normalizing the same fixture twice yields identical incident and update external ids", () => {
    const first = statuspageV2Adapter.normalize({ source: anthropicSource, documents: [currentDoc(operational), incidentsDoc(incidentsResolved)], observedAt: "2026-07-17T20:00:00Z" });
    const second = statuspageV2Adapter.normalize({ source: anthropicSource, documents: [currentDoc(operational), incidentsDoc(incidentsResolved)], observedAt: "2026-07-17T20:05:00Z" });
...
describe("statuspageV2Adapter.normalize: failure handling", () => {
  ...
  it("throws MISSING_DOCUMENT when the current document was never fetched", () => {
    try {
      statuspageV2Adapter.normalize({ source: anthropicSource, documents: [], observedAt: "2026-07-19T15:00:00Z" });
      expect.unreachable();
    } catch (error) {
