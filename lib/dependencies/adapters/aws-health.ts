...
function statusCodeToState(code: string, sourceId: string): ComponentState | null {
  switch (code) {
    case "0":
      return null;
    case "1":
    ...
    case "3":
      return "OUTAGE";
    default:
      throw new AdapterParseError("UNKNOWN_STATUS", `${sourceId}: unrecognized AWS status code "${code}"`);
  }
}
...
function epochSecondsToIso(value: number | string, sourceId: string, field: string): string {
  const seconds = typeof value === "string" ? Number(value) : value;
  if (!Number.isFinite(seconds)) {
    throw new AdapterParseError("SCHEMA_INVALID", `${sourceId}: invalid epoch seconds for ${field}: "${value}"`);
  }
...
function epochMillisToIso(value: number | string, sourceId: string, field: string): string {
  const millis = typeof value === "string" ? Number(value) : value;
  if (!Number.isFinite(millis)) {
    throw new AdapterParseError("SCHEMA_INVALID", `${sourceId}: invalid epoch millis for ${field}: "${value}"`);
  }
...
const eventSchema = z.object({
  ...
  impacted_services: z.record(z.string(), impactedServiceSchema).optional().default({}),
  impacted_service_status_changes: z.array(statusChangeSchema).optional().default([]),
});
...
function impactedServiceIds(event: AwsEvent, sourceId: string): string[] {
  ...
  for (const change of latestChangePerService(event).values()) {
    if (statusCodeToState(change.current_status, sourceId) === null) continue;
    ids.push(change.service);
  }
  return ids;
}

function mapEvent(event: AwsEvent, sourceId: string): NormalizedProviderSnapshot["incidents"][number] {
  const startedAt = epochSecondsToIso(event.date, sourceId, "event.date");
  const updates = event.event_log.map((entry) => {
    const at = epochSecondsToIso(entry.timestamp, sourceId, "event_log.timestamp");
    return {
  ...
  const updatedAt = latestTimestamp(updates.map((update) => update.updatedAt)) ?? startedAt;
  return {
...
export const awsHealthAdapter: DependencyAdapter = {
  ...
  normalize(input: NormalizeInput): NormalizedProviderSnapshot {
    ...
    if (!result.success) {
      throw new AdapterParseError("SCHEMA_INVALID", `${source.id}: currentevents failed schema validation: ${result.error.message}`);
    }
    ...
    for (const event of events) {
      for (const change of latestChangePerService(event).values()) {
        ...
        const updatedAt = epochMillisToIso(change.timestamp, source.id, "impacted_service_status_changes.timestamp");
