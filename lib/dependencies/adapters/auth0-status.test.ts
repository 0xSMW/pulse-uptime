...
const REGIONS = ["US-1", "US-3", "US-4", "US-5", "EU-1", "EU-2", "AU", "JP-1", "UK-1", "CA-1"];
...
function normalize(payload: unknown, observedAt = "2026-07-20T10:15:00.000Z") {
  return auth0StatusAdapter.normalize({ source, documents: [htmlWith(payload)], observedAt });
}
...
describe("auth0StatusAdapter.normalize: all operational", () => {
  it("maps every stable region to OPERATIONAL and emits no incidents", () => {
    ...
    for (const region of REGIONS) {
      expect(snapshot.components[region]).toMatchObject({ state: "OPERATIONAL" });
    }
  ...
  it("resolves a region-scoped preset install to its own region state", () => {
    ...
    expect(resolveDependencyState(preset.selector, "US-1", combined, snapshot)).toBe("OPERATIONAL");
    expect(resolveDependencyState(preset.selector, "EU-2", combined, snapshot)).toBe("OPERATIONAL");
  });
});
...
describe("auth0StatusAdapter.normalize: single region incident", () => {
  ...
  it("resolves the scoped install to OUTAGE for the affected region only", () => {
    const snapshot = normalize(singleIncident);
    const combined = combinedComponentStates(snapshot);
    expect(resolveDependencyState(preset.selector, "EU-1", combined, snapshot)).toBe("OUTAGE");
    expect(resolveDependencyState(preset.selector, "US-1", combined, snapshot)).toBe("OPERATIONAL");
  });
});
...
describe("auth0StatusAdapter.normalize: authenticationAffected surfacing", () => {
  it("aggregates one incident across every affected region and surfaces authenticationAffected in the update body", () => {
    ...
    // reports it affected.
    expect(incident.updates[0].bodyText).toBe("Authentication affected: yes. Impact: critical.");
  ...
  it("reports authentication not affected when no region flags it", () => {
    ...
    body.props.pageProps.activeIncidents.find((r: { region: string }) => r.region === "EU-1").response.incidents[0].authenticationAffected = false;
    const recomputed = normalize(body);
    expect(recomputed.incidents[0].updates[0].bodyText).toBe("Authentication affected: no. Impact: major.");
  });
});
...
describe("auth0StatusAdapter.normalize: maintenance", () => {
  it("routes a scheduled window to maintenances and marks the region MAINTENANCE", () => {
    ...
    const combined = combinedComponentStates(snapshot);
    expect(resolveDependencyState(preset.selector, "JP-1", combined, snapshot)).toBe("MAINTENANCE");
  });
});
...
describe("auth0StatusAdapter.normalize: transport and parse failures resolve to UNKNOWN, never OUTAGE", () => {
  it("throws AdapterParseError when the homepage has no __NEXT_DATA__ payload", () => {
    const doc: AdapterDocument = { kind: "current", url: source.currentUrl, text: "<html><body>maintenance mode</body></html>" };
    expect(() => auth0StatusAdapter.normalize({ source, documents: [doc], observedAt: "2026-07-20T10:00:00.000Z" })).toThrow(AdapterParseError);
  });
  ...
  it("throws SCHEMA_INVALID when the embedded payload is not valid JSON", () => {
    ...
    try {
      auth0StatusAdapter.normalize({ source, documents: [doc], observedAt: "2026-07-20T10:00:00.000Z" });
      expect.unreachable();
    } catch (error) {
  ...
  it("throws MISSING_DOCUMENT when no current text document was fetched", () => {
    expect(() => auth0StatusAdapter.normalize({ source, documents: [], observedAt: "2026-07-20T10:00:00.000Z" })).toThrow(AdapterParseError);
  });

  it("throws on an unrecognized incident status rather than mislabeling it", () => {
    const body = JSON.parse(JSON.stringify(singleIncident));
    body.props.pageProps.activeIncidents.find((r: { region: string }) => r.region === "EU-1").response.incidents[0].status = "smoldering";
    try {
      normalize(body);
      expect.unreachable();
    } catch (error) {
  ...
  it("resolves a region the successful feed no longer enumerates to UNKNOWN under a complete snapshot", () => {
    ...
    expect(resolveDependencyState(preset.selector, "ZZ-9", combined, snapshot)).toBe("UNKNOWN");
  });
});
