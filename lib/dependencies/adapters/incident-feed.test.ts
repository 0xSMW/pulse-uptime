...
import { incidentFeedAdapter, parseIncidentFeedUpdateMarker, requireIncidentInventory } from "./incident-feed";
...
function fixture(name: string): string {
  return readFileSync(new URL(`./fixtures/openrouter/${name}`, import.meta.url), "utf8");
}

function normalizeFixture(name: string) {
  const document: AdapterDocument = { kind: "current", url: source.currentUrl, text: fixture(name) };
  return incidentFeedAdapter.normalize({ source, documents: [document], observedAt: OBSERVED_AT });
}
...
describe("incidentFeedAdapter.requests", () => {
  it("asks for a single raw-text current document", () => {
    ...
    expect(requests).toEqual([
      { kind: "current", url: source.currentUrl, optional: false, mode: "text" },
    ]);
  });
});
...
describe("incidentFeedAdapter.catalogDirectory", () => {
  it("declares a complete directory that tracks no components", () => {
    ...
    const document: AdapterDocument = { kind: "current", url: source.currentUrl, text: fixture("empty.rss") };
    const directory = incidentFeedAdapter.catalogDirectory({ source, documents: [document] });
...
describe("incidentFeedAdapter.normalize", () => {
  ...
  it("surfaces an active incident as unresolved with no component identity", () => {
    ...
    const active = snapshot.incidents.find((incident) => incident.externalId === "status.openrouter.ai/incidents/aBc123XyZ")!;
    ...
    expect(active.canonicalUrl).toBe("https://status.openrouter.ai/incidents/aBc123XyZ");

    // The historical entry in the same feed is read as resolved from its own marker.
    const historical = snapshot.incidents.find((incident) => incident.externalId === "status.openrouter.ai/incidents/Old999Prev")!;
  ...
  it("marks a resolved incident and a completed maintenance as closed", () => {
    ...
    const incident = snapshot.incidents.find((entry) => entry.externalId === "status.openrouter.ai/incidents/Res456Done")!;
    expect(incident.state).toBe("resolved");
    expect(incident.resolvedAt).toBe("2026-07-06T05:30:00.000Z");

    const maintenance = snapshot.incidents.find((entry) => entry.externalId === "status.openrouter.ai/incidents/Mnt0DVmalJZ")!;
    ...
    expect(snapshot.incidents.every((entry) => entry.resolvedAt !== null)).toBe(true);
  });
  ...
  it("displays incident prose verbatim as bounded, markup-stripped update text", () => {
    ...
    expect(update.bodyText).toContain("IDENTIFIED - We have identified the cause");
...
<rss version="2.0"><channel>
  ...
  it("throws when the poller hands it no document", () => {
    expect(() => incidentFeedAdapter.normalize({ source, documents: [], observedAt: OBSERVED_AT }))
...
const azurePreset = manifest.presets.find((entry) => entry.id === "azure_platform")!;

function azureFixture(name: string): string {
  return readFileSync(new URL(`./fixtures/azure/${name}`, import.meta.url), "utf8");
}

function normalizeAzure(name: string) {
  const document: AdapterDocument = { kind: "current", url: azureSource.currentUrl, text: azureFixture(name) };
  return incidentFeedAdapter.normalize({ source: azureSource, documents: [document], observedAt: OBSERVED_AT });
}
...
describe("Azure catalog wiring", () => {
  it("routes Azure through the incident_feed adapter with incident_only fidelity", () => {
    ...
    expect(azureSource.currentUrl).toBe("https://azure.status.microsoft/en-us/status/feed/");
...
describe("incidentFeedAdapter.normalize on the Azure feed", () => {
  ...
  it("surfaces a synthetic Azure incident as active and displays its prose verbatim", () => {
    ...
    expect(incident.title).toBe("Azure Service Issue - Virtual Machines - West Europe");
    ...
    expect(incident.canonicalUrl).toBe("https://azure.status.microsoft/en-us/status");
    expect(incident.updates).toHaveLength(1);
    const [update] = incident.updates;
    expect(update.bodyText).toBe("Starting at 08:45 UTC on 20 Jul 2026, a subset of customers using Virtual Machines in West Europe may experience connection failures or high latency. Engineers are actively investigating the underlying cause.");
    expect(update.bodyText).not.toContain("<");
  });
  ...
  it("throws on a malformed Azure document so prior state is held, never an empty complete set", () => {
    ...
    expect(() => incidentFeedAdapter.normalize({
      source: azureSource,
      documents: [doc],
      observedAt: OBSERVED_AT,
    })).toThrow(AdapterParseError);
...
describe("requireIncidentInventory", () => {
  ...
  it("rejects a missing or unknown inventory value", () => {
    expect(() => requireIncidentInventory({ ...azureSource, config: {} })).toThrow(AdapterParseError);
    expect(() => requireIncidentInventory({
      ...azureSource,
...
describe("parseIncidentFeedUpdateMarker", () => {
  it("reads RESOLVED at the start of the description", () => {
    expect(parseIncidentFeedUpdateMarker("RESOLVED - All clear.")).toEqual({ state: "resolved", resolved: true });
  });

  it("reads IDENTIFIED immediately after a UTC timezone token", () => {
    expect(parseIncidentFeedUpdateMarker("Jul 15, 09:15 UTC IDENTIFIED - Cause found.")).toEqual({
      state: "identified",
      resolved: false,
    });
  });
  ...
  it("ignores PREIDENTIFIED, which is not an exact vocabulary token", () => {
    expect(parseIncidentFeedUpdateMarker("PREIDENTIFIED - not a real marker")).toEqual({
      state: "investigating",
      resolved: false,
    });
  });
  ...
  it("ignores a vocabulary word that only appears mid-body prose", () => {
    expect(parseIncidentFeedUpdateMarker(
  ...
  it("uses the first valid marker in provider order on multi-segment Statuspage prose", () => {
    const multi = "Jul 6, 06:45 UTC RESOLVED - Fixed. Jul 6, 05:30 UTC INVESTIGATING - Looking into it.";
    expect(parseIncidentFeedUpdateMarker(multi)).toEqual({ state: "resolved", resolved: true });
  });

  it("parses existing Statuspage RSS prose after markup stripping", () => {
    const statuspage = "Jul 15, 09:15 UTC IDENTIFIED - We have identified the cause and are working on a fix. Jul 15, 08:50 UTC INVESTIGATING - We are investigating elevated error rates on the routing API.";
    expect(parseIncidentFeedUpdateMarker(statuspage)).toEqual({ state: "identified", resolved: false });
  });

  it("falls back to active when description is empty", () => {
    expect(parseIncidentFeedUpdateMarker(null)).toEqual({ state: "investigating", resolved: false });
    expect(parseIncidentFeedUpdateMarker("")).toEqual({ state: "investigating", resolved: false });
  });
});
