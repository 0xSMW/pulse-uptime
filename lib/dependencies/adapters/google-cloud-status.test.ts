...
const googleSource = manifest.sources.find(
  (source) => source.id === "google_cloud"
)!
const geminiPreset = manifest.presets.find(
  (preset) => preset.id === "google_vertex_gemini"
)!
const cloudRunPreset = manifest.presets.find(
  (preset) => preset.id === "google_cloud_run"
)!
...
describe("googleCloudStatusAdapter.requests", () => {
  it("requests the single incidents.json document that serves both current state and history", () => {
    expect(googleCloudStatusAdapter.requests(googleSource)).toEqual([
      { kind: "current", url: googleSource.currentUrl, optional: false },
    ])
  })
})
...
describe("googleCloudStatusAdapter.normalize: status_impact mapping", () => {
  it("treats a product with no active matching incident as absent, callers read that as OPERATIONAL", () => {
    const snapshot = googleCloudStatusAdapter.normalize({
      source: googleSource,
      documents: [currentDoc(operational)],
      observedAt: "2026-07-19T12:00:00Z",
    })
    expect(
      geminiPreset.selector.kind === "google_product"
  ...
  it("maps SERVICE_OUTAGE to OUTAGE for the cloud run preset's product id", () => {
    expect(cloudRunPreset.selector).toMatchObject({
      kind: "google_product",
      productId: "9D7d2iNBQWN24zc1VamE",
    })
    const snapshot = googleCloudStatusAdapter.normalize({
      source: googleSource,
      documents: [currentDoc(outage)],
      observedAt: "2026-07-19T09:20:00Z",
    })
    expect(snapshot.components["9D7d2iNBQWN24zc1VamE"]).toMatchObject({
      state: "OUTAGE",
    })
  })
...
describe("googleCloudStatusAdapter.normalize: location selection", () => {
  it("carries a productId@locationId composite alongside the bare product id for optional location scoping", () => {
    ...
    expect(incident.scope.componentIds).toContain(
      "Z0FZJAMvEB4j3NbCJs6B@us-east4"
    )
  })
  ...
  it("excludes a recovered location from an active incident's composites so its scope is no longer touched", () => {
    ...
    expect(incident.scope.componentIds).toContain("Z0FZJAMvEB4j3NbCJs6B")
    expect(incident.scope.componentIds).toContain(
      "Z0FZJAMvEB4j3NbCJs6B@us-central1"
    )
    expect(incident.scope.componentIds).not.toContain(
      "Z0FZJAMvEB4j3NbCJs6B@us-east4"
    )
  })
...
describe("googleCloudStatusAdapter.normalize: incident and update idempotency", () => {
  it("normalizing the same fixture twice yields identical incident and update external ids", () => {
    const first = googleCloudStatusAdapter.normalize({
      source: googleSource,
      documents: [currentDoc(outage)],
      observedAt: "2026-07-19T09:20:00Z",
    })
    const second = googleCloudStatusAdapter.normalize({
      source: googleSource,
      documents: [currentDoc(outage)],
      observedAt: "2026-07-19T09:25:00Z",
    })
...
describe("googleCloudStatusAdapter.normalize: failure handling", () => {
  it("throws AdapterParseError when the document is not the documented top-level array", () => {
    expect(() =>
      googleCloudStatusAdapter.normalize({
        source: googleSource,
