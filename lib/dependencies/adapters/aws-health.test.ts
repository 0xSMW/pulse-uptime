...
const lambdaPreset = manifest.presets.find((preset) => preset.id === "aws_lambda")!;
const dynamoPreset = manifest.presets.find((preset) => preset.id === "aws_dynamodb")!;
...
describe("aws catalog source", () => {
  it("uses the aws_health adapter against the utf-16 public currentevents feed", () => {
    ...
    expect(source.currentUrl).toBe("https://health.aws.amazon.com/public/currentevents");
...
describe("awsHealthAdapter.requests", () => {
  it("requests the single currentevents document that serves both current state and incident prose", () => {
    expect(awsHealthAdapter.requests(source)).toEqual([{ kind: "current", url: source.currentUrl, optional: false }]);
  });
});
...
describe("awsHealthAdapter.normalize: empty feed means all operational", () => {
  it("returns no components and no incidents when the feed is an empty array", () => {
    const snapshot = awsHealthAdapter.normalize({ source, documents: [currentDoc(operational)], observedAt: "2026-07-20T12:00:00Z" });
...
describe("awsHealthAdapter.normalize: status code mapping", () => {
  const snapshot = awsHealthAdapter.normalize({ source, documents: [currentDoc(degraded)], observedAt: "2026-07-20T12:00:00Z" });

  it("maps status 2 to DEGRADED for the ec2 service id an aws_ec2 install scopes to", () => {
    const usEast1 = ec2Preset.scope?.kind === "required_options" ? ec2Preset.scope.options.find((option) => option.label === "us-east-1")!.id : "";
    expect(usEast1).toBe("ec2-us-east-1");
    expect(snapshot.components["ec2-us-east-1"]).toMatchObject({ state: "DEGRADED" });
  });
  ...
  it("maps informational status 1 to DEGRADED and keeps its prose in the incident updates", () => {
    expect(snapshot.components["lambda-us-east-1"]).toMatchObject({ state: "DEGRADED" });
    const bodies = snapshot.incidents[0].updates.map((update) => update.bodyText).join(" ");
  ...
  it("carries the ISO-converted change timestamp on the component, converting epoch millis", () => {
    expect(snapshot.components["ec2-us-east-1"].updatedAt).toBe("2026-03-03T01:06:40.000Z");
  });
});
...
describe("awsHealthAdapter.normalize: incident shape", () => {
  ...
  it("uses the event arn as the stable incident external id and the summary as the title", () => {
    expect(incident.externalId).toBe("arn:aws:health:us-east-1::event/MULTIPLE_SERVICES/AWS_MULTIPLE_SERVICES_OPERATIONAL_ISSUE/AWS_MULTIPLE_SERVICES_OPERATIONAL_ISSUE_SYNTHETIC_USE1");
  ...
  it("derives stable event_log update identities from their timestamps, idempotent across polls", () => {
    const again = awsHealthAdapter.normalize({ source, documents: [currentDoc(degraded)], observedAt: "2026-07-20T12:05:00Z" });
    expect(snapshot.incidents[0].updates.map((update) => update.externalId)).toEqual(again.incidents[0].updates.map((update) => update.externalId));
    expect(snapshot.incidents[0].updates.every((update) => update.state === "identified")).toBe(true);
  });
...
describe("awsHealthAdapter.normalize: latest-per-service reduction", () => {
  ...
  const recoveredWithinEvent = [
    {
      ...
      impacted_service_status_changes: [
        { service: "cloudfront", previous_status: "0", current_status: "2", timestamp: 1772500000000 },
        { service: "cloudfront", previous_status: "2", current_status: "0", timestamp: 1772501800000 },
        { service: "s3-us-east-1", previous_status: "0", current_status: "3", timestamp: 1772500000000 },
      ],
    },
  ];

  const snapshot = awsHealthAdapter.normalize({ source, documents: [currentDoc(recoveredWithinEvent)], observedAt: "2026-07-20T12:00:00Z" });
  ...
  it("keeps the worse of two concurrent events touching one service", () => {
    ...
    const twoEvents = [
      {
        date: "1772500000", arn: "arn:aws:health:global::event/A", status: "3", service: "svc-a", event_log: [],
        impacted_services: {},
        impacted_service_status_changes: [{ service: "cloudfront", previous_status: "0", current_status: "3", timestamp: 1772500000000 }],
      },
      {
        date: "1772500000", arn: "arn:aws:health:global::event/B", status: "1", service: "svc-b", event_log: [],
        impacted_services: {},
        impacted_service_status_changes: [
          { service: "cloudfront", previous_status: "2", current_status: "0", timestamp: 1772503600000 },
        ],
      },
    ];
    const both = awsHealthAdapter.normalize({ source, documents: [currentDoc(twoEvents)], observedAt: "2026-07-20T12:00:00Z" });
...
describe("awsHealthAdapter.normalize: live feed shape", () => {
  it("normalizes the sanitized live capture, mapping status 3 services to OUTAGE across both regions", () => {
    const snapshot = awsHealthAdapter.normalize({ source, documents: [currentDoc(live)], observedAt: "2026-07-20T12:00:00Z" });
    expect(snapshot.incidents).toHaveLength(2);
    expect(snapshot.components["ec2-me-central-1"]).toMatchObject({ state: "OUTAGE" });
    expect(snapshot.components["s3-me-south-1"]).toMatchObject({ state: "OUTAGE" });
...
describe("awsHealthAdapter.normalize: failure handling", () => {
  it("throws AdapterParseError when the document is not the documented top-level array", () => {
    expect(() => awsHealthAdapter.normalize({ source, documents: [currentDoc(malformed)], observedAt: "2026-07-20T12:00:00Z" })).toThrow(AdapterParseError);
  });

  it("throws AdapterParseError on an unrecognized status code rather than guessing a state", () => {
    expect(() => awsHealthAdapter.normalize({ source, documents: [currentDoc(unknownStatus)], observedAt: "2026-07-20T12:00:00Z" })).toThrow(AdapterParseError);
  });

  it("throws MISSING_DOCUMENT when the poller did not fetch the currentevents document", () => {
    expect(() => awsHealthAdapter.normalize({ source, documents: [], observedAt: "2026-07-20T12:00:00Z" })).toThrow(AdapterParseError);
  });
});
...
const AWS_FETCH_SOURCE = { id: "aws", allowedHosts: ["health.aws.amazon.com"], maxBodyBytes: 2 * 1024 * 1024 };
...
describe("aws_health fetch pipeline and truncation honesty", () => {
  it("decodes a big-endian UTF-16 body over the 512KB default but under the 2MB cap", async () => {
    ...
    const event = { ...(degraded as unknown[])[0] as Record<string, unknown>, summary: "x".repeat(400 * 1024) };
    ...
    const result = await fetchProviderDocument(AWS_FETCH_SOURCE, { url: AWS_URL }, { request, createDispatcher: () => fakeDispatcher() });
  ...
  it("aborts a body larger than the 2MB cap as TOO_LARGE, so the source reads UNKNOWN not a false state", async () => {
    ...
    await expect(
      fetchProviderDocument(AWS_FETCH_SOURCE, { url: AWS_URL }, { request, createDispatcher: () => fakeDispatcher() }),
