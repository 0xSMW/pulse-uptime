...
const apiPreset = manifest.presets.find(
  (preset) => preset.id === "digitalocean_api"
)!
const dropletsPreset = manifest.presets.find(
  (preset) => preset.id === "digitalocean_droplets"
)!
const databasesPreset = manifest.presets.find(
  (preset) => preset.id === "digitalocean_managed_databases"
)!
...
describe("DigitalOcean preset component selection against the live feed", () => {
  const snapshot = statuspageV2Adapter.normalize({
    ...
    observedAt: "2026-07-19T15:32:39Z",
  })

  it("resolves every selector id present in the sanitized live summary", () => {
    const selectorIds = [apiPreset, dropletsPreset, databasesPreset].flatMap(
      (preset) =>
        preset.selector.kind === "component_ids" ? preset.selector.ids : []
    )
  ...
  it("selects the API component and reports it operational", () => {
    expect(apiPreset.selector).toMatchObject({
      kind: "component_ids",
      ids: ["p1x9rv4137gx"],
    })
  ...
  it("scopes Droplets and Managed Databases to their regional group aggregates", () => {
    expect(dropletsPreset.scope).toMatchObject({
      kind: "discovered_children",
      groupId: "4rgs7bbljl8d",
      required: true,
    })
    expect(databasesPreset.scope).toMatchObject({
      kind: "discovered_children",
      groupId: "kl8qv98c9brp",
      required: true,
    })
    // The selector aggregate id is the parent group's own rollup component,
    // mirroring the Supabase and Upstash regional presets.
    if (dropletsPreset.selector.kind === "component_ids") {
      expect(dropletsPreset.selector.ids).toEqual([
        dropletsPreset.scope?.kind === "discovered_children"
          ? dropletsPreset.scope.groupId
          : "",
      ])
    }
    if (databasesPreset.selector.kind === "component_ids") {
      expect(databasesPreset.selector.ids).toEqual([
        databasesPreset.scope?.kind === "discovered_children"
          ? databasesPreset.scope.groupId
          : "",
      ])
    }
  ...
  it("builds a complete catalog directory with childrenByParent for Droplets and Managed Databases", () => {
    ...
    const databaseChildren =
      directory.childrenByParent.get("kl8qv98c9brp") ?? []
    expect(dropletChildren.map((child) => child.id)).toContain("kkg2cfkqkwj1")
    expect(
      dropletChildren.find((child) => child.id === "kkg2cfkqkwj1")?.label
    ).toBe("FRA1")
  ...
  it("maps a degraded group rollup onto DEGRADED", () => {
    ...
    const droplets = degraded.components.find(
      (component: { id: string }) => component.id === "4rgs7bbljl8d"
    )
    droplets.status = "partial_outage"
    const snap = statuspageV2Adapter.normalize({
      source,
