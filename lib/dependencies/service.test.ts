...
function preset(
  overrides: Partial<DependencyPresetRow> = {}
): DependencyPresetRow {
  return {
    id: "vercel_runtime",
    sourceId: "vercel",
...
describe("addDependency validation matrix", () => {
  it("rejects an unknown preset with PRESET_NOT_FOUND", async () => {
    ...
    await expect(
      addDependency({ presetId: "nope" }, { store, now: () => NOW })
    ).rejects.toMatchObject({ code: "PRESET_NOT_FOUND" })
  })

  it("rejects a disabled preset with PRESET_UNAVAILABLE", async () => {
    const store = fakeStore({
      loadPreset: vi.fn().mockResolvedValue(preset({ enabled: false })),
    })
    await expect(
      addDependency({ presetId: "vercel_runtime" }, { store, now: () => NOW })
  ...
  it("accepts a preset that has never passed catalog validation (validatedAt and validationError both null)", async () => {
    const store = fakeStore({
      loadPreset: vi
        .fn()
        .mockResolvedValue(
          preset({ validatedAt: null, validationError: null })
        ),
    })
    await addDependency(
      { presetId: "vercel_runtime" },
      { store, now: () => NOW, newId: () => "id" }
    )
  ...
  it("rejects a preset with a recorded validation error with PRESET_UNAVAILABLE, even when enabled", async () => {
    const store = fakeStore({
      loadPreset: vi.fn().mockResolvedValue(
        preset({
          enabled: true,
          validationError: "Missing upstream component ids: renamed-id",
        })
      ),
    })
    await expect(
  ...
  it("rejects a scopeId for a preset with no scope contract", async () => {
    const store = fakeStore({
      loadPreset: vi.fn().mockResolvedValue(preset({ scope: null })),
    })
    await expect(
      addDependency(
        { presetId: "vercel_runtime", scopeId: "us-east-1" },
        { store, now: () => NOW }
      )
  ...
  it("requires a scopeId for a required_options preset", async () => {
    ...
    await expect(
      addDependency({ presetId: "neon_database" }, { store, now: () => NOW })
  ...
  it("rejects a scopeId outside the required_options catalog list", async () => {
    ...
    await expect(
      addDependency(
        { presetId: "neon_database", scopeId: "eu-west-2" },
        { store, now: () => NOW }
      )
    ).rejects.toMatchObject({ code: "INVALID_SCOPE" })
  })
  ...
  it("accepts a scopeId that matches a required_options entry", async () => {
    ...
    await addDependency(
      { presetId: "neon_database", scopeId: "us-east-1" },
      { store, now: () => NOW, newId: () => "id" }
    )
  ...
  it("rejects an arbitrary scopeId that is not in the discovered option set", async () => {
    ...
    await expect(
      addDependency(
        { presetId: "supabase_database", scopeId: "made-up" },
        { store, now: () => NOW }
      )
    ).rejects.toMatchObject({ code: "INVALID_SCOPE" })
  })
  ...
  it("rejects a discovered scopeId when options have not been materialised yet", async () => {
    ...
    await expect(
      addDependency(
        { presetId: "supabase_database", scopeId: "us-region" },
        { store, now: () => NOW }
      )
...
describe("addDependency ten-minute snapshot rule", () => {
  ...
  it("reuses a fresh (< 10 minutes old) prior observation instead of UNKNOWN", async () => {
    ...
    expect(store.loadRecentStateForCatalogScope).toHaveBeenCalledWith(
      "vercel_runtime",
      null,
...
describe("addDependency duplicates and defaults", () => {
  ...
  it("defaults notificationsEnabled to true and honors an explicit false", async () => {
    ...
    await addDependency(
      { presetId: "vercel_runtime", notificationsEnabled: false },
      { store, now: () => NOW, newId: () => "id" }
    )
    expect(store.insertDependency).toHaveBeenCalledWith(
      expect.objectContaining({
  ...
  it("pins the dependency's own id to a supplied dependencyId (idempotency crash recovery)", async () => {
    ...
    await addDependency(
      { presetId: "vercel_runtime" },
      { store, now: () => NOW, dependencyId: "op-123" }
    )
    expect(store.insertDependency).toHaveBeenCalledWith(
      expect.objectContaining({
...
describe("read wrappers", () => {
  it("listDependencies delegates to the dashboard query", async () => {
    vi.mocked(queries.listDependenciesForDashboard).mockResolvedValue([
      { id: "dep-1" },
    ] as never)
  ...
  it("listCatalog delegates to the catalog query", async () => {
    vi.mocked(queries.listCatalog).mockResolvedValue([
      { category: "hosting", presets: [] },
    ] as never)
...
describe("patchDependency", () => {
  it("rejects unknown fields (notificationsEnabled only)", async () => {
    const store = fakeStore()
    await expect(
      patchDependency("dep-1", { url: "https://evil.example" }, { store })
  ...
  it("throws DEPENDENCY_NOT_FOUND when no active dependency matches", async () => {
    const store = fakeStore({
      patchNotifications: vi.fn().mockResolvedValue(false),
    })
    await expect(
      patchDependency("dep-1", { notificationsEnabled: false }, { store })
    ).rejects.toMatchObject({ code: "DEPENDENCY_NOT_FOUND" })
  })

  it("patches and returns the refreshed detail", async () => {
    const store = fakeStore({
      patchNotifications: vi.fn().mockResolvedValue(true),
    })
    await expect(
      patchDependency("dep-1", { notificationsEnabled: false }, { store })
...
describe("removeDependency (soft remove)", () => {
  it("throws DEPENDENCY_NOT_FOUND when no active dependency matches", async () => {
    const store = fakeStore({
      removeDependency: vi.fn().mockResolvedValue(false),
    })
    await expect(
      removeDependency("dep-1", { store, now: () => NOW })
    ).rejects.toMatchObject({ code: "DEPENDENCY_NOT_FOUND" })
  })
  ...
  it("delegates the close-interval semantics to the store and reports removed", async () => {
    ...
    await expect(
      removeDependency("dep-1", { store, now: () => NOW })
    ).resolves.toEqual({ id: "dep-1", removed: true })
...
describe("scheduleDependencyPoll", () => {
  it("throws DEPENDENCY_NOT_FOUND when the dependency has no source (removed or missing)", async () => {
    const store = fakeStore({
      loadSourceIdForDependency: vi.fn().mockResolvedValue(null),
    })
    await expect(
...
describe("databaseDependenciesStore validator clearing (FIX D)", () => {
  it("clears the source's etag and last_modified in the same transaction that installs a dependency", async () => {
    ...
    const sourceUpdate = setCalls.find(
      (call) => call.table === dependencySources
    )
    expect(sourceUpdate?.patch).toMatchObject({
      nextPollAt: NOW,
      etag: null,
      lastModified: null,
    })
...
describe("databaseDependenciesStore insertDependency race handling", () => {
  ...
  it("throws DependencyInstallConflictError when a unique violation lands on the caller's own transaction handle", async () => {
    ...
    await expect(
      databaseDependenciesStore.insertDependency({
        dependency: {
          id: "dep-1",
          catalogId: "vercel_runtime",
          scopeId: null,
  ...
  it("maps a unique violation to false when no handle is given, wrapping runInsert in its own transaction", async () => {
    vi.mocked(db.transaction).mockImplementation((async (
      work: (tx: unknown) => Promise<unknown>
    ) => work(txMock({ code: "23505" }))) as never)
...
describe("databaseDependenciesStore removeDependency (FIX D1/D2)", () => {
      ...
      update: (table: unknown) => ({
        set: (patch: Record<string, unknown>) => {
          ...
          const where = () => {
            ...
            result.returning = () =>
              Promise.resolve(matched ? [{ id: "dep-1" }] : [])
            return result
          }
          return { where }
        },
  ...
  it("closes the open interval with greatest(now, started_at), not a bare now, so a slightly-behind now never aborts the transaction", async () => {
    ...
    const removed = await databaseDependenciesStore.removeDependency(
      "dep-1",
      NOW,
      tx as unknown as typeof db
    )
    expect(removed).toBe(true)
    const intervalUpdate = setCalls.find(
      (call) => call.table === dependencyStateIntervals
    )
  ...
  it("returns false and closes no interval when no active dependency matches", async () => {
    ...
    expect(
      setCalls.find((call) => call.table === dependencyStateIntervals)
    ).toBeUndefined()
  })
    ...
    const handle = {
      update: (table: unknown) => ({
        set: (patch: Record<string, unknown>) => {
          ...
          return {
            where: () => ({
              returning: vi.fn().mockResolvedValue([{ id: "dep-1" }]),
            }),
          }
        },
      }),
    }
    const patched = await databaseDependenciesStore.patchNotifications(
      "dep-1",
      false,
      handle as unknown as typeof db
    )
    ...
    vi.mocked(db.update).mockImplementation(((table: unknown) => ({
      set: () => ({
        where: () => ({
          returning: vi
            .fn()
            .mockResolvedValue(table === dependencies ? [{ id: "dep-1" }] : []),
        }),
      }),
    })) as never)
    const patched = await databaseDependenciesStore.patchNotifications(
...
describe("DependencyApiError", () => {
  it("carries a stable name and optional details", () => {
    const error = new DependencyApiError("SCOPE_REQUIRED", "scope required", {
      presetId: "neon_database",
    })
