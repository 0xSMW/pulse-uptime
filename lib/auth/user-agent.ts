...
function parseOs(userAgent: string): string {
  ...
  if (
    /\b(?:Macintosh|Mac OS X)\b/.test(userAgent) ||
    /\bdarwin\b/i.test(userAgent)
  ) {
    return "macOS"
  }
...
export function parseUserAgent(
  ...
  if (!value) {
    return { browser: UNKNOWN_BROWSER, os: UNKNOWN_OS }
  }
