  ...
  it("rejects the account when core readiness is blocked", async () => {
    ...
    await expect(
      createOnlyAdmin(
        ...
        {
          store: fake.store,
          checkReadiness: async () => readiness({ canContinue: false }),
          verifyBootstrap: allowBootstrap,
        }
      )
  ...
  const enforce: NonNullable<LoginDependencies["enforceLimit"]> = async (
    ...
    return {
      ...
      retryAfterSeconds: Math.max(
        1,
        Math.ceil(
          ((windowStart + 1) * policy.windowSeconds * 1000 - now.getTime()) /
            1000
        )
      ),
    }
...
describe("persistent login rate limiting", () => {
  it("uses digest-only email and IP bucket keys", () => {
    const emailKey = loginRateLimitKey(
      "email",
      "admin@example.com",
      deterministicDigest
    )
  ...
  it("starts a fresh distributed bucket after the fixed window", async () => {
    ...
    for (let attempt = 0; attempt < 6; attempt += 1) {
      await login(
        { email: "admin@example.com", password: "wrong", ip: "203.0.113.7" },
        dependencies
      ).catch(() => undefined)
    }
...
describe("lastSeenAt refresh throttle", () => {
  ...
  it("skips the write inside the 60-second window", () => {
    expect(shouldRefreshLastSeen(new Date("2026-07-18T11:59:30Z"), now)).toBe(
      false
    )
    expect(shouldRefreshLastSeen(new Date("2026-07-18T11:59:00Z"), now)).toBe(
      false
    )
  })
  ...
  it("refreshes once the last touch is older than 60 seconds", () => {
    expect(
      shouldRefreshLastSeen(new Date("2026-07-18T11:58:59.999Z"), now)
    ).toBe(true)
  })
})
