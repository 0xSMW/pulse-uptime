...
describe("parseReleaseId", () => {
  it("accepts a nonempty bounded id", () => {
    expect(parseReleaseId("dpl_abc123")).toEqual({ ok: true, releaseId: "dpl_abc123" });
  });

  it("trims surrounding whitespace", () => {
    expect(parseReleaseId("  dpl_trim  ")).toEqual({ ok: true, releaseId: "dpl_trim" });
  });
...
describe("resolveReleaseIdFromEnv", () => {
  it("prefers explicit PULSE_RELEASE_ID over VERCEL_DEPLOYMENT_ID", () => {
    expect(resolveReleaseIdFromEnv({
  ...
  it("reports missing when neither is set", () => {
    expect(resolveReleaseIdFromEnv({})).toEqual({ ok: false, reason: "missing" });
  });
});
...
describe("getPulseReleaseId / requirePulseReleaseId", () => {
  it("returns null when invalid and throws from require", () => {
    ...
    expect(() => requirePulseReleaseId({})).toThrow(/PULSE_RELEASE_ID is missing/);
  });
  ...
  it("fails closed for missing production release id", () => {
    ...
    expect(() => requirePulseReleaseId(env)).toThrow(/PULSE_RELEASE_ID is missing/);
  });
});
