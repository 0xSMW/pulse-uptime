...
export const CHECK_COMPLETION_BUFFER_MS = 8_000;
export const MAX_ACTIVE_MONITORS = 100;

export type MonitorRunOutcome = "success" | "failure" | "duplicate";

export type MonitorRunner = (monitor: MonitorConfig, scheduledAt: Date) => Promise<MonitorRunOutcome>;
...
export async function dispatchDueMonitors(options: {
  ...
  if (active.length > MAX_ACTIVE_MONITORS) throw new Error("Active monitor limit exceeded");
  ...
  const workers = Array.from({ length: Math.min(options.concurrency, due.length) }, async () => {
    while (cursor < due.length) {
      ...
      try {
        ...
        else if (outcome === "failure") counts.failureCount += 1;
