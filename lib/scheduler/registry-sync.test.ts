...
vi.mock("drizzle-orm", async (importOriginal) => {
  ...
  return {
    ...
    inArray: (column: unknown, values: unknown[]) => ({
      __op: "inArray" as const,
      column,
      values,
    }),
    notInArray: (column: unknown, values: unknown[]) => ({
      __op: "notInArray" as const,
      column,
      values,
    }),
...
function createFakeTx(seed: {
  ...
  const stateRows = new Map(
    seed.state.map((row) => [row.monitorId, { ...row }])
  )
  const incidentRows = new Map(
    seed.incidents.map((row) => [row.id, { ...row }])
  )
            ...
            return {
              where(cond: Cond) {
                ...
                return Promise.resolve(
                  ids
                    .map((id) => registryRows.get(id))
                    .filter((row): row is RegistryRow => !!row)
                )
              },
            }
          }
            ...
            onConflictDoNothing() {
              if (table === monitorState) {
                ...
                for (const row of list as {
                  monitorId: string
                  state: MonitorStateSnapshot["state"]
                  updatedAt: Date
                }[]) {
                  if (stateRows.has(row.monitorId)) {
                    continue
                  }
          ...
          return {
            where(cond: Cond) {
              ...
              if (table === incidents) {
                ...
                promise.returning = () =>
                  Promise.resolve(affected ? [{ id: incidentId }] : [])
                return promise
              }
              if (table === monitorState) {
...
function monitorConfig(
  id: string,
  overrides: Partial<MonitoringConfig["monitors"][number]> = {}
): MonitoringConfig["monitors"][number] {
  return {
    id,
    name: id,
    url: `https://${id}.example.com`,
...
function buildConfig(
  ...
  groups: MonitoringConfig["groups"] = []
): MonitoringConfig {
  return {
    schemaVersion: 2,
    configVersion: 1,
...
describe("synchronizeRegistry - statement bounds", () => {
  ...
  it("scales only the per-changed-monitor updates and a single batched exception insert, not the fixed statements", async () => {
    ...
    const { tx, statementLog, exceptionRows } = createFakeTx({
      registry: ids.map((id) =>
        registryRow(id, { configHash: oldHash, enabled: true })
      ),
...
describe("synchronizeRegistry - semantic equivalence", () => {
  it("creates a new monitor without exceptions and leaves lifecycle untouched", async () => {
    ...
    const { tx, registryRows, stateRows, exceptionRows, statementLog } =
      createFakeTx({ registry: [], state: [], incidents: [] })

    await synchronizeRegistry(tx, config, "hash-1", now, RUNTIME_MODE)

    expect(registryRows.get("fresh")).toMatchObject({
      firstSeenAt: now,
      lastSeenAt: now,
      archivedAt: null,
      enabled: true,
    })
    expect(stateRows.get("fresh")).toMatchObject({
      state: "PENDING",
      version: 0,
    })
  ...
  it("preserves firstSeenAt and emits a pause exception when an existing monitor is disabled", async () => {
    ...
    expect(registryRows.get("site")).toMatchObject({
      enabled: false,
      firstSeenAt: new Date("2020-01-01T00:00:00Z"),
      lastSeenAt: now,
    })
  ...
  it("emits a configuration exception (not resume/pause) when only the config hash changes", async () => {
    ...
    const { tx, exceptionRows } = createFakeTx({
      registry: [
        registryRow("site", { configHash: "hash-old", enabled: true }),
      ],
  ...
  it("archives a removed monitor and emits pause-then-configuration exceptions with MONITOR_ARCHIVED", async () => {
    ...
    expect(registryRows.get("gone")).toMatchObject({
      enabled: false,
      archivedAt: now,
    })
    expect(stateRows.get("gone")).toMatchObject({ state: "ARCHIVED" })
    expect(exceptionRows.map((row) => row.eventType)).toEqual([
      "pause",
      "configuration",
    ])
  ...
  it("restores a previously archived monitor and emits a resume exception", async () => {
    ...
    expect(registryRows.get("back")).toMatchObject({
      enabled: true,
      archivedAt: null,
    })
    expect(stateRows.get("back")).toMatchObject({
      state: "PENDING",
      version: 5,
    })
    expect(exceptionRows).toHaveLength(1)
  ...
  it("resolves the active incident when a down monitor is paused, in runtime mode", async () => {
    ...
    const { tx, incidentRows } = createFakeTx({
      ...
      state: [
        stateRow("site", {
          ...
          firstFailureAt: new Date("2026-07-18T00:00:00Z"),
        }),
      ],
      incidents: [
        {
    ...
    expect(incidentRows.get("inc-1")).toMatchObject({
      resolvedAt: now,
      resolutionReason: "monitor_paused",
    })
  })
  ...
  it("throws in runtime mode when the active incident row is missing, but not in API mode", async () => {
    ...
    const seed = () => ({
      ...
      state: [
        stateRow("site", {
          state: "DOWN",
          activeIncidentId: "missing-incident",
          firstFailureAt: new Date("2026-07-18T00:00:00Z"),
        }),
      ],
      incidents: [],
    ...
    await expect(
      synchronizeRegistry(runtimeFake.tx, config, hash, now, RUNTIME_MODE)
    ).rejects.toThrow("Active incident not found: missing-incident")

    const apiFake = createFakeTx(seed())
    await expect(
      synchronizeRegistry(apiFake.tx, config, hash, now, API_MODE)
