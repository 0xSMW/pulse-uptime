...
import { createMonitorWithDefaults, DEFAULT_MONITOR_SETTINGS } from "@/lib/config/defaults";
...
describe("configuration source fallback", () => {
  ...
  it.each(["read failure", "missing value"])("uses the accepted snapshot after %s", async (scenario) => {
    const loaded = await evaluateConfigurationSource({
      readDesired: scenario === "read failure"
    ...
    if (loaded.result.status === "rejected") expect(loaded.result.config).toEqual(previous.config);
  });

  it("fails when neither Edge Config nor an accepted snapshot exists", async () => {
    const loaded = await evaluateConfigurationSource({
      readDesired: async () => { throw new Error("edge unavailable"); },
      previous: null,
      now: new Date(),
    });
  ...
  it("falls back if conditional approval consumption loses its race", async () => {
    ...
    if (approved.status === "accepted") expect(approved.approvalConsumed).toBe(true);
