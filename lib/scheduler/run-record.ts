...
const MESSAGE_FIELD_CAP = 4_096;
const STACK_FIELD_CAP = 8_192;
const SMALL_FIELD_CAP = 1_024;
const MAX_CAUSE_DEPTH = 4;

function truncatedString(value: string, cap: number, mark: { truncated: boolean }): string {
  if (value.length <= cap) return value;
  mark.truncated = true;
  return `${value.slice(0, cap)} [truncated]`;
}

function readField(source: Record<string, unknown>, keys: readonly string[]): string | undefined {
...
function buildCapture(error: unknown, depth: number, mark: { truncated: boolean }): CronErrorCapture {
  if (error instanceof Error) {
    const source = error as unknown as Record<string, unknown>;
    const capture: CronErrorCapture = {
      message: truncatedString(error.message || error.name || "Error", MESSAGE_FIELD_CAP, mark),
    ...
    if (code) capture.code = truncatedString(code, SMALL_FIELD_CAP, mark);
    if (detail) capture.detail = truncatedString(detail, MESSAGE_FIELD_CAP, mark);
    if (hint) capture.hint = truncatedString(hint, SMALL_FIELD_CAP, mark);
    if (severity) capture.severity = truncatedString(severity, SMALL_FIELD_CAP, mark);
    if (constraint) capture.constraint = truncatedString(constraint, SMALL_FIELD_CAP, mark);
    if (table) capture.table = truncatedString(table, SMALL_FIELD_CAP, mark);
    if (column) capture.column = truncatedString(column, SMALL_FIELD_CAP, mark);
    if (schema) capture.schema = truncatedString(schema, SMALL_FIELD_CAP, mark);
    if (routine) capture.routine = truncatedString(routine, SMALL_FIELD_CAP, mark);
  ...
  return {
    message: truncatedString(typeof error === "string" ? error : String(error), MESSAGE_FIELD_CAP, mark),
    name: "NonError",
  };
}
...
function boundBySize(capture: CronErrorCapture, cap: number): CronErrorCapture {
  if (encodedBytes(capture) <= cap) return capture;
  for (let node: CronErrorCapture | undefined = capture; node; node = node.cause) {
    if (node.stack) {
      delete node.stack;
...
export function captureCronError(error: unknown): CronErrorCapture {
  ...
  if (mark.truncated) capture.truncated = true;
