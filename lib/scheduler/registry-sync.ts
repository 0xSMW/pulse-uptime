...
import { type LifecycleTarget, transitionLifecycle } from "./lifecycle"
...
export async function synchronizeRegistry(
  ...
  const previousRegistryById = new Map<
    string,
    { configHash: string; enabled: boolean }
  >()
  if (trackExceptions && desiredIds.length > 0) {
    const previous = await tx
      ...
      .where(inArray(monitorRegistry.id, desiredIds))
    for (const row of previous) {
      previousRegistryById.set(row.id, row)
    }
  }
  ...
  const removed = await tx
    ...
    .where(
      and(
        ...
        notInArray(monitorRegistry.id, desiredIds)
      )
    )
  ...
  if (allIds.length > 0) {
    ...
    for (const state of states) {
      statesByMonitorId.set(state.monitorId, state)
    }
  }
    ...
    if (mutation.resolution) {
      ...
      if (assertIncidentResolution) {
        ...
        if (resolved.length !== 1) {
          throw new Error(
            `Active incident not found: ${mutation.resolution.incidentId}`
          )
        }
      } else {
        await resolution
  ...
  for (const monitor of config.monitors) {
    ...
    if (previous && previous.configHash !== hash) {
      addException(monitor.id, "configuration", null)
    }
