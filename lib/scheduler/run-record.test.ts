...
describe("captureCronError", () => {
  it("captures the message, name, and Postgres diagnostic fields", () => {
    const capture = captureCronError(pgError("duplicate key value violates unique constraint", {
      code: "23505",
      detail: "Key (job_name, scheduled_minute)=(monitor-check, 2026-07-20 00:00:00) already exists.",
  ...
  it("walks the wrapped cause chain", () => {
    const root = pgError("connection terminated unexpectedly", { code: "57P01" });
  ...
  it("marks truncation and stays within the byte cap for a huge error", () => {
    ...
    expect(Buffer.byteLength(JSON.stringify(capture))).toBeLessThanOrEqual(CRON_ERROR_CAPTURE_BYTES);
  ...
  it("bounds a deep cause chain and marks it truncated", () => {
    ...
    for (let level = 0; level < 12; level += 1) {
      error = new Error(`wrap ${level} ${"y".repeat(2_048)}`, { cause: error });
    }
...
describe("toCronRunFailure", () => {
  it("pairs a single-line message with the structured capture", () => {
    const failure = toCronRunFailure(pgError("boom\nsecond line", { code: "XX000" }));
