...
describe("help registry", () => {
  ...
  it("groups concepts and guides separately", () => {
    expect(helpGroups.map((group) => group.label)).toEqual([
      "Core Concepts",
      "How-to Guides",
    ])
    expect(
      helpGroups[0]!.entries.every((entry) => entry.kind === "concept")
    ).toBe(true)
    expect(
      helpGroups[1]!.entries.every((entry) => entry.kind === "guide")
    ).toBe(true)
  })
  ...
  it("keeps every summary to one paragraph of at most 70 words", () => {
    for (const entry of helpEntries) {
      ...
      expect(
        words,
        `${helpEntryId(entry)} has ${words} words`
  ...
  it("maps every entry to an existing demo", () => {
    for (const entry of helpEntries) {
      expect(
        helpDemos[entry.demo],
        `${helpEntryId(entry)} → ${entry.demo}`
      ).toBeDefined()
    }
  })
    ...
    for (const entry of helpEntries) {
      ...
      for (const link of entry.relatedLinks) {
        if (link.href.startsWith("#")) {
          expect(
            ids.has(link.href.slice(1)),
            `${helpEntryId(entry)} → ${link.href}`
          ).toBe(true)
        } else {
          expect(
            productRoutes,
  ...
  it("avoids generic copy in titles, summaries, and link labels", () => {
    for (const entry of helpEntries) {
      ...
      for (const phrase of genericCopy) {
        expect(
          text,
          `${helpEntryId(entry)} contains "${phrase}"`
