  ...
  const db: SqlExecutor = {
    async query<T>(
      ...
      if (
        ...
        text === CLAIM_NOTIFICATIONS_BY_EVENT_TYPE_SQL
      ) {
        ...
        const due = [...rows.values()]
          .filter((row) => {
            ...
            if (row.nextAttemptAt.getTime() > now.getTime()) {
              return false
            }
            if (eventTypes && !eventTypes.has(row.eventType)) {
              return false
            }
            return true
          })
      ...
      if (
        ...
        for (const row of rows.values()) {
          ...
          if (row.claimedAt.getTime() >= cutoff.getTime()) {
            continue
          }
          if (eventTypes && !eventTypes.has(row.eventType)) {
            continue
          ...
          const status: "failed" | "dead" =
            row.claimedAt.getTime() <= safeRetryCutoff.getTime()
              ? "dead"
              : "failed"
          rows.set(row.id, {
            ...row,
            ...
            claimedAt: null,
            lastError:
              status === "dead" ? "AMBIGUOUS_PROVIDER_RESULT" : "STALE_CLAIM",
            updatedAt: now,
          })
...
describe("system.alert durability", () => {
  ...
  it("marks a Resend failure failed with the retry ladder nextAttemptAt", async () => {
    const store = createMemoryOutbox([
      systemRow({ nextAttemptAt: hour, createdAt: hour }),
    ])
  ...
  it("later sweep same hour: enqueue inserts 0 and the failed row is claimed and retried", async () => {
    ...
    const sweepNow = new Date(retryReady.getTime() + 1000)
    ...
    const { db: enqueueDb, inserted } = fakeEnqueueDb(
      new Set([existing.idempotencyKey])
    )
  ...
  it("process dies with system.alert in sending: later sweep reconciles and retries", async () => {
    ...
    const deadCount = await reconcileStaleClaims(
      store.db,
      ancient,
      5 * 60_000,
      {
  ...
  it("repeated unhealthy sweeps in one hour yield one outbox row per normalized recipient", async () => {
    ...
    const first = await enqueueSystemAlert(
      db,
      {
        kind: "monitoring-loop-failure",
        title: "title",
