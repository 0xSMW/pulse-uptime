...
describe("outbox SQL", () => {
  ...
  it("scopes claim and reconcile by event_type when requested", async () => {
    expect(CLAIM_NOTIFICATIONS_BY_EVENT_TYPE_SQL).toMatch(/event_type = any\(\$4\)/i);
    expect(CLAIM_NOTIFICATIONS_BY_EVENT_TYPE_SQL).toMatch(/for update skip locked/i);
    expect(RECONCILE_STALE_CLAIMS_BY_EVENT_TYPE_SQL).toMatch(/event_type = any\(\$4\)/i);
    ...
    await expect(reconcileStaleClaims(db, now, 5 * 60_000, { eventTypes: ["system.alert"] }))
      .resolves.toBe(1);
    expect(query).toHaveBeenCalledWith(RECONCILE_STALE_CLAIMS_BY_EVENT_TYPE_SQL, [
      now,
