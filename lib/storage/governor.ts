...
export function governorMode(projectedBytes: bigint, budgetBytes = BigInt(STORAGE_BUDGET_BYTES)): GovernorMode {
  ...
  if (basisPoints < BigInt(GOVERNOR_THRESHOLD_PERCENTS.compactEarly) * 100n) return "full";
  if (basisPoints < BigInt(GOVERNOR_THRESHOLD_PERCENTS.shortened) * 100n) return "compact_early";
  if (basisPoints < BigInt(GOVERNOR_THRESHOLD_PERCENTS.incidentOnly) * 100n) return "shortened";
  if (projectedBytes * 100n <= budgetBytes * BigInt(GOVERNOR_THRESHOLD_PERCENTS.essential)) return "incident_only";
  return "essential";
}
