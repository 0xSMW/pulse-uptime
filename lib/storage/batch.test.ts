...
import { WRITE_PACKED_MINUTE_SQL, writePackedMinute } from "./batch"
...
describe("packed minute writer", () => {
  it("uses one no-returning request and records incomplete checks as gaps", async () => {
    ...
    await writePackedMinute(
      { query },
      {
        ...
        results: [
          { monitorId: "b", completed: true, failed: false, latencyMs: 42 },
        ],
    ...
    expect(WRITE_PACKED_MINUTE_SQL.trim().toLowerCase()).not.toMatch(
      /returning\s+\w+\s*$/
    )
  ...
  it("upserts identical failures without losing aggregate fields", () => {
    expect(WRITE_PACKED_MINUTE_SQL).toContain(
