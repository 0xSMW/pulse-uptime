...
function selectByPerMinuteOrderByLimit1(
  snapshots: readonly Snapshot[],
  minute: number
): number | undefined {
  const candidates = snapshots.filter(
    (snapshot) => snapshot.acceptedAt <= minute
  )
  if (candidates.length === 0) {
    return
  }
  const sorted = [...candidates].sort(
    (a, b) => b.acceptedAt - a.acceptedAt || b.seenAt - a.seenAt
  )
...
function buildAcceptedRanges(snapshots: readonly Snapshot[]) {
  ...
  const distinctAcceptedAtsAsc = [...winnerByAcceptedAt.keys()].sort(
    (a, b) => a - b
  )
...
function selectByPrecomputedRange(
  ...
  const match = ranges.find(
    (range) =>
      range.acceptedAt <= minute &&
      (range.nextAcceptedAt === null || minute < range.nextAcceptedAt)
  )
  return match?.configVersion
}

function expectEquivalentAcrossMinutes(
  snapshots: readonly Snapshot[],
  minutes: readonly number[]
) {
  const ranges = buildAcceptedRanges(snapshots)
  for (const minute of minutes) {
    expect(selectByPrecomputedRange(ranges, minute)).toBe(
      selectByPerMinuteOrderByLimit1(snapshots, minute)
    )
  }
}
...
describe("scheduler gap accepted-config selection equivalence", () => {
  ...
  it("agrees across many distinct accepted_at values and out-of-order insertion", () => {
    ...
    const minutes = [
      0, 99, 100, 150, 199, 200, 299, 300, 399, 400, 499, 500, 999,
    ]
  ...
  it("agrees across a randomized fuzz sweep of accepted_at/seen_at combinations", () => {
    ...
    const minutes = Array.from({ length: 250 }, () =>
      Math.floor(nextRandom() * 250)
    )
    expectEquivalentAcrossMinutes(snapshots, minutes)
  })
})
