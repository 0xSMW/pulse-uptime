...
suite("atomic minute PostgreSQL transaction", () => {
  ...
  const input = (
    ...
    states,
    schedulerStartedAt: minute,
    schedulerCompletedAt: new Date(minute.getTime() + 1000),
    results: [
      {
        monitorId: "api",
        monitorName: "API",
        checkedAt: new Date(minute.getTime() + 500),
        successful: true,
        statusCode: 204,
        latencyMs: 25,
        effectiveUrl: null,
  ...
  it("commits once and makes a duplicate minute a strict no-op", async () => {
    ...
    await persistAtomicMinute(
      executor,
      input(minute, new Map([["api", baseState]]))
    )
    await persistAtomicMinute(
      executor,
      input(minute, new Map([["api", baseState]]))
    ...
    const [state] = await client<
      { version: number }[]
    >`select version from monitor_state where monitor_id = 'api'`
    const [batches] = await client<
      { count: number }[]
    >`select count(*)::integer count from check_batches`
    expect(state?.version).toBe(1)
    expect(batches?.count).toBe(1)
    await client.unsafe(COMPACT_15_MINUTE_SQL, [
      minute,
      new Date("2026-07-18T03:30:00Z"),
      new Date("2026-07-18T03:30:00Z"),
    ] as never[])
  ...
  it("rolls the batch back when the expected state version is stale", async () => {
    ...
    const [batch] = await client<
      { count: number }[]
    >`select count(*)::integer count from check_batches where scheduled_minute = ${minute}`
  ...
  it("aggregates identical failures per incident and retains expiring detail payloads", async () => {
    const loadState = async () =>
      (
        await client<MonitorStateSnapshot[]>`select
      ...
      version, updated_at "updatedAt" from monitor_state where monitor_id = 'api'`
      )[0]!
    ...
    await persistAtomicMinute(
      executor,
      failureInput(firstMinute, await loadState())
    )
    ...
    const [history] = await client<
      { occurrence_count: number; worst_latency_ms: number }[]
    >`
  ...
  it("recovers more than 48 hours of missing scheduler coverage as Unknown", async () => {
    ...
    const [exceptions] = await client<
      { count: number }[]
    >`select count(*)::integer count from monitor_exceptions
    ...
    for (let attempt = 0; attempt < 2; attempt += 1) {
      ...
      await client.unsafe(PROMOTE_ROLLUP_SQL, [
        "15m",
        "hour",
        scanStart,
        scanEnd,
    ...
    const [daily] = await client<
      { expected: number; completed: number; unknown: number; rows: number }[]
    >`
  ...
  it("picks the accepted snapshot whose range covers each minute across a mid-gap config change", async () => {
    ...
    for (const row of before) {
      ...
      expect(row.expected_bitmap).toEqual(Buffer.from([0b0000_0001]))
    }
    for (const row of after) {
      expect(row.config_version).toBe(31)
      expect(row.monitor_ids).toEqual(["gap-a", "gap-b"])
      expect(row.expected_bitmap).toEqual(Buffer.from([0b0000_0011]))
    }
