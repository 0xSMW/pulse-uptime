...
export function encodeTelemetry(
  ...
  values.forEach((value, index) => {
    if (value.completed && !value.expected) {
      throw new Error("An unexpected check cannot complete")
    }
    if (value.failed && !value.completed) {
      throw new Error("An incomplete check cannot fail")
    }
    if (value.completed && value.latencyMs === null) {
      throw new Error("A completed check requires latency")
    }
    if (
      value.latencyMs !== null &&
      (!Number.isSafeInteger(value.latencyMs) ||
        value.latencyMs < 0 ||
        value.latencyMs >= 0xff_ff_ff_ff)
    ) {
      throw new RangeError("Latency must be a nonnegative 32-bit integer")
    }
    latencies.writeUInt32BE(value.latencyMs ?? 0xff_ff_ff_ff, index * 4)
  })
  return {
...
export function decodeTelemetry(packed: PackedTelemetry): TelemetryValue[] {
  if (packed.encodingVersion !== PACKED_TELEMETRY_VERSION) {
    throw new Error(
      `Unsupported packed telemetry version: ${packed.encodingVersion}`
    )
  }
  const bitmapBytes = Math.ceil(packed.monitorCount / 8)
  if (
    [packed.expectedBitmap, packed.completedBitmap, packed.failureBitmap].some(
      (bitmap) => bitmap.length !== bitmapBytes
    )
  ) {
    throw new Error("Invalid packed bitmap length")
  }
  const result: TelemetryValue[] = []
  if (packed.latencyValues.length !== packed.monitorCount * 4) {
    throw new Error("Invalid packed latency length")
  }
  for (let index = 0; index < packed.monitorCount; index += 1) {
    ...
    if (failed && !completed) {
      throw new Error("Invalid failure bitmap")
    }
    if (completed && !expected) {
      throw new Error("Invalid completed bitmap")
    }
    if (completed === (encodedLatency === 0xff_ff_ff_ff)) {
      throw new Error("Invalid packed latency completion marker")
    }
    result.push({
      expected,
      completed,
      failed,
      latencyMs: encodedLatency === 0xff_ff_ff_ff ? null : encodedLatency,
    })
  }
  return result
}
