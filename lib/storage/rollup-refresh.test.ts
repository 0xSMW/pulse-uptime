...
describe("completesQuarterHourBucket", () => {
  it("fires on the last minute of each quarter-hour bucket", () => {
    expect(
      completesQuarterHourBucket(new Date("2026-07-18T12:14:00.000Z"))
    ).toBe(true)
    expect(
      completesQuarterHourBucket(new Date("2026-07-18T12:29:00.000Z"))
    ).toBe(true)
    expect(
      completesQuarterHourBucket(new Date("2026-07-18T12:44:00.000Z"))
    ).toBe(true)
    expect(
      completesQuarterHourBucket(new Date("2026-07-18T12:59:00.000Z"))
    ).toBe(true)
  })

  it("stays quiet on every other minute", () => {
    expect(
      completesQuarterHourBucket(new Date("2026-07-18T12:00:00.000Z"))
    ).toBe(false)
    expect(
      completesQuarterHourBucket(new Date("2026-07-18T12:15:00.000Z"))
    ).toBe(false)
    expect(
      completesQuarterHourBucket(new Date("2026-07-18T12:37:00.000Z"))
    ).toBe(false)
  })
})
...
describe("refreshRecentRollups", () => {
  ...
  it("caps the backfill window when coverage is missing entirely", async () => {
    ...
    await refreshRecentRollups(db, scheduledMinute, now)
    expect(db.calls[1]!.values[0]).toEqual(
      new Date(end.getTime() - ROLLUP_REFRESH_MAX_LOOKBACK_MS)
    )
  })
})
