...
describe("packed telemetry codec", () => {
  it.each([
    0, 1, 100,
  ])("round trips %i monitor values deterministically", (count) => {
    const values: TelemetryValue[] = Array.from(
      { length: count },
      (_, index) => ({
        expected: index % 5 !== 0,
    ...
    if (count === 100) {
      expect(
        first.expectedBitmap.length +
          first.completedBitmap.length +
          first.failureBitmap.length +
          first.latencyValues.length
  ...
  it("rejects unknown versions independently of the payload", () => {
    ...
    expect(() =>
      decodeTelemetry({
        ...packed,
        encodingVersion: PACKED_TELEMETRY_VERSION + 1,
      })
