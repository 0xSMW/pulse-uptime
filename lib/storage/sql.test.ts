...
describe("storage maintenance SQL", () => {
  it("compacts explicit missing completion as Unknown idempotently", () => {
    ...
    expect(COMPACT_15_MINUTE_SQL).toContain(
      "on conflict (monitor_id, resolution, bucket_start) do update"
    )
  ...
  it("materializes entirely missing scheduler minutes idempotently", () => {
    ...
    expect(FILL_SCHEDULER_GAPS_SQL).toContain(
      "max(bucket_start) + interval '15 minutes'"
    )
    expect(FILL_SCHEDULER_GAPS_SQL).toContain("where resolution = '15m'")
    expect(FILL_SCHEDULER_GAPS_SQL).toContain(
      "coalesce(coverage.covered_until, accepted_start.accepted_at"
    )
    expect(FILL_SCHEDULER_GAPS_SQL).toContain(
  ...
  it("resolves the accepted config per minute from precomputed effective ranges, not a per-minute lateral order-by", () => {
    expect(FILL_SCHEDULER_GAPS_SQL).not.toMatch(
      /order by accepted_at desc, seen_at desc limit 1/
    )
    expect(FILL_SCHEDULER_GAPS_SQL).toContain(
      "partition by accepted_at order by seen_at desc"
    )
    expect(FILL_SCHEDULER_GAPS_SQL).toContain(
    ...
    expect(FILL_SCHEDULER_GAPS_SQL).toContain(
      "next_accepted_at is null or minute.scheduled_minute < next_accepted_at"
    )
    expect(FILL_SCHEDULER_GAPS_SQL).toContain(
  ...
  it("measures relation and index allocation and writes one daily snapshot", () => {
    ...
    expect(MEASURE_USAGE_SQL).toContain(
  ...
  it("classifies governor mode from the shared threshold constants", () => {
    ...
    expect(MEASURE_USAGE_SQL).toContain(
      `when projected_bytes * 100 < $2::bigint * ${p.compactEarly} then 'full'`
    )
    expect(MEASURE_USAGE_SQL).toContain(
      `when projected_bytes * 100 < $2::bigint * ${p.shortened} then 'compact_early'`
    )
    expect(MEASURE_USAGE_SQL).toContain(
      `when projected_bytes * 100 < $2::bigint * ${p.incidentOnly} then 'shortened'`
    )
    expect(MEASURE_USAGE_SQL).toContain(
      `when projected_bytes * 100 <= $2::bigint * ${p.essential} then 'incident_only'`
    )
