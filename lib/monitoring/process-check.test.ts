...
function fakeStore(
  options: { inserted?: boolean; state?: MonitorStateSnapshot | null } = {}
) {
  const calls: string[] = []
  const captured: {
    incident?: unknown
  ...
  const tx: ProcessCheckTransaction = {
    async insertCheck() {
      calls.push("insert-check")
      return options.inserted ?? true
    },
    async lockMonitorState() {
      calls.push("lock-state")
      return options.state === undefined ? monitor() : options.state
    },
    async insertIncident(value) {
      calls.push("insert-incident")
      captured.incident = value
    },
    async updateIncidentProgress() {
      calls.push("update-incident")
    },
    async resolveIncident() {
      calls.push("resolve-incident")
    },
    async insertOutbox(value) {
      calls.push("insert-outbox")
      captured.outbox = value
    },
    async updateMonitorState(value) {
...
describe("processCheckWithStore", () => {
  ...
  it("inserts the result before locking and persists the incident atomically", async () => {
    ...
    expect(fake.captured.state).toMatchObject({
      state: "DOWN",
      activeIncidentId: result.incidentId,
    })
  })
  ...
  it("uses stable exactly-once keys and canonical recipient addresses", async () => {
    ...
    expect(first.captured.outbox?.map((row) => row.recipient)).toEqual([
      "ops@example.com",
      "owner@example.com",
    ])
    expect(first.captured.outbox?.map((row) => row.idempotencyKey)).toEqual(
      second.captured.outbox?.map((row) => row.idempotencyKey)
    )
    expect(
      first.captured.outbox?.every((row) =>
        String(row.idempotencyKey).includes("/opened/")
      )
    ).toBe(true)
    expect(
      first.captured.outbox?.every((row) => {
  ...
  it("resolves an incident and queues recovery after the configured success threshold", async () => {
    ...
    const result = await processCheckWithStore(
      fake.store,
      check({
        successful: true,
        statusCode: 200,
    ...
    expect(
      fake.captured.outbox?.every((row) =>
        String(row.idempotencyKey).includes("/resolved/")
      )
    ).toBe(true)
    expect(
      fake.captured.outbox?.every((row) => {
  ...
  it("fails the transaction when monitor state is missing", async () => {
    ...
    await expect(processCheckWithStore(fake.store, check())).rejects.toThrow(
      "Monitor state not found: api"
    )
