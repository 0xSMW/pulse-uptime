...
let uptimeRows: Array<{ id: string; activatedAt: Date | null; uptime24h: number | string | null }> = [];
...
describe("uptime24hByMonitorId", () => {
  ...
  it("passes uptime through only once the completed window covers a full post-activation day", async () => {
    ...
    const uptime = await uptime24hByMonitorId(["settled", "collecting", "never-activated"]);
  ...
  it("reads null when the window is unlocked but holds no data", async () => {
    uptimeRows = [{ id: "gap", activatedAt: new Date(Date.now() - 3 * DAY_MS), uptime24h: null }];
