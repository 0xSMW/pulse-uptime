...
function stubSelects(rows: Rows = {}) {
  ...
  dbMock.select
    ...
    .mockReturnValueOnce(
      selectChain(rows.maintenance ?? [{ completedAt: minutesAgo(60) }])
    )
    .mockReturnValueOnce(selectChain(rows.dependencyCheck ?? fresh))
    .mockReturnValueOnce(
      selectChain(rows.installedDependency ?? [{ id: "dep" }])
    )
    .mockReturnValueOnce(
      selectChain(rows.configSnapshot ?? [{ status: "accepted" }])
    )
...
describe("getHealthWarnings", () => {
  ...
  it("keeps surfacing the monitor cron warning independent of the dependency poller", async () => {
    stubSelects({
      monitorCheck: [{ completedAt: minutesAgo(10) }],
      dependencyCheck: [{ completedAt: minutesAgo(1) }],
    })
    const codes = (await getHealthWarnings(now)).map((warning) => warning.code)
