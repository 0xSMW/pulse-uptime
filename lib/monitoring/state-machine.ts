...
const CHECK_STATES = new Set([
  "PENDING", "UP", "VERIFYING_DOWN", "DOWN", "VERIFYING_UP",
]);
...
function checkedState(current: MonitorStateSnapshot, event: CheckTransitionEvent): {
  ...
  if (!CHECK_STATES.has(current.state)) return { next: current, incident: null };
  ...
  if (event.successful) {
    ...
    const firstSuccessAt = current.state === "VERIFYING_UP" && current.firstSuccessAt
  ...
  if (current.state === "DOWN" || current.state === "VERIFYING_UP") {
    next.state = "DOWN";
    next.consecutiveFailures = Math.max(current.consecutiveFailures, event.failureThreshold);
    next.firstFailureAt = current.firstFailureAt;
    return { next, incident: null };
  }

  const firstFailureAt = (current.state === "VERIFYING_DOWN" || current.state === "PENDING") && current.firstFailureAt
    ? current.firstFailureAt
    : event.checkedAt;
  const failures = current.state === "VERIFYING_DOWN" || current.state === "PENDING"
  ...
  next.state = "DOWN";
  return { next, incident: { type: "open", openedAt: firstFailureAt, firstFailureAt } };
}
...
export function transitionMonitor(
  ...
  if (event.type === "disable" && CHECK_STATES.has(current.state)) state = "PAUSED";
  if (event.type === "archive" && current.state !== "ARCHIVED") state = "ARCHIVED";
  if (event.type === "enable" && current.state === "PAUSED") state = "PENDING";
  if (event.type === "restore" && current.state === "ARCHIVED") state = "PENDING";

  if (state === current.state) {
    return { previousState: current.state, state: current, changed: false, incident: null };
  }
  return {
    previousState: current.state,
    state: {
