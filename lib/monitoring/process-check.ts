...
export function createDrizzleProcessCheckStore(
  database: Database
): ProcessCheckStore {
  return {
    transaction: async <T>(
      ...
      database.transaction(async (databaseTx) =>
        work({
          ...
          async updateIncidentProgress(incidentId, progress, now) {
            const rows = await databaseTx
              ...
              .where(
                and(eq(incidents.id, incidentId), isNull(incidents.resolvedAt))
              )
              .returning({ id: incidents.id })
            if (rows.length !== 1) {
              throw new Error(`Active incident not found: ${incidentId}`)
            }
          },
          ...
          async updateMonitorState(state) {
            const rows = await databaseTx
              ...
              .returning({ id: monitorState.monitorId })
            if (rows.length !== 1) {
              throw new Error(`Monitor state not found: ${state.monitorId}`)
            }
          },
        })
      ),
