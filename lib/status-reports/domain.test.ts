...
function report(affected: Array<{ monitorId: string; groupName?: string | null }>) {
  return {
...
describe("filterReportsForGroup", () => {
  ...
  it("keeps reports matching a visible monitor id in the group", () => {
    ...
    expect(filterReportsForGroup([matching], { slug: "apis", visibleMonitorIds })).toEqual([matching]);
  });
  ...
  it("matches even when the snapshot spells the group with accents or case the slug folds away", () => {
    ...
    expect(filterReportsForGroup([matching], { slug: "cafe", visibleMonitorIds })).toEqual([matching]);
  });

  it("collapses null group names to the Other bucket", () => {
    const matching = report([{ monitorId: "gone", groupName: null }]);
    expect(filterReportsForGroup([matching], { slug: "other", visibleMonitorIds })).toEqual([matching]);
    expect(filterReportsForGroup([matching], { slug: "apis", visibleMonitorIds })).toEqual([]);
  });
  ...
  it("drops reports with no overlap and reports with no affected services", () => {
    ...
    expect(filterReportsForGroup([other, report([])], { slug: "apis", visibleMonitorIds })).toEqual([]);
  });
});

describe("vocabulary", () => {
  it("orders the per-type update statuses", () => {
    expect(REPORT_STATUSES.incident).toEqual(["investigating", "identified", "monitoring", "resolved"]);
    expect(REPORT_STATUSES.maintenance).toEqual(["scheduled", "in_progress", "completed"]);
  });
  ...
  it("scopes the impact vocabulary to the report type", () => {
    expect(IMPACT_BY_TYPE.incident).toEqual<readonly ReportImpact[]>(["down", "degraded"]);
    expect(IMPACT_BY_TYPE.maintenance).toEqual<readonly ReportImpact[]>(["maintenance", "degraded"]);
  });
