...
const refreshes = new WeakMap<
  ...
  Promise<DatabaseHealthMeasurement | null>
>()
...
export function deriveDatabaseHealthState(
  ...
  if (measurement.maintenanceHealthy === false) {
    return "CRITICAL"
  }
  ...
  if (utilization > 0.95 || measurement.governorMode === "ESSENTIALS_ONLY") {
    return "CRITICAL"
  }
  if (
    utilization >= 0.85 ||
    measurement.governorMode === "INCIDENT_HOURLY_ONLY"
  ) {
    return "PROTECTING"
  }
  if (
    utilization >= 0.75 ||
    ["EARLY_COMPACTION", "SHORTENED_RETENTION"].includes(
      measurement.governorMode
    )
  ) {
    return "OPTIMIZING"
  }
...
function summaryFor(state: DatabaseHealthState): string {
  switch (state) {
    case "HEALTHY":
      return "Storage remains within its configured budget"
    case "WATCHING":
      return "Projected usage is approaching the budget"
    case "OPTIMIZING":
      return "Automatic compaction is reducing growth"
    case "PROTECTING":
      return "Fine-grained retention is being reduced"
    case "CRITICAL":
      return "Storage or maintenance needs attention"
    case "UNKNOWN":
      return "Current database metrics are unavailable"
  }
}
...
export function presentDatabaseHealth(
  ...
  options: {
    now?: Date
    refreshStatus?: DatabaseHealth["refresh"]["status"]
  } = {}
): DatabaseHealth {
  ...
  const providerAgeMs =
    measurement.providerMetricsCapturedAt === null
      ? null
      : Math.max(
          0,
  ...
  const stale =
    !Number.isFinite(ageMs) || ageMs > DATABASE_HEALTH_STALE_AFTER_MS
  const attributed = DATABASE_HEALTH_CATEGORY_KEYS.filter(
    (key): key is AttributedDatabaseCategoryKey => key !== "other"
  ).map((key) => ({
    key,
    label: CATEGORY_LABELS[key],
    bytes: finiteBytes(measurement.categoryBytes[key]) ?? 0,
  }))
  const attributedBytes = attributed.reduce(
  ...
  const categories = [
    ...attributed,
    {
      ...
      bytes:
        finiteBytes(measurement.otherBytes) ??
        Math.max(0, (usedBytes ?? attributedBytes) - attributedBytes),
    },
  ]
  const health = deriveDatabaseHealthState(measurement, now)
  const eligibleAt =
    measurement.capturedAt.valueOf() + DATABASE_HEALTH_REFRESH_INTERVAL_MS
  ...
  return {
    ...
    availableBytes:
      usedBytes === null
        ? null
        : Math.max(0, DATABASE_STORAGE_BUDGET_BYTES - usedBytes),
    projected30DayBytes,
    categories,
    retention: measurement.retention.map((item) => ({
      ...item,
      configuredSeconds:
        item.configuredSeconds === null
          ? null
          : Math.max(0, Math.round(item.configuredSeconds)),
      oldestAt: item.oldestAt?.toISOString() ?? null,
    })),
    governor: {
      mode: measurement.governorMode,
      action:
        measurement.governorAction?.trim() ||
        GOVERNOR_ACTIONS[measurement.governorMode],
      lastCompactionAt: measurement.lastCompactionAt?.toISOString() ?? null,
    },
    schedulerCoverage:
      measurement.schedulerCoverage === null
        ? null
        : Math.min(1, Math.max(0, measurement.schedulerCoverage)),
    transfer: {
    ...
    freshness: {
      ...
      providerCapturedAt:
        measurement.providerMetricsCapturedAt?.toISOString() ?? null,
      providerAgeSeconds:
        providerAgeMs === null ? null : Math.round(providerAgeMs / 1000),
    },
...
export async function getDatabaseHealth(
  ...
  if (cached && cached.eligibleAt > now.valueOf()) {
    return presentDatabaseHealth(cached.measurement, {
      now,
...
export async function refreshDatabaseHealth(
  ...
  if (
    current &&
    current.capturedAt.valueOf() + DATABASE_HEALTH_REFRESH_INTERVAL_MS >
      now.valueOf()
  ) {
    cache.set(repository, {
      measurement: current,
...
export function clearDatabaseHealthCache(
  repository: DatabaseHealthRepository
): void {
