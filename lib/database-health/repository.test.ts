...
const snapshot = {
  ...
  category_bytes: JSON.stringify({ recentCheckBatches: 3_000_000, coreData: 8_000_000, indexes: 14_000_000, other: 4_000_000 }),
...
describe("database health repository", () => {
  ...
  it("captures allocation in Postgres before reading the stored snapshot", async () => {
    ...
    await expect(databaseHealthRepository.capture()).resolves.toMatchObject({ capturedAt: new Date(snapshot.captured_at) });
  ...
  it("treats running maintenance as unknown and applies effective retention", async () => {
    unsafe
      .mockResolvedValueOnce([{ ...snapshot, governor_mode: "shortened", maintenance_status: "running" }])
      .mockResolvedValueOnce([
        { key: "minute", label: "Recent checks", configured_seconds: 172800, oldest_at: "2026-07-17T00:00:00.000Z" },
        { key: "15m", label: "15-minute rollups", configured_seconds: 604800, oldest_at: null },
      ]);
    const result = await databaseHealthRepository.readLatest();
  ...
  it("normalizes timestamps created by another runtime realm", async () => {
    const foreignDate = runInNewContext("new Date('2026-07-18T04:00:00.000Z')") as Date;
    expect(foreignDate).not.toBeInstanceOf(Date);
    unsafe
      .mockResolvedValueOnce([{ ...snapshot, captured_at: foreignDate, provider_metrics_captured_at: foreignDate }])
      .mockResolvedValueOnce([{ key: "minute", label: "Recent checks", configured_seconds: 172800, oldest_at: foreignDate }]);
    const result = await databaseHealthRepository.readLatest();
    expect(result?.capturedAt.toISOString()).toBe("2026-07-18T04:00:00.000Z");
    expect(result?.retention[0]?.oldestAt?.toISOString()).toBe("2026-07-18T04:00:00.000Z");
  });
});
