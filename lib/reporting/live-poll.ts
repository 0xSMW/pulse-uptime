...
export function livePollIntervalMs(input: {
  phase: MonitorPhase
  state: MonitorState
}): number {
  if (input.phase === "setup") {
    return LIVE_POLL_ATTENTIVE_MS
  }
  return ATTENTIVE_STATES.has(input.state)
