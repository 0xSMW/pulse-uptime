...
describe("livePollIntervalMs", () => {
  it("polls a steady active monitor on the slow cadence", () => {
    expect(livePollIntervalMs({ phase: "active", state: "UP" })).toBe(
      LIVE_POLL_STEADY_MS
    )
    expect(livePollIntervalMs({ phase: "collecting", state: "UP" })).toBe(
      LIVE_POLL_STEADY_MS
    )
    expect(livePollIntervalMs({ phase: "active", state: "PAUSED" })).toBe(
      LIVE_POLL_STEADY_MS
    )
  })

  it("polls faster while in setup regardless of state", () => {
    expect(livePollIntervalMs({ phase: "setup", state: "PENDING" })).toBe(
      LIVE_POLL_ATTENTIVE_MS
    )
    expect(livePollIntervalMs({ phase: "setup", state: "UP" })).toBe(
      LIVE_POLL_ATTENTIVE_MS
    )
  })

  it("polls faster while verifying or down", () => {
    expect(livePollIntervalMs({ phase: "active", state: "DOWN" })).toBe(
      LIVE_POLL_ATTENTIVE_MS
    )
    expect(
      livePollIntervalMs({ phase: "active", state: "VERIFYING_DOWN" })
    ).toBe(LIVE_POLL_ATTENTIVE_MS)
    expect(livePollIntervalMs({ phase: "active", state: "VERIFYING_UP" })).toBe(
      LIVE_POLL_ATTENTIVE_MS
    )
    expect(livePollIntervalMs({ phase: "collecting", state: "PENDING" })).toBe(
      LIVE_POLL_ATTENTIVE_MS
    )
  })
...
describe("livePollBackoffMs", () => {
  ...
  it("caps at the maximum delay", () => {
    ...
    expect(livePollBackoffMs(2000)).toBe(LIVE_BACKOFF_MAX_MS)
  })
})
