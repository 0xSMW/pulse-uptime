...
export function groupByMonitorId<T extends { monitorId: string }>(
  rows: T[]
): Map<string, T[]> {
...
export const getStatusPageDisplayConfig = cache(
  async (): Promise<StatusPageConfigDocument> => {
    try {
      ...
      if (
        error instanceof StatusPageConfigError &&
...
export const getStatusFaviconDataUri = cache(
  async (): Promise<string | null> => {
    const config = await getStatusPageDisplayConfig()
    if (!config.faviconImageId) {
      return null
    }
    try {
...
async function loadPublicStatus(group?: string) {
  ...
  const monitors = await db
    ...
    .where(
      and(eq(monitorRegistry.enabled, true), isNull(monitorRegistry.archivedAt))
    )
    .limit(100)
  const visible = group
    ? monitors.filter(
        (monitor) => statusGroupSlug(monitor.groupName ?? "Other") === group
      )
    : monitors
  ...
  const publicReportsFilter = group
    ? { monitorIds: ids, groupSlug: group }
    : undefined
    ...
    ids.length === 0
      ...
      : Promise.all([
          ...
          db
            ...
            .innerJoin(
              monitorRegistry,
              eq(monitorRegistry.id, incidents.monitorId)
            )
            .where(
              and(
  ...
  const reports: PublicReports = group
    ? {
        ...
        windowEnded: filterReportsForGroup(
          publicReports.windowEnded,
          groupFilter
        ),
    ...
    .map(([name, entries]) => ({
      ...
      monitors: entries
        ...
        .map((monitor) => {
          ...
          const rows = rollupsSinceActivation(
            rollupsByMonitor.get(monitor.id) ?? [],
  ...
  return {
    ...
    reports,
    currentIncidents: excludePromotedIncidents(current, ongoingPromoted).map(
      (incident) => ({
        id: incident.id,
        ...
        elapsedSeconds: Math.max(
          0,
          Math.floor((now.getTime() - incident.openedAt.getTime()) / 1000)
        ),
