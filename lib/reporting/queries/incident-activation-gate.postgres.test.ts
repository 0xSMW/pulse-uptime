...
suite("incident activation gate", () => {
  ...
  async function seed(): Promise<void> {
    ...
    const incidentRows = [
      {
        id: INC_PRE,
        monitor: "mon-act",
        opened: new Date("2026-07-05T00:00:00.000Z"),
        resolved: new Date("2026-07-06T00:00:00.000Z"),
      },
      {
        id: INC_POST,
        monitor: "mon-act",
        opened: new Date("2026-07-12T00:00:00.000Z"),
        resolved: new Date("2026-07-13T00:00:00.000Z"),
      },
      {
        id: INC_LIVE,
        monitor: "mon-act",
        opened: new Date("2026-07-11T00:00:00.000Z"),
        resolved: null,
      },
      {
        id: INC_SETUP_RESOLVED,
        monitor: "mon-setup",
        opened: new Date("2026-07-08T00:00:00.000Z"),
        resolved: new Date("2026-07-09T00:00:00.000Z"),
      },
      {
        id: INC_SETUP_LIVE,
        monitor: "mon-setup",
        opened: new Date("2026-07-08T12:00:00.000Z"),
        resolved: null,
      },
    ]
    ...
    const backfill = await readFile(
      resolve(process.cwd(), "drizzle", "0013_worried_hairball.sql"),
      "utf8"
    )
  ...
  it("blocks promotion of a pre-activation setup incident but promotes a real one", async () => {
    await expect(promoteIncident(INC_PRE)).rejects.toBeInstanceOf(
      StatusReportError
    )
    await expect(promoteIncident(INC_PRE)).rejects.toMatchObject({
      code: "INCIDENT_NOT_FOUND",
    })
    await expect(promoteIncident(INC_SETUP_LIVE)).rejects.toMatchObject({
      code: "INCIDENT_NOT_FOUND",
    })
  ...
  it("clamps activated_at to the ongoing incident open so a recovery-race outage stays visible", async () => {
    ...
    const paletteIds = (await listCommandPaletteIncidents()).map(
      (row) => row.id
    )
