...
suite("recent raw checks read straight from check_batches", () => {
  ...
  it("returns only a 10-minute monitor's due minutes, decodes latency, and surfaces a scheduler gap", async () => {
    ...
    const rows = await getRecentRawChecks(
      "raw-10m",
    ...
    expect(rows).toEqual([
      {
        checked_at: new Date(base.getTime() + 20 * 60_000),
        completed: true,
        failed: true,
        latency_ms: 850,
      },
      {
        checked_at: new Date(base.getTime() + 10 * 60_000),
        completed: false,
        failed: false,
        latency_ms: null,
      },
  ...
  it("nulls latency defensively when a completed check's bytes still carry the no-latency sentinel", async () => {
    ...
    expect(rows).toEqual([
      { checked_at: minute, completed: true, failed: false, latency_ms: null },
    ])
  })
  ...
  it("counts the straddling-bucket tail and the uncompacted tail but not the compacted middle", async () => {
    ...
    const beforeFloor = await getRawTailCounts(
      "straddle-mon",
      {
