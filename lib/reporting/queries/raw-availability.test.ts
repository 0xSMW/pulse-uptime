...
describe("RAW_AVAILABILITY_BUCKETS_SQL shape", () => {
  it("ranges check_batches before unnest and filters by monitor ids", () => {
    ...
    expect(sql).toContain(
      "expected = 1 and slots.completed = 1 and slots.failed = 0"
    )
    expect(sql).toContain(
      "expected = 1 and slots.completed = 1 and slots.failed = 1"
    )
...
describe("fetchRawAvailabilityBuckets", () => {
  ...
  it("skips the database when no monitors are requested", async () => {
    expect(
      await fetchRawAvailabilityBuckets(
        [],
  ...
  it("maps rows from the shared batch query", async () => {
    ...
    expect(sqlMock.unsafe).toHaveBeenCalledWith(RAW_AVAILABILITY_BUCKETS_SQL, [
      start.toISOString(),
      end.toISOString(),
      ["mon-a"],
    ])
