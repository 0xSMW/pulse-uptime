...
describe("buildCheckTimeline", () => {
  ...
  it("produces identical results regardless of input row order", () => {
    ...
    const forward = buildCheckTimeline(rows, 6, 60 * 60 * 1000, now)
    const reversed = buildCheckTimeline(
      [...rows].reverse(),
      6,
      60 * 60 * 1000,
      now
...
describe("buildRollupTimeline", () => {
  it("produces identical results for unsorted rows as for sorted rows", () => {
    ...
    const rowAt = (
      iso: string,
      overrides: Partial<Parameters<typeof buildRollupTimeline>[0][number]> = {}
    ) => ({
    ...
    const sortedResult = buildRollupTimeline(sorted, 3, 3 * 60 * 60 * 1000, now)
    const shuffledResult = buildRollupTimeline(
      shuffled,
      3,
      3 * 60 * 60 * 1000,
      now
  ...
  it("excludes rows outside the requested range, before or after", () => {
    ...
    const beforeRange = {
      ...inRange,
      bucketStart: new Date("2026-07-18T10:59:59.999Z"),
    }
    const afterRange = {
      ...inRange,
      bucketStart: new Date("2026-07-18T12:00:00.001Z"),
    }
...
describe("buildCheckTimeline/buildRollupTimeline equivalence with the reference filter-per-bucket behavior", () => {
  ...
  function randomCheckRows(
    ...
    for (let i = 0; i < count; i += 1) {
      ...
      rows.push({
        checkedAt: new Date(startMs + offset),
        successful: rand() > 0.4,
      })
    }
    return rows
  ...
  function boundaryCheckRows(
    ...
    rows.push({
      checkedAt: new Date(startMs + bucketCount * width + 1),
      successful: false,
    })
    return rows
  }

  function boundaryRollupRows(
    startMs: number,
    width: number,
    bucketCount: number
  ): RollupAvailability[] {
  ...
  it("matches the reference across many random non-divisible window shapes, not just fixed cases", () => {
    for (let trial = 0; trial < 25; trial += 1) {
      ...
      const durationMs = 1000 + Math.floor(rand() * 50_000_000)
      const startMs = now.getTime() - durationMs
      const width = durationMs / bucketCount
      ...
      expect(
        buildCheckTimeline(checkRows, bucketCount, durationMs, now)
      ...
      expect(
        buildRollupTimeline(rollupRows, bucketCount, durationMs, now)
      ).toEqual(
        referenceRollupTimeline(rollupRows, bucketCount, durationMs, now)
      )
    }
  })
  ...
  it("handles empty input identically for both functions", () => {
    ...
    expect(buildCheckTimeline([], bucketCount, durationMs, now)).toEqual(
      referenceCheckTimeline([], bucketCount, durationMs, now)
    )
    expect(buildRollupTimeline([], bucketCount, durationMs, now)).toEqual(
      referenceRollupTimeline([], bucketCount, durationMs, now)
    )
  })
})
...
describe("blendRawAvailability", () => {
  const rollupAt = (
    iso: string,
    overrides: Partial<RollupAvailability> = {}
  ): RollupAvailability => ({
    bucketStart: new Date(iso),
    expectedChecks: 4,
    completedChecks: 4,
  ...
  const rawAt = (
    iso: string,
    overrides: Partial<RawBucketAvailability> = {}
  ): RawBucketAvailability =>
    rollupAt(iso, {
      completedChecks: 1,
      expectedChecks: 1,
      successfulChecks: 1,
  ...
  it("drops a raw bucket whose quarter-hour already has a rollup, so it is never counted twice", () => {
    const rollups = [rollupAt("2026-07-20T00:00:00Z")]
    const raw = [
      rawAt("2026-07-20T00:00:00Z", { successfulChecks: 0, failedChecks: 1 }),
    ]
  ...
  it("keeps a raw bucket for a quarter-hour with no rollup, so compaction lag renders instead of no-data", () => {
    ...
    const timeline = buildRollupTimeline(merged, 2, 30 * 60 * 1000, now)
  ...
  it("leaves a quarter-hour with neither a rollup nor a raw bucket as no-data", () => {
    const merged = blendRawAvailability(
      [rollupAt("2026-07-20T00:00:00Z")],
      [rawAt("2026-07-20T00:30:00Z")]
    )
    const timeline = buildRollupTimeline(
      merged,
      3,
      45 * 60 * 1000,
      new Date("2026-07-20T00:45:00Z")
    )
