...
function mockUnsafeBySql(
  ...
  sqlMock.unsafe.mockImplementation(async (query: string) => {
    ...
    if (
      query.includes("$4::timestamptz is null") ||
      query.includes("lastCompletedBucketEnd")
    ) {
      return (
        options.tail ?? [
          { expected: 0, completed: 0, successful: 0, failed: 0 },
        ]
      )
    }
...
describe("getMonitorLive p95 and incidents", () => {
  ...
  it("surfaces no incidents and issues no incident query when the monitor never activated", async () => {
    dbMock.select
      .mockReturnValueOnce(
        selectChain([identity({ activatedAt: null, state: "PENDING" })])
      ) // identity
...
describe("selectRecentRollupWindow", () => {
  ...
  it("returns an empty array when nothing falls in the window", () => {
    ...
    expect(
      selectRecentRollupWindow([rowAt("2026-07-10T00:00:00Z")], cutoff, end)
    ).toEqual([])
  })
})
