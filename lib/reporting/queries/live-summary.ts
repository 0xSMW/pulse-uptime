...
export function rollupVersionOf(
  rollups15m: Array<{ bucketStart: Date }>
): string | null {
...
export function buildLatestIncident(
  rows: LiveIncidentRow[],
  now: Date
): LiveLatestIncident | null {
  const latest = rows[0]
  if (!latest) {
    return null
  }
  const withinDay =
    ...
    latest.resolvedAt.getTime() >= now.getTime() - DAY_MS
  if (!withinDay) {
    return null
  }
  return {
    ...
    openingFailure: openingFailure(
      latest.openingErrorCode,
      latest.openingStatusCode
    ),
  }
}

export function buildRecentIncidents(
  rows: LiveIncidentRow[],
  now: Date
): LiveRecentIncident[] {
  return rows.map((incident) => ({
    ...
    openingFailure: openingFailure(
      incident.openingErrorCode,
      incident.openingStatusCode
    ),
  }))
}
...
export function buildRecentChecksFromRaw(
  checks: RawMinuteCheck[]
): LiveRecentCheck[] {
...
export function buildRecentChecks(
  ...
  return rollups24h
    ...
    .map((rollup) => ({
      ...
      successful:
        rollup.failedChecks === 0 &&
        rollup.completedChecks === rollup.expectedChecks,
      statusCode: null,
      resultLabel:
        rollup.unknownChecks > 0
          ? "Unknown coverage"
          : rollup.failedChecks > 0
            ? "Failed checks"
            : "Healthy rollup",
      latencyMs:
        rollup.latencyCount === 0
          ? null
          : Math.round(Number(rollup.latencySumMs) / rollup.latencyCount),
    }))
}
...
export function rawTailBounds(
  ...
  const activationFloor = new Date(
    Math.floor(activatedAt.getTime() / MINUTE_MS) * MINUTE_MS
  )
  ...
  return {
    ...
    lastCompletedBucketEnd: new Date(
      last.bucketStart.getTime() + FIFTEEN_MINUTE_MS
    ),
  }
}
...
export function buildFirstRun(
  ...
  return {
    ...
    observedSeconds: Math.floor(observedMs(activatedAt, now) / 1000),
    observed: {
    ...
    setupError:
      phase === "setup" && (monitor.consecutiveFailures ?? 0) > 0
