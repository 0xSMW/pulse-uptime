...
import { type MonitorConfig, validateMonitoringConfig } from "@/lib/config"
...
const LATENCY_BUCKET_MAX_MS = [100, 250, 500, 1000, 2500, 5000, 10_000] as const

function completedRangeEnd(
  now: Date,
...
function p95Latency(
  ...
  for (let index = 0; index < 8; index += 1) {
    cumulative += rows.reduce(
      (sum, row) => sum + (row.latencyHistogram[index] ?? 0),
      0
    )
    if (cumulative >= rank) {
...
type RecentMinuteCheckDbRow = Omit<RecentMinuteCheckRow, "checked_at"> & {
...
export async function getRecentRawChecks(
  monitorId: string,
  end: Date
): Promise<RecentMinuteCheckRow[]> {
...
const EMPTY_TAIL_COUNTS: RawTailCounts = {
  expected: 0,
  completed: 0,
  successful: 0,
  failed: 0,
...
export async function getRawTailCounts(
  monitorId: string,
  bounds: RawTailBounds,
  end: Date
): Promise<RawTailCounts> {
  if (bounds.activationFloor.getTime() >= end.getTime()) {
...
export async function findMonitorDetail(id: string) {
      ...
      : db
          ...
          .where(
            and(
              ...
              gte(incidents.openedAt, activatedAt)
            )
          )
  ...
  const rollups24h = selectRecentRollupWindow(
    rollups7d,
    end15m.getTime() - 86_400_000,
    end15m.getTime()
  )
  ...
  const tailBounds =
    firstRunPhase(activatedAt, now) === "collecting"
      ? rawTailBounds(activeRollups24h, activatedAt)
      : null
  const rawTail = tailBounds
    ? await getRawTailCounts(id, tailBounds, now)
    : EMPTY_TAIL_COUNTS
  const observedCollecting = observedWithRawTail(activeRollups24h, rawTail)
  const observed7d = summarizeCounts(
    rollupsSinceActivation(rollups7d, activatedAt)
  )
  const observed30d = summarizeCounts(
    rollupsSinceActivation(rollups30d, activatedAt)
  )
  const observed90d = summarizeCounts(
  ...
  const config: MonitorConfig | undefined = acceptedConfig?.monitors.find(
    (candidate) => candidate.id === id
  )
  const groupName = config?.groupId
    ? (acceptedConfig?.groups.find((group) => group.id === config.groupId)
        ?.name ?? monitor.group)
    : null
  const responsePoints = (rows: typeof rollups24h) =>
    rows
      ...
      .map((row) => ({
        ...
        successful:
          row.failedChecks === 0 && row.completedChecks === row.expectedChecks,
      }))
  ...
  const latestIncidentOverlaps = latestIncidentRow
    ? await listOverlappingDependencyIncidents({
        openedAt: latestIncidentRow.openedAt,
        resolvedAt: latestIncidentRow.resolvedAt,
      })
    : []
  ...
  return {
    ...
    intervalMinutes:
      config?.intervalMinutes ?? DEFAULT_MONITOR_VALUES.intervalMinutes,
    timeoutMs: config?.timeoutMs ?? DEFAULT_MONITOR_VALUES.timeoutMs,
    recipients: config?.recipients ?? [],
    state: monitor.state ?? "PENDING",
    intervalSeconds:
      (config?.intervalMinutes ?? DEFAULT_MONITOR_VALUES.intervalMinutes) * 60,
    timeoutSeconds:
      (config?.timeoutMs ?? DEFAULT_MONITOR_VALUES.timeoutMs) / 1000,
    expectedStatusMin:
      config?.expectedStatus.minimum ??
      DEFAULT_MONITOR_VALUES.expectedStatus.minimum,
    expectedStatusMax:
      config?.expectedStatus.maximum ??
      DEFAULT_MONITOR_VALUES.expectedStatus.maximum,
    failureThreshold:
      config?.failureThreshold ?? DEFAULT_MONITOR_VALUES.failureThreshold,
    recoveryThreshold:
      config?.recoveryThreshold ?? DEFAULT_MONITOR_VALUES.recoveryThreshold,
    ...
    availability: {
      h24: {
        ...
        buckets: buildRollupTimeline(
          rollupsSinceActivation(
            blendRawAvailability(rollups24h, rawAvailability24h),
            activatedAt
          ),
          60,
          86_400_000,
...
export async function findMonitorLive(
  ...
  const endHour = completedRangeEnd(now, "hour")
  const endDay = completedRangeEnd(now, "day")
  const [rollups7d, recentIncidents, recentRawChecks, accepted] =
    await Promise.all([
      fetchRollups(id, "15m", end15m, 7 * 86_400_000),
      ...
      db
        .select({ acceptedAt: monitoringConfigSnapshots.acceptedAt })
        .from(monitoringConfigSnapshots)
        .where(eq(monitoringConfigSnapshots.status, "accepted"))
        .orderBy(desc(monitoringConfigSnapshots.acceptedAt))
        .limit(1),
