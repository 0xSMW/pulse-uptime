...
// the rolled-up telemetry and monitor/incident identity, plus check_results for
// the bounded raw blend that fills any quarter-hour a rollup has not closed yet.
// It never reads the dropped daily_rollups, and never decodes check_batches
// through the drizzle surface, that packed telemetry is read only through the
// storage layer's raw SQL.
...
function makeCapturingChain() {
  ...
  const node: Record<string, unknown> = {
    from(table: unknown) { fromTable = table; captured.add(table); return node; },
    leftJoin(table: unknown) { captured.add(table); return node; },
    innerJoin(table: unknown) { captured.add(table); return node; },
    where() { return node; },
    orderBy() { return node; },
    limit() { return settle(); },
    then(res: (value: unknown[]) => unknown, rej?: (reason: unknown) => unknown) { return settle().then(res, rej); },
    catch(rej: (reason: unknown) => unknown) { return settle().catch(rej); },
    finally(fn: () => void) { return settle().finally(fn); },
  };
  return node;
}
...
async function referencedTables(run: () => Promise<unknown>): Promise<Set<string>> {
  captured = new Set();
  try {
    await run();
  } catch {
...
function assertBoundary(tables: Set<string>) {
  ...
  // The dropped daily_rollups and the packed check_batches never enter the
  // drizzle reporting surface. check_results is allowed for the raw blend.
...
describe("rollup reporting boundaries", () => {
  ...
  it("monitor detail and live reads target metric_rollups and no legacy raw telemetry", async () => {
    ...
    for (const table of await referencedTables(() => getMonitorDetail("mon-detail"))) tables.add(table);
    for (const table of await referencedTables(() => getMonitorLive("mon-live"))) tables.add(table);
