...
export async function authorize(
  ...
  if (!principal) {
    const response = apiError(requestId, 401, "AUTHENTICATION_REQUIRED", "Valid authentication is required");
    response.headers.set("WWW-Authenticate", 'Bearer realm="pulse", error="invalid_token"');
    return response;
  }
  if (principal.type === "human" && !["GET", "HEAD", "OPTIONS"].includes(request.method) && !mutationOriginAllowed(request)) {
    return apiError(requestId, 403, "INVALID_ORIGIN", "Request origin is not allowed");
  }
  if (options.scope && !hasScope(principal, options.scope)) {
    return apiError(requestId, 403, "SCOPE_DENIED", "The credential lacks the required scope", {
      scope: options.scope,
    });
  }
  const principalKey = `${principal.type}:${principal.id}`;
  const defaults = request.method === "GET" ? AUTHENTICATED_READ_LIMIT : AUTHENTICATED_MUTATION_LIMIT;
  const policy = options.rateLimit === false
    ? null
    : options.rateLimit ?? { ...defaults, routeKey: request.method === "GET" ? "authenticated-read" : "authenticated-mutation" };
  if (policy) {
