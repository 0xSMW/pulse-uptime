...
vi.mock("./config-mutation", async (importOriginal) => ({
  ...
  mutateConfig: vi.fn(async (_principalKey: string, mutator: (config: MonitoringConfig) => MonitoringConfig) => mutator(seededConfig())),
}));
...
import { addGroup, createGroup, deleteGroup, GroupApiError, removeGroup, renameGroup, updateGroup } from "./groups";
