...
describe("token request validation", () => {
  ...
  it("applies the default lifetime when no expiry is requested and the creator is unbounded", () => {
    const result = validateTokenInput({ name: "Default", scopes: ["monitors:read"] }, principal, now);
  ...
  it("clamps the default below a time-bounded creator instead of rejecting", () => {
    const creator = { ...principal, expiresAt: new Date("2026-08-01T00:00:00.000Z") };
    const result = validateTokenInput({ name: "CLI child", scopes: ["monitors:read"] }, creator, now);
  ...
  it("still rejects an explicit expiry beyond a time-bounded creator", () => {
    ...
    expect(() => validateTokenInput({
      ...
      scopes: ["monitors:read"],
      expiresAt: "2026-09-01T00:00:00.000Z",
    }, creator, now)).toThrow(/cannot outlive/);
  });

  it("does not clamp the default for a token principal creator that outlives it", () => {
    const creator = { type: "api_token", scopes: ["monitors:read"], expiresAt: new Date("2027-06-01T00:00:00.000Z") };
    const result = validateTokenInput({ name: "Reader", scopes: ["monitors:read"] }, creator, now);
  ...
  it("rejects a default when the creating credential expires within the safety margin", () => {
    const creator = { ...principal, expiresAt: new Date("2026-07-18T00:00:30.000Z") };
    expect(() => validateTokenInput({ name: "About to expire", scopes: ["monitors:read"] }, creator, now))
