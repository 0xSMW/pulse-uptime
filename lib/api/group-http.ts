...
export function groupError(error: unknown, requestId: string): Response | null {
  ...
  if (error instanceof ConfigMutationError) {
    return apiError(requestId, 503, error.code, error.message)
  }
  if (error instanceof ConfigSizeError) {
...
export function storedGroupError(
  error: GroupApiError,
  requestId: string
): StoredResponse {
  return {
