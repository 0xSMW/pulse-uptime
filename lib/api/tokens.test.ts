...
describe("bearer token secrets", () => {
  it("generates 256-bit random secrets and stores deterministic digests", () => {
    ...
    expect(token.digest.equals(digestBearerToken(token.raw, hashKey))).toBe(
      true
    )
    expect(
      token.digest.equals(digestBearerToken(`${token.raw}x`, hashKey))
    ).toBe(false)
    expect(
      token.digest.equals(digestBearerToken(token.raw, `${hashKey}-different`))
    ).toBe(false)
  })

  it("parses only a complete bearer authorization value", () => {
    expect(parseBearerAuthorization("Bearer pulse_live_secret")).toBe(
      "pulse_live_secret"
    )
    expect(parseBearerAuthorization("bearer\tpulse_cli_secret")).toBe(
      "pulse_cli_secret"
    )
  ...
  it("binds replay credentials to canonical request identity and the persisted operation", () => {
    ...
    const replay = deriveBearerToken(
      credentialDerivationContext({
        ...base,
        body: {
          expiresAt: "2027-01-01T00:00:00.000Z",
    ...
    const changedBody = deriveBearerToken(
      credentialDerivationContext({
        ...base,
        body: { ...base.body, name: "Different" },
      })
    )
    const afterPersistenceExpiry = deriveBearerToken(
      credentialDerivationContext({
        ...base,
