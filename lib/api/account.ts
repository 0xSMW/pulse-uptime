...
import { hashPassword, normalizeEmail, validatePassword, verifyPassword } from "@/lib/auth/credentials";
...
export function validateProfilePatch(input: unknown): ProfilePatch {
  if (!input || typeof input !== "object" || Array.isArray(input)) {
    throw new AccountServiceError("INVALID_PROFILE", "Profile details are required");
  }
  const value = input as Record<string, unknown>;
  const allowedKeys = new Set(["name", "timezone", "avatarImageId"]);
  if (Object.keys(value).some((key) => !allowedKeys.has(key))) {
    throw new AccountServiceError("INVALID_PROFILE", "Profile details contain unsupported fields");
  }
  const patch: ProfilePatch = {};
  if ("name" in value) {
    if (typeof value.name !== "string" || !value.name.trim() || value.name.trim().length > 120) {
      throw new AccountServiceError("INVALID_PROFILE", "Name must contain 1 to 120 characters");
    }
  ...
  if ("timezone" in value) {
    if (value.timezone === null) {
      patch.timezone = null;
    } else if (typeof value.timezone === "string" && value.timezone !== "system" && isValidIanaTimeZone(value.timezone)) {
      patch.timezone = value.timezone;
    } else {
      throw new AccountServiceError("INVALID_PROFILE", "Time zone must be a valid IANA zone name or null");
    }
  }
  ...
  if (Object.keys(patch).length === 0) {
    throw new AccountServiceError("INVALID_PROFILE", "Provide at least one field to update");
  }
  return patch;
}
...
export interface ProfileUpdateStore {
  ...
  applyProfileUpdate(input: {
    ...
    patch: ProfilePatch;
    now: Date;
  }): Promise<(AccountProfile & { previousAvatarImageId: string | null }) | null>;
...
export async function updateAccountProfile(
  ...
  if (patch.avatarImageId) {
    ...
    if (!image || image.kind !== "avatar") {
      throw new AccountServiceError("IMAGE_NOT_FOUND", "Upload the avatar image first, then attach it");
    }
  }
  ...
  if (!result) throw new AccountServiceError("ACCOUNT_NOT_FOUND", "The account no longer exists");
...
const databaseProfileUpdateStore: ProfileUpdateStore = {
  async findAvatarImage(imageId) {
    const [row] = await db.select({ kind: images.kind }).from(images).where(eq(images.id, imageId)).limit(1);
    return row ?? null;
  },
  async applyProfileUpdate({ userId, patch, now }) {
    return db.transaction(async (tx) => {
      ...
      return row ? { ...row, previousAvatarImageId: current.avatarImageId } : null;
    });
  },
  async deleteAvatarImage(imageId) {
    await db.delete(images).where(and(eq(images.id, imageId), eq(images.kind, "avatar")));
  },
};
...
export function validateEmailChangeInput(input: unknown): EmailChangeInput {
  if (!input || typeof input !== "object" || Array.isArray(input)) {
    throw new AccountServiceError("INVALID_EMAIL", "Email change details are required");
  }
  const value = input as Record<string, unknown>;
  const allowedKeys = new Set(["email", "emailConfirm", "currentPassword"]);
  if (Object.keys(value).some((key) => !allowedKeys.has(key))) {
    throw new AccountServiceError("INVALID_EMAIL", "Email change details contain unsupported fields");
  }
  if (typeof value.email !== "string" || typeof value.emailConfirm !== "string" || typeof value.currentPassword !== "string") {
    throw new AccountServiceError("INVALID_EMAIL", "Email, confirmation, and current password are required");
  }
  return { email: value.email, emailConfirm: value.emailConfirm, currentPassword: value.currentPassword };
}

export interface EmailChangeStore {
  findUser(userId: string): Promise<{ email: string; passwordDigest: string } | null>;
...
export async function changeAccountEmail(
  input: EmailChangeInput & { userId: string; currentSessionId: string; ip: string },
  dependencies: EmailChangeDependencies = {},
): Promise<{ email: string }> {
  const email = normalizeEmail(input.email);
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
    throw new AccountServiceError("INVALID_EMAIL", "Enter a valid email address");
  }
  if (normalizeEmail(input.emailConfirm) !== email) {
    throw new AccountServiceError("EMAIL_MISMATCH", "Email addresses do not match");
  }

  const store = dependencies.store ?? databaseEmailChangeStore;
  const user = await store.findUser(input.userId);
  if (!user) throw new AccountServiceError("ACCOUNT_NOT_FOUND", "The account no longer exists");
  ...
  if (!(await verify(user.passwordDigest, input.currentPassword))) {
    throw new AccountServiceError("INVALID_PASSWORD", "Current password is incorrect");
  }
...
export function validatePasswordChangeInput(input: unknown): PasswordChangeInput {
  if (!input || typeof input !== "object" || Array.isArray(input)) {
    throw new AccountServiceError("INVALID_PROFILE", "Password change details are required");
  }
  const value = input as Record<string, unknown>;
  const allowedKeys = new Set(["currentPassword", "newPassword"]);
  if (Object.keys(value).some((key) => !allowedKeys.has(key))) {
    throw new AccountServiceError("INVALID_PROFILE", "Password change details contain unsupported fields");
  }
  if (typeof value.currentPassword !== "string" || typeof value.newPassword !== "string") {
    throw new AccountServiceError("INVALID_PROFILE", "Current and new passwords are required");
  }
...
export async function changeAccountPassword(
  input: PasswordChangeInput & { userId: string; currentSessionId: string; ip: string },
  dependencies: PasswordChangeDependencies = {},
): Promise<{ changed: true }> {
  const policyError = validatePassword(input.newPassword);
  if (policyError) throw new AccountServiceError("PASSWORD_POLICY", policyError);
  ...
  const passwordDigest = await (dependencies.hash ?? hashPassword)(input.newPassword);
...
const databasePasswordChangeStore: PasswordChangeStore = {
  async findUser(userId) {
    const [row] = await db
      .select({ email: adminUsers.email, passwordDigest: adminUsers.passwordDigest })
      .from(adminUsers)
      .where(eq(adminUsers.id, userId))
      .limit(1);
    return row ?? null;
...
export interface SessionRevocationStore {
  revokeSession(input: { userId: string; sessionId: string; now: Date }): Promise<boolean>;
  revokeOtherSessions(input: { userId: string; currentSessionId: string; now: Date }): Promise<number>;
}
...
export async function revokeAccountSession(
  ...
  if (input.sessionId === input.currentSessionId) {
    throw new AccountServiceError("CURRENT_SESSION", "You cannot revoke the session you are currently using");
  }
  ...
  if (!revoked) {
    throw new AccountServiceError("SESSION_NOT_FOUND", "The session was not found or is already signed out");
  }
}
...
const databaseEmailChangeStore: EmailChangeStore = {
  ...
  async applyEmailChange({ userId, currentSessionId, previousEmail, email, now }) {
    await db.transaction(async (tx) => {
      await tx.update(adminUsers).set({ email, updatedAt: now }).where(eq(adminUsers.id, userId));
      await tx
        .update(humanSessions)
        .set({ revokedAt: now })
        .where(and(
      ...
      await tx.update(cliSessions).set({ userEmail: email }).where(eq(cliSessions.userEmail, previousEmail));
      await tx.update(cliInstallations).set({ userEmail: email }).where(eq(cliInstallations.userEmail, previousEmail));
    });
  },
};
