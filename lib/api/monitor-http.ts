...
export function monitorErrorStatus(code: MonitorApiError["code"]): number {
  return code === "MONITOR_NOT_FOUND" ? 404 : code === "MONITOR_EXISTS" ? 409 : code === "INVALID_REQUEST" ? 400 : 503;
}

/** Shared HTTP mapping for the monitors route family. */
export function monitorError(error: unknown, requestId: string): Response | null {
  if (error instanceof MonitorApiError) return apiError(requestId, monitorErrorStatus(error.code), error.code, error.message);
...
// roll back.
export function storedMonitorError(error: unknown, requestId: string): StoredResponse | null {
 ...
 */
export function monitorEnabledRoute(config: { enabled: boolean; routeKey: string }) {
  return async function POST(request: Request, { params }: { params: Promise<{ monitorId: string }> }) {
    const context = await authorize(request, { scope: "monitors:write" });
    if (isApiResponse(context)) return context;
    const id = (await params).monitorId;
    try {
      const result = await executeIdempotent({ request, principalKey: context.principalKey, routeKey: `/api/v1/monitors/${id}/${config.routeKey}`, body: {},
        work: async ({ transaction }) => transaction(async (tx) => {
          try {
            return { status: 200, body: objectEnvelope("Monitor", await setMonitorEnabled(id, config.enabled, context.principalKey, tx), context.requestId) };
          } catch (error) {
