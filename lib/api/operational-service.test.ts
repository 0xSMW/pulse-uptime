...
function enqueueDatabase(options: { monitor?: { id: string } } = { monitor: { id: "monitor-1" } }) {
  ...
  return {
    ...
    insert: () => ({
      values: () => ({
        onConflictDoNothing: () => ({ returning: async () => [{ id: "outbox-1" }] }),
      }),
    }),
  } as never;
}
...
describe("operational service seams", () => {
  ...
  it("enqueues an explicit recipient without ever reading the accepted config", async () => {
    ...
    const service = createOperationalService({ database: enqueueDatabase(), getStatus: async () => ({}) });
    await expect(service.enqueueTestNotification({ recipient: "alerts@example.com", testId: "t1" }))
  ...
  it("falls back to RECIPIENT_REQUIRED when the accepted config cannot be read", async () => {
    findAcceptedSnapshot.mockRejectedValue(new Error("hash mismatch"));
    const service = createOperationalService({ database: enqueueDatabase(), getStatus: async () => ({}) });
    await expect(service.enqueueTestNotification({ testId: "t2" })).rejects.toThrow(OperationalInputError);
    await expect(service.enqueueTestNotification({ testId: "t2" })).rejects.toThrow("A configured recipient is required");
  });

  it("uses a default recipient from a healthy accepted config", async () => {
    findAcceptedSnapshot.mockResolvedValue({ config: { settings: { defaultRecipients: ["default@example.com"] } } });
    const service = createOperationalService({ database: enqueueDatabase(), getStatus: async () => ({}) });
    await expect(service.enqueueTestNotification({ testId: "t3" }))
      .resolves.toEqual({ id: "outbox-1", state: "accepted" });
  });
});
