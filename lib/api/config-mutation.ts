...
export async function requireAcceptedConfig(executor: typeof db = db): Promise<AcceptedConfigSnapshot> {
  ...
  if (!snapshot) throw new ConfigMutationError("CONFIGURATION_UNAVAILABLE", "No accepted monitoring configuration is available");
...
async function writeEdgeConfig(config: MonitoringConfig): Promise<void> {
  try {
    ...
    throw new ConfigMutationError("EDGE_CONFIG_UNAVAILABLE", "Could not update Edge Config");
  }
}
