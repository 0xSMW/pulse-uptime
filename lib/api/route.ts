...
export function success<T>(kind: string, data: T, requestId: string, status = 200) {
  return apiJson(objectEnvelope(kind, data, requestId), { status });
}

export function storedSuccess<T>(kind: string, data: T, requestId: string, status = 200): StoredResponse {
