(() => {
  ...
  function highlight(source, rules) {
    ...
    while (index < source.length) {
      ...
      for (const [cls, regex, wordStart] of rules) {
        if (wordStart && index > 0 && isWordChar(source[index - 1])) continue;
  ...
  $$(".docs-main h2[id], .docs-main h3[id], .docs-main h4[id]").forEach((heading) => {
  ...
  const spyTargets = [...new Set(sidebarLinks.map((link) => link.hash.slice(1)))]
  ...
  function updateSpy() {
    ...
    for (const target of spyTargets) {
      if (target.offsetTop <= cursor) current = target;
      else break;
    }
  ...
  const PALETTE_COMMANDS = sidebarLinks.map((link) => ({
    ...
    hint: link.closest(".sidebar-group").querySelector("p").textContent.trim().toLowerCase(),
    run: () => {
  ...
  function renderPalette(query = "") {
    ...
    paletteMatches = PALETTE_COMMANDS.filter((command) => command.label.toLowerCase().includes(value));
    ...
    $$(".palette-item", paletteList).forEach((item) => {
      ...
      item.addEventListener("mousemove", () => {
        ...
        $$(".palette-item", paletteList).forEach((other) =>
          other.classList.toggle("selected", Number(other.dataset.index) === paletteIndex),
        );
      });
    });
  }
