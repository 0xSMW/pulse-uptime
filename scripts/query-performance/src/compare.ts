...
function pctDelta(
  ...
  if (baselineValue === 0) {
    return candidateValue === 0 ? 0 : null
  }
...
function compareFixtureMetadata(
  ...
  for (const key of allKeys) {
    const inBaseline = Object.hasOwn(baseline.cardinalities, key)
    const inCandidate = Object.hasOwn(candidate.cardinalities, key)
    if (inBaseline && !inCandidate) {
      reasons.push(
        `Cardinality key "${key}" is missing from the candidate fixture.`
      )
      continue
    }
    if (!inBaseline && inCandidate) {
      reasons.push(
        `Cardinality key "${key}" is extra in the candidate fixture.`
      )
      continue
    }
    const baselineValue =
      baseline.cardinalities[key as keyof typeof baseline.cardinalities]
    const candidateValue =
      candidate.cardinalities[key as keyof typeof candidate.cardinalities]
...
function compareCase(
  ...
  if (!baselineResult) {
    return {
      name,
      verdict: "missing-in-baseline",
      baseline: null,
      candidate: medianSample(candidateResult!.samples),
      executionTimeDeltaPct: null,
      bufferReadDeltaPct: null,
      rowCountChanged: false,
  ...
  if (!candidateResult) {
    return {
      name,
      verdict: "missing-in-candidate",
      baseline: medianSample(baselineResult.samples),
      candidate: null,
      executionTimeDeltaPct: null,
      bufferReadDeltaPct: null,
      rowCountChanged: false,
  ...
  const executionTimeDeltaPct = pctDelta(
    baseline.executionTimeMs,
    candidate.executionTimeMs
  )
  const bufferReadDeltaPct = pctDelta(
    baseline.sharedReadBlocks,
    candidate.sharedReadBlocks
  )
  const rowCountChanged = baseline.rootRows !== candidate.rootRows

  const reasons: string[] = []
  let regressed = false
  const absoluteTimeDeltaMs =
    candidate.executionTimeMs - baseline.executionTimeMs
  ...
  const absoluteBufferDelta =
    candidate.sharedReadBlocks - baseline.sharedReadBlocks
  // Compare candidate reads to the absolute threshold when baseline reads are zero.
  const zeroBaselineBufferRegression =
    baseline.sharedReadBlocks === 0 &&
    candidate.sharedReadBlocks > thresholds.minAbsoluteBlocks
  if (
  ...
  if (rowCountChanged) {
    reasons.push(
      `Root row count changed (${baseline.rootRows} -> ${candidate.rootRows}) — verify the candidate still returns equivalent results.`
    )
  }
...
export function compareArtifacts(
  ...
  const { hasMetadataMismatch, metadataMismatchReasons } = compareRunMetadata(
    baseline,
    candidate
  )
  ...
  const candidateByName = new Map(
    candidate.results.map((result) => [result.name, result])
  )
  const names = [
    ...new Set([...baselineByName.keys(), ...candidateByName.keys()]),
  ].sort()

  const cases = names.map((name) =>
    compareCase(
      name,
      baselineByName.get(name),
      candidateByName.get(name),
      thresholds
    )
  )
  const hasRegression = cases.some((entry) => entry.verdict === "regressed")
  // Candidate-only cases are allowed.
  const hasMissingCases = cases.some(
    (entry) => entry.verdict === "missing-in-candidate"
  )
