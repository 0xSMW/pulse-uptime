...
const SEED = 0x51_48_5f_31 // Fixed seed for reproducible fixture shape.
...
export function buildMonitorPlan(): MonitorPlan[] {
  ...
  const states: MonitorPlan["state"][] = monitorStateDistribution.flatMap(
    (entry) => Array.from({ length: entry.count }, () => entry.state)
  )
  ...
  return Array.from({ length: MONITOR_COUNT }, (_, zeroBased) => {
    ...
    return {
      ...
      enabled:
        states[zeroBased] !== "ARCHIVED" && states[zeroBased] !== "PAUSED",
...
async function resetFixtureData(conn: GatedConnection): Promise<void> {
  ...
  await db
    .delete(schema.monitorExceptions)
    .where(dsql`${schema.monitorExceptions.monitorId} like 'qh-%'`)
  await db
    .delete(schema.notificationOutbox)
    .where(dsql`${schema.notificationOutbox.monitorId} like 'qh-%'`)
  await db
    .delete(schema.incidents)
    .where(dsql`${schema.incidents.monitorId} like 'qh-%'`)
  await db
    .delete(schema.checkResults)
    .where(dsql`${schema.checkResults.monitorId} like 'qh-%'`)
  await db
    .delete(schema.metricRollups)
    .where(dsql`${schema.metricRollups.monitorId} like 'qh-%'`)
  await db
    .delete(schema.monitorState)
    .where(dsql`${schema.monitorState.monitorId} like 'qh-%'`)
  ...
  await db
    .delete(schema.checkBatches)
    .where(
      dsql`exists (select 1 from unnest(${schema.checkBatches.monitorIds}) as m where m like 'qh-%')`
    )
  await db
    .delete(schema.monitorRegistry)
    .where(dsql`${schema.monitorRegistry.id} like 'qh-%'`)

  await db
    .delete(schema.monitoringConfigSnapshots)
    .where(dsql`${schema.monitoringConfigSnapshots.source} = 'qh-fixture'`)
  await db
    .delete(schema.configChangeApprovals)
    .where(
      dsql`${schema.configChangeApprovals.createdByPrincipal} = 'qh-fixture'`
    )
  await db
    .delete(schema.configOperations)
    .where(dsql`${schema.configOperations.principalKey} = 'qh-fixture'`)
  await db
    .delete(schema.cronRuns)
    .where(dsql`${schema.cronRuns.jobName} like 'qh-%'`)
  await db
    .delete(schema.jobLeases)
    .where(dsql`${schema.jobLeases.name} like 'qh-%'`)
  ...
  await db
    .delete(schema.cliInstallations)
    .where(dsql`${schema.cliInstallations.installationKey} like 'qh-%'`)
  await db
    .delete(schema.deviceAuthorizations)
    .where(dsql`${schema.deviceAuthorizations.installationKey} like 'qh-%'`)
  await db
    .delete(schema.apiIdempotency)
    .where(dsql`${schema.apiIdempotency.principalKey} = 'qh-fixture'`)
  await db
    .delete(schema.apiRateLimitBuckets)
    .where(dsql`${schema.apiRateLimitBuckets.principalKey} = 'qh-fixture'`)
}
...
async function insertMonitors(
  ...
  for (const rows of chunk(plan, CHUNK_SIZE)) {
    await db.insert(schema.monitorState).values(
      rows.map((monitor) => {
        const failing =
          monitor.state === "DOWN" || monitor.state === "VERIFYING_DOWN"
        return {
...
export type OutboxIncidentSeed = Pick<
  IncidentPlan,
  "id" | "monitorId" | "openedAt" | "resolvedAt"
>
export type NotificationOutboxFixtureRow =
  typeof schema.notificationOutbox.$inferInsert
...
export function fixtureRecipientsForIncident(position: number): string[] {
  const count =
    position % 10 === 0 ? MAX_FIXTURE_RECIPIENTS : position % 3 === 0 ? 5 : 1
  return Array.from({ length: count }, (_, index) =>
    index === 0
      ? `oncall@${FIXTURE_EMAIL_DOMAIN}`
      : `oncall-${index}@${FIXTURE_EMAIL_DOMAIN}`
  )
}
...
export function buildNotificationOutboxRows(
    ...
    for (const event of events) {
      for (const recipient of recipients) {
        ...
        outboxRows.push({
          ...
          recipient,
          idempotencyKey: incidentNotificationKey(
            incident.id,
            event.kind,
            recipient
          ),
          ...
          nextAttemptAt:
            status === "pending" || status === "failed" ? now : event.at,
          claimToken: null,
          claimedAt: null,
          providerMessageId:
            status === "sent" ? `qh-fixture-msg-${createId()}` : null,
          lastError:
            status === "failed" || status === "dead"
              ? "qh-fixture synthetic delivery failure"
              : null,
  ...
  if (outboxRows.length > 0) {
    const target =
      outboxRows.find((row) => row.status === "sent") ?? outboxRows[0]!
...
async function insertChecksIncidentsAndOutbox(
  ...
  const checkCountPerMonitor =
    (CHECK_HISTORY_HOURS * 60) / CHECK_INTERVAL_MINUTES
  ...
  for (const monitor of plan) {
    ...
    for (let step = checkCountPerMonitor; step >= 1; step -= 1) {
      const scheduledAt = new Date(
        NOW.getTime() - step * CHECK_INTERVAL_MINUTES * 60_000
      )
      ...
      checkRows.push({
        ...
        latencyMs: successful
          ? intBetween(rand, 30, 400)
    ...
    if (failing) {
      const openedAt = new Date(
        NOW.getTime() - failureRunLength * CHECK_INTERVAL_MINUTES * 60_000
      )
      incidentPlans.push({
        id: randomUUID(),
        monitorId: monitor.id,
      ...
      for (
        ...
        const resolvedAt = new Date(
          openedAt.getTime() + durationMinutes * 60_000
        )
        incidentPlans.push({
          id: randomUUID(),
          monitorId: monitor.id,
    ...
    if (checkRows.length >= CHUNK_SIZE * 4) {
      for (const rows of chunk(checkRows, CHUNK_SIZE)) {
  ...
  const outboxRows = buildNotificationOutboxRows(incidentPlans, {
    now: NOW,
    rand,
  })
  for (const rows of chunk(outboxRows, CHUNK_SIZE)) {
    await db.insert(schema.notificationOutbox).values(rows)
  }

  return {
    incidentCount: incidentPlans.length,
...
function* bucketStarts(
  ...
  for (let index = count; index >= 1; index -= 1) {
    yield new Date(end.getTime() - index * stepMs)
  }
}

async function insertRollups(
  conn: GatedConnection,
  plan: MonitorPlan[]
): Promise<{ metricRollupCount: number }> {
  const { db } = conn
  ...
  const resolutions: {
  ...
  }[] = [
    {
      ...
      days: ROLLUP_15M_DAYS,
      stepMs: 15 * 60_000,
    },
    {
      resolution: "hour",
      end: endHour,
      days: ROLLUP_HOUR_DAYS,
      stepMs: 3_600_000,
    },
    {
      resolution: "day",
      end: endDay,
      days: ROLLUP_DAY_DAYS,
      stepMs: 86_400_000,
    },
  ]
  for (const monitor of plan) {
    ...
    for (const { resolution, end, days, stepMs } of resolutions) {
      ...
      for (const bucketStart of bucketStarts(end, bucketCount, stepMs)) {
        ...
        const latencySum = histogram.reduce(
          (sum, value, index) =>
            sum + value * [80, 200, 400, 800, 2000, 4000, 8000, 15_000][index]!,
          0
        )
        metricRows.push({
          monitorId: monitor.id,
      ...
      if (metricRows.length >= CHUNK_SIZE * 4) {
        for (const rows of chunk(metricRows, CHUNK_SIZE)) {
          await db.insert(schema.metricRollups).values(rows)
        }
        metricRows = []
      }
    }
...
export type FixtureConfigMonitor = Pick<
  MonitorPlan,
  "id" | "name" | "url" | "groupName" | "enabled"
>
...
export function buildAcceptedFixtureConfig(
  ...
  const config = validateMonitoringConfig({
    ...
    monitors: plan.map((monitor) => ({
      ...
      url: monitor.url,
      method: "GET",
      groupId: monitor.groupName
        ? `qh-group-${monitor.groupName.toLowerCase().replace(/\s+/g, "-")}`
        : null,
      enabled: monitor.enabled,
...
async function insertConfigAndOperations(
  conn: GatedConnection,
  plan: MonitorPlan[]
): Promise<void> {
  const { db } = conn
  ...
  const operationRows = Array.from({ length: 10 }, (_, index) => ({
    ...
    writtenAt: new Date(NOW.getTime() - index * 600_000 + 1000),
    acceptedAt: null,
    failedAt: null,
  }))
...
export async function insertMaintenanceAndScheduler(
  conn: GatedConnection,
  plan: MonitorPlan[]
): Promise<{ cronRunCount: number; checkBatchCount: number }> {
  const { db } = conn
  ...
  const cronRows = Array.from({ length: 200 }, (_, index) => {
    ...
    return {
      ...
      completedAt:
        status === "completed"
          ? new Date(scheduledMinute.getTime() + 5000)
          : null,
  ...
  for (const rows of chunk(cronRows, CHUNK_SIZE)) {
    await db.insert(schema.cronRuns).values(rows)
  }
  ...
  for (let minute = SCHEDULER_BATCH_MINUTES; minute >= 1; minute -= 1) {
    ...
    const latencyValues = encodeLatencies(
      monitorIds.map(() => intBetween(rand, 30, 900))
    )
    checkBatchRows.push({
      scheduledMinute,
      encodingVersion: 1,
      ...
      schedulerCompletedAt: new Date(scheduledMinute.getTime() + 3000),
      createdAt: scheduledMinute,
    })
    commitRows.push({
      scheduledMinute,
      stateMutationCount: monitorIds.length,
      committedAt: new Date(scheduledMinute.getTime() + 3500),
    })
  }
  // Insert batches before commits to satisfy the `scheduled_minute` foreign key.
  // Insert each table in chunks.
  for (const rows of chunk(checkBatchRows, CHUNK_SIZE)) {
    await db.insert(schema.checkBatches).values(rows)
  }
  for (const rows of chunk(commitRows, CHUNK_SIZE)) {
    await db.insert(schema.atomicMinuteCommits).values(rows)
  }
  ...
  const usageSnapshots = Array.from({ length: 30 }, (_, index) => {
    ...
    return {
      ...
      categoryBytes: {
        rollups: Number(storageBytes) * 0.4,
        coreData: Number(storageBytes) * 0.2,
      },
  ...
  for (const rows of chunk(usageSnapshots, CHUNK_SIZE)) {
    await db.insert(schema.databaseUsageSnapshots).values(rows)
  }

  return { cronRunCount: cronRows.length, checkBatchCount }
}

async function insertExceptionsAndAuth(
  conn: GatedConnection,
  plan: MonitorPlan[]
): Promise<{ exceptionCount: number; adminCount: number }> {
  const { db } = conn
  const rand = mulberry32(SEED + 6)
  const failingMonitors = plan.filter(
    (monitor) => monitor.state === "DOWN" || monitor.state === "VERIFYING_DOWN"
  )
  ...
  if (payloadRows.length > 0) {
    await db.insert(schema.exceptionPayloads).values(payloadRows)
  }
  ...
  const exceptionRows = failingMonitors.map((monitor, index) => ({
    ...
    worstLatencyMs: intBetween(rand, 1000, 9000),
    incidentId: null,
    payloadId: payloadRows[index]?.id ?? null,
  }))
  if (exceptionRows.length > 0) {
    await db.insert(schema.monitorExceptions).values(exceptionRows)
  }
  ...
  await db.insert(schema.adminUsers).values(
    adminIds.map((id, index) => ({
      ...
      onboardingCompletedAt:
        index === 0 ? new Date(NOW.getTime() - 89 * 86_400_000) : null,
    }))
  )

  await db.insert(schema.humanSessions).values(
    adminIds.map((userId) => ({
      id: randomUUID(),
      userId,
  ...
  await db.insert(schema.cliInstallations).values(
  ...
  await db.insert(schema.cliSessions).values(
    installationIds.map((installationId, index) => ({
      id: randomUUID(),
      installationId,
...
export async function deleteFixtureMarker(
  conn: GatedConnection
): Promise<void> {
...
export async function seedFixture(
  ...
  const { incidentCount, outboxCount, checkCount } =
    await insertChecksIncidentsAndOutbox(conn, plan)
  const { metricRollupCount } = await insertRollups(conn, plan)
  await insertConfigAndOperations(conn, plan)
  const { cronRunCount, checkBatchCount } = await insertMaintenanceAndScheduler(
    conn,
    plan
  )
  const { exceptionCount, adminCount } = await insertExceptionsAndAuth(
    conn,
    plan
  )
