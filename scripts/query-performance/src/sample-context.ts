...
export async function loadSampleContext(
  ...
  if (monitors.length === 0) {
    throw new Error(
      "Fixture has no monitors — run the seed-fixture command before benchmarking."
    )
  }

  // Match the production join, ordering, and 100-row limit.
  const listedIncidents = await db
  ...
  const [ongoing] = await db
    .select({ id: schema.incidents.id, monitorId: schema.incidents.monitorId })
    .from(schema.incidents)
