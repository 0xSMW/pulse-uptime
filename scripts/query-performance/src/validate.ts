...
function countPlaceholders(text: string): number {
  ...
  return matches.length === 0
    ? 0
    : Math.max(...matches.map((match) => Number(match.slice(1))))
}
...
export function validateInventoryStatically(): ValidationReport {
  ...
  for (const queryCase of queryCases) {
    if (names.has(queryCase.name)) {
      issues.push({
        scope: queryCase.name,
        message: "Duplicate query case name.",
      })
    }
    names.add(queryCase.name)
    if (!queryCase.description.trim()) {
      issues.push({ scope: queryCase.name, message: "Missing description." })
    }
    if (!queryCase.source.trim()) {
      issues.push({ scope: queryCase.name, message: "Missing source pointer." })
    }
  }
  for (const excluded of excludedQueries) {
    if (names.has(excluded.name)) {
      issues.push({
        scope: excluded.name,
        message: "Excluded query name collides with an included query case.",
      })
    }
    if (!excluded.reason.trim()) {
      issues.push({
        scope: excluded.name,
        message: "Excluded query is missing an exclusion reason.",
      })
    }
    if (!excluded.source.trim()) {
      issues.push({
        scope: excluded.name,
        message: "Excluded query is missing a source pointer.",
      })
    }
  }
  ...
  const sql = postgres(
    "postgres://validate:validate@0.0.0.0:1/validate-never-connects",
    { max: 1 }
  )
  ...
  for (const queryCase of queryCases) {
    try {
      ...
      if (!built.text.trim()) {
        issues.push({
          scope: queryCase.name,
          message: "Built query has empty SQL text.",
        })
        continue
      }
      ...
      issues.push({
        scope: queryCase.name,
        message: `Failed to build query statically: ${error instanceof Error ? error.message : String(error)}`,
      })
    }
  }
  ...
  return {
    queryCaseCount: queryCases.length,
...
function scanForForbiddenEnvUsage(): ValidationIssue[] {
  ...
  for (const entry of readdirSync(srcDir, { withFileTypes: true })) {
    ...
    const contents = stripComments(
      readFileSync(resolve(srcDir, entry.name), "utf8")
    )
    for (const pattern of FORBIDDEN_PATTERNS) {
      if (pattern.test(contents)) {
        issues.push({
          scope: entry.name,
          message: `File matches forbidden pattern ${pattern} — this tool must never read ambient DB env vars.`,
        })
      }
    }
  }
...
async function main() {
  ...
  if (report.ok) {
    console.error(
      `[validate] ${report.queryCaseCount} query case(s) and ${report.excludedQueryCount} excluded entries validated statically — no connection opened.`
    )
  } else {
