...
const FAKE_SAMPLE_CONTEXT: SampleContext = {
  ...
  monitorIds: Array.from(
    { length: 10 },
    (_, index) => `qh-monitor-${String(index + 1).padStart(4, "0")}`
  ),
  groupSlug: "api",
  incidentIds: [
...
function fakeConnection(): GatedConnection {
  const sql = postgres("postgres://test:test@0.0.0.0:1/test-never-connects", {
    max: 1,
  })
...
function findCase(name: string) {
  ...
  if (!found) {
    throw new Error(`query case not found: ${name}`)
  }
  return found
}
  ...
  return params
    .filter(
      (param): param is string =>
        typeof param === "string" && ISO_DATE.test(param)
    )
...
describe("dashboard monitor query sources", () => {
  it("references production monitoring queries", () => {
    expect(findCase("dashboard-monitors-uptime24h").source).toMatch(
      /^lib\/monitoring\/queries\.ts/
    )
    expect(findCase("command-palette-monitors").source).toMatch(
      /^lib\/monitoring\/queries\.ts/
    )
    expect(findCase("dashboard-monitors-uptime24h").source).not.toContain(
      "lib/reporting/queries/status.ts"
    )
    expect(findCase("command-palette-monitors").source).not.toContain(
      "lib/reporting/queries/status.ts"
    )
  })
})
...
describe("dashboard monitor incident join", () => {
  it("joins only open incidents by monitor", () => {
    const built = findCase("dashboard-monitors-uptime24h").build(
      fakeConnection(),
      FAKE_SAMPLE_CONTEXT
    )
...
describe("monitor detail recent incidents", () => {
  it("binds the incident-bearing monitor when present", () => {
    ...
    const built = findCase("monitor-detail-recent-incidents").build(
      fakeConnection(),
      ctx
    )
...
describe("incidents notification summary", () => {
  it("binds every listed incident ID in the IN predicate", () => {
    ...
    const built = findCase("incidents-notification-summary").build(
      fakeConnection(),
      ctx
    )
    ...
    expect(
      built.params.filter(
        (param) => typeof param === "string" && param.startsWith("00000000-")
      )
    ).toEqual(incidentIds)
  })
  ...
  it("uses the deterministic sentinel when no incidents exist", () => {
    ...
    expect(
      built.params.filter((param) => param === EMPTY_INCIDENT_SENTINEL)
    ).toHaveLength(1)
  })
})
...
describe("monitor detail rollup windows", () => {
  ...
  it("scans seven days for 15m rollups", () => {
    const built = findCase("monitor-detail-rollups-7d").build(
      fakeConnection(),
      FAKE_SAMPLE_CONTEXT
    )
  ...
  it("scans 30 and 90 days for hour and day rollups", () => {
    const thirtyDay = findCase("monitor-detail-rollups-30d").build(
      fakeConnection(),
      FAKE_SAMPLE_CONTEXT
    )
    ...
    const ninetyDay = findCase("monitor-detail-rollups-90d").build(
      fakeConnection(),
      FAKE_SAMPLE_CONTEXT
    )
  ...
  it("references production monitor reporting queries", () => {
    for (const name of [
      "monitor-detail-rollups-7d",
      "monitor-detail-rollups-30d",
      "monitor-detail-rollups-90d",
    ]) {
      expect(findCase(name).source).toMatch(
        /^lib\/reporting\/queries\/monitors\.ts/
      )
    }
  })
})
...
describe("public status incident overfetch limits", () => {
  it("fetches 60 resolved-history rows to mirror RECENT_INCIDENTS_FETCH_LIMIT, not the display count of 10", () => {
    const built = findCase("public-status-recent-incidents-resolved").build(
      fakeConnection(),
      FAKE_SAMPLE_CONTEXT
    )
  ...
  it("fetches 500 current-incident rows to mirror CURRENT_INCIDENTS_FETCH_LIMIT", () => {
    const built = findCase("public-status-current-incidents").build(
      fakeConnection(),
      FAKE_SAMPLE_CONTEXT
    )
    expect(built.text).toMatch(/limit \$\d+$/i)
