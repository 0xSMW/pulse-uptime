...
describe("buildNotificationOutboxRows", () => {
  ...
  function expectedRowCount(
    incidents: ReturnType<typeof sampleIncidents>
  ): number {
    return incidents.reduce(
      (sum, incident, position) =>
        sum +
        (1 + (incident.resolvedAt ? 1 : 0)) *
          fixtureRecipientsForIncident(position).length,
      0
    )
  }
  ...
  it("always seeds at least one stale sending row with a valid claim pair", () => {
    ...
    for (const row of staleSending) {
      ...
      expect(row.claimedAt!.getTime()).toBe(
        now.getTime() - STALE_SENDING_CLAIM_AGE_MS
      )
      expect(row.claimedAt!.getTime()).toBeLessThan(
        now.getTime() - STALE_CLAIM_CUTOFF_MS
      )
  ...
  it("fans events out across deterministic recipients with production idempotency keys", () => {
    ...
    const maxFanOut = rows.filter(
      (row) =>
        row.incidentId === incidents[0]!.id &&
        row.eventType === "incident.opened"
    )
    expect(maxFanOut).toHaveLength(MAX_FIXTURE_RECIPIENTS)
    expect(new Set(maxFanOut.map((row) => row.recipient)).size).toBe(
      MAX_FIXTURE_RECIPIENTS
    )

    // At least part of the fixture keeps a single recipient per event.
    const singleFanOut = rows.filter(
      (row) => row.incidentId === incidents[1]!.id
    )
    ...
    for (const row of rows) {
      ...
      expect(row.idempotencyKey).toBe(
        incidentNotificationKey(row.incidentId!, kind, row.recipient)
      )
    }
  ...
  it("derives recipient fan-out purely from incident position", () => {
    ...
    expect(fixtureRecipientsForIncident(10)).toEqual(
      fixtureRecipientsForIncident(10)
    )
...
describe("buildAcceptedFixtureConfig", () => {
  it("validates and hashes a seeded accepted monitoring config with production contracts", () => {
    ...
    expect(new Set(revalidated.groups.map((group) => group.name))).toEqual(
      new Set(GROUP_NAMES)
    )
  ...
  it("accepts the full 100-monitor fixture shape under the production size limit", () => {
    const plan = Array.from({ length: MONITOR_COUNT }, (_, zeroBased) => {
      ...
      return {
        ...
        groupName:
          zeroBased % 10 === 0
            ? null
            : GROUP_NAMES[zeroBased % GROUP_NAMES.length]!,
    ...
    const { config, configHash } = buildAcceptedFixtureConfig(plan)
    expect(validateMonitoringConfig(config).monitors).toHaveLength(
      MONITOR_COUNT
    )
...
describe("buildMonitorPlan", () => {
  it("seeds paused and archived monitors disabled and every other state enabled", () => {
    ...
    for (const monitor of plan) {
      const shouldBeDisabled =
        monitor.state === "PAUSED" || monitor.state === "ARCHIVED"
    ...
    expect(plan.filter((monitor) => !monitor.enabled)).toHaveLength(
      expectedDisabled
    )
  })
  ...
  it("propagates disabled paused monitors into the accepted fixture config", () => {
    ...
    const enabledById = new Map(
      config.monitors.map((monitor) => [monitor.id, monitor.enabled])
    )
...
describe("insertMaintenanceAndScheduler", () => {
  ...
  it("batches all 60 rows per table into a single chunked insert instead of one insert per minute", async () => {
    ...
    const checkBatchInserts = inserts.filter(
      (entry) => entry.table === schema.checkBatches
    )
    const commitInserts = inserts.filter(
      (entry) => entry.table === schema.atomicMinuteCommits
    )
...
describe("raw check history window", () => {
  ...
  it("keeps total check_results cardinality bounded for the harness", () => {
    expect(MONITOR_COUNT * CHECK_HISTORY_HOURS * 60).toBeLessThanOrEqual(
      150_000
    )
  })
})
...
describe("CHUNK_SIZE parameter budget", () => {
  it("keeps a full chunk of the widest fixture row (metric_rollups) under the Postgres per-statement bind-parameter limit", () => {
    const metricRollupColumnCount = Object.keys(
      getTableColumns(schema.metricRollups)
    ).length
    ...
    expect(metricRollupColumnCount).toBeGreaterThanOrEqual(
      widestOtherTableColumnCount
    )

    expect(CHUNK_SIZE * metricRollupColumnCount).toBeLessThan(
