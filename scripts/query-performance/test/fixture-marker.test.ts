...
import { FIXTURE_CLOCK, deleteFixtureMarker, resetFixture, seedFixture } from "../src/fixtures";
...
describe("deleteFixtureMarker", () => {
  it("issues a delete against the marker table for the qh-fixture tag", async () => {
    ...
    expect(log[0]).toMatchObject({ type: "delete-marker", values: ["qh-fixture"] });
  });
});
...
describe("seedFixture marker clock", () => {
  it("writes the fixture clock into seeded_at instead of relying on database now()", async () => {
    ...
    expect(insertMarker!.text).toContain("(tag, version, seeded_at, cardinalities)");
    const boundDates = insertMarker!.values!.filter((value): value is Date => value instanceof Date);
