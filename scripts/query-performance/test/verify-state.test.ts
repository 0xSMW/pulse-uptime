...
function exactFixtureProof(overrides: Partial<RetainedStateProof> = {}): RetainedStateProof {
  return {
    ...
    tables: [{ table: "monitoring_config_snapshots", expected: 2, actual: 2, matches: true, nonZero: true }],
    allMatch: true,
    allNonZero: true,
    safeScope: true,
    ...overrides,
...
describe("passesRetainedStateCliGate", () => {
  ...
  it("still fails for cardinality, empty tables, and version mismatches", () => {
    expect(passesRetainedStateCliGate(exactFixtureProof({ allMatch: false }))).toBe(false);
    expect(passesRetainedStateCliGate(exactFixtureProof({ allNonZero: false }))).toBe(false);
    expect(passesRetainedStateCliGate(exactFixtureProof({ fixtureVersion: FIXTURE_VERSION + 1 }))).toBe(false);
    expect(passesRetainedStateCliGate(exactFixtureProof({ fixtureVersion: null }))).toBe(false);
  });
});
