...
const NEON_DIRECT = `postgresql://pulse_owner:${encodeURIComponent(SECRET)}@ep-cool-name-123456.us-east-2.aws.neon.tech/neondb?sslmode=require`
const NEON_POOLER = `postgresql://pulse_owner:${encodeURIComponent(SECRET)}@ep-cool-name-123456-pooler.us-east-2.aws.neon.tech/neondb?sslmode=require`
...
describe("migrate-deploy main", () => {
  it("skips when not production and never connects", async () => {
    ...
    expect(logs.info.some((line) => line.includes("migrate.skipped"))).toBe(
      true
    )
    logs.restore()
  })
  ...
  it("rejects a pooler URL and never calls postgres()", async () => {
    ...
    expect(
      logs.info.some((line) => line.includes("migrate.url.rejected"))
    ).toBe(true)

    const all = [
      ...logs.info,
      ...logs.error,
      String(exitWithError.mock.calls[0]?.[0]),
    ].join("\n")
  ...
  it("rejects explicit pgbouncer options and never calls postgres()", async () => {
    ...
    const exitWithError = vi.fn()
    const logs = captureLogs()
    const url = `postgresql://pulse_owner:${encodeURIComponent(SECRET)}@ep-cool-name-123456.us-east-2.aws.neon.tech/neondb?pgbouncer=true`
  ...
  it("logs sanitized host and database on success path before connecting", async () => {
    ...
    const sql = Object.assign(vi.fn().mockResolvedValue([{ acquired: true }]), {
      end,
    })
    ...
    expect(connect).toHaveBeenCalledWith(NEON_DIRECT, {
      max: 1,
    ...
    const validatedLine = logs.info.find((line) =>
      line.includes("migrate.url.validated")
    )
    expect(validatedLine).toBeDefined()
    expect(validatedLine).toContain(
  ...
  it("fails when DATABASE_URL_UNPOOLED is unset without connecting", async () => {
    ...
    expect(connect).not.toHaveBeenCalled()
    expect(exitWithError.mock.calls[0]?.[0]).toMatch(
